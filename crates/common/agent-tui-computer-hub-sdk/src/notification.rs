                ...
                match (frame_result, session_id) {
                    (Ok(frame), Some(session_id)) => {
                        if frame
                            ...
                            && let agent_tui_tool_protocol::notification_wire::WireToolNotification::Custom(ref c) = frame.notification
    ...
    fn parse_tool_server_status_changed() {
        ...
        match notif {
            HubNotification::ToolServerStatusChanged { session_id, status } => {
                ...
                assert_eq!(
                    status.status,
                    agent_tui_tool_protocol::ToolServerLifecycleStatus::Busy
                );
