...
//! ```rust
//! use serde::Serialize;
//! use agent_tui_tool_runtime::render::ToolOutput;
...
/// Lets `agent_tui_tool_types::TaskOutputOutput` be used directly as a `Tool::Output`
/// (handy for stub/test tools and pass-through proxies).
impl ToolOutput for agent_tui_tool_types::TaskOutputOutput {}

/// Lets `agent_tui_tool_types::SubagentCompletedOutput` be used directly as a
/// `Tool::Output` (the `task` tool's structured completion output).
impl ToolOutput for agent_tui_tool_types::SubagentCompletedOutput {}

/// Lets `agent_tui_tool_types::KillTaskOutput` be used directly as a `Tool::Output`
/// (the `kill_task` tool's typed result / not-found output).
impl ToolOutput for agent_tui_tool_types::KillTaskOutput {}
