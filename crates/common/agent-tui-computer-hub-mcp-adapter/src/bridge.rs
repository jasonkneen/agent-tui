...
use agent_tui_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use agent_tui_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`agent_tui_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`agent_tui_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl agent_tui_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            agent_tui_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == agent_tui_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
