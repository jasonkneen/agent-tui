...
/// `agent_tui_file_utils::events::SessionRelationship::Primary`).
pub const DEFAULT_SESSION_RELATIONSHIP: &str = "primary";

/// Default `schema_version` wire value. Bare literal (not the
/// `agent-tui-file-utils` constant) to avoid a dependency cycle.
...
pub struct BeforeTurnPayload {
    ...
    /// (`"primary"` | `"subagent"`). A `String`, not the `agent-tui-file-utils`
...
pub struct AfterTurnPayload {
    ...
    /// the workspace decodes it into the `agent-tui-file-utils`
