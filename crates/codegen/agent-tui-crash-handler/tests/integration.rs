//! Integration tests for agent-tui-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = agent_tui_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    agent_tui_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob = agent_tui_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report =
        agent_tui_crash_handler::check_previous_crash(tmp.path()).expect("should produce a crash report");
...
fn clean_exit_does_not_produce_crash_report() {
    ...
    let report = agent_tui_crash_handler::check_previous_crash(tmp.path());
