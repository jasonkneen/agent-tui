...
use agent_tui_tools::types::{
    output::{MCPOutput, MCPOutputDetails, ToolOutput},
    tool::{ToolKind, ToolNamespace},
    tool_metadata::ToolMetadata,
};
use agent_tui_tools::util::{ProcessGroup, ProcessScope};

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `agent_tui_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use agent_tui_workspace_types::MCP_TOOL_NAME_DELIMITER;

/// Reqwest 0.13 adapter over `agent_tui_extra_ca::extra_root_ders` (DER is version-neutral).
fn with_extra_root_certificates(mut builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    for der in agent_tui_extra_ca::extra_root_ders() {
...
pub struct McpState {
    ...
    event_writer: agent_tui_file_utils::events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: agent_tui_file_utils::events::EventWriter::noop(),
    ...
    pub fn set_event_writer(&mut self, writer: agent_tui_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &agent_tui_file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `agent-tui-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use agent_tui_telemetry::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`agent_tui_tool_protocol::ToolId`] syntax.
pub fn parse_mcp_qualified_name(name: &str) -> Option<(agent_tui_tool_protocol::ToolId, &str, &str)> {
    ...
    Some((agent_tui_tool_protocol::ToolId::new(name).ok()?, server, tool))
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> agent_tui_file_utils::events::McpErrorCategory {
        use agent_tui_file_utils::events::McpErrorCategory;
        match self {
...
/// `agent_tui_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl agent_tui_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> agent_tui_tool_protocol::ToolId {
        ...
        agent_tui_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| agent_tui_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        agent_tui_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(agent_tui_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(agent_tui_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            agent_tui_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(agent_tui_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(agent_tui_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &agent_tui_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, agent_tui_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| agent_tui_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(agent_tui_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(agent_tui_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(agent_tui_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(agent_tui_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(agent_tui_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(agent_tui_file_utils::events::Event::McpTransportDecodeError {
...
/// External-config overrides for an MCP server, surfaced to agent-tui-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: agent_tui_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            ...
            Implementation::new(
                ...
                agent_tui_version::VERSION.to_string(),
            ),
        )
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            agent_tui_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = agent_tui_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = agent_tui_config::grok_home().join("logs").join("mcp");
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a agent_tui_file_utils::events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    pub fn for_session(
        session_id: &'a str,
        event_writer: &'a agent_tui_file_utils::events::EventWriter,
        mode: OauthInteractivity,
    ...
    pub fn session_less(event_writer: &'a agent_tui_file_utils::events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = agent_tui_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = agent_tui_telemetry::instrumentation::timer("mcp_stdio_spawn");
            ...
            .map_err(|e| {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: agent_tui_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    agent_tui_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        ctx.event_writer.emit(
                            agent_tui_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = agent_tui_telemetry::instrumentation::timer("mcp_start_servers");
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            agent_tui_file_utils::events::EventWriter::noop(),
        );
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use agent_tui_computer_hub_sdk::LocalRegistry;
        use agent_tui_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = agent_tui_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = agent_tui_file_utils::events::EventWriter::open(tmp.path());
