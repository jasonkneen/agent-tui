...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p agent-tui-voice --bin voice-probe -- --seconds 5
//! ```
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = agent_tui_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,agent_tui_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                agent_tui_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
Environment:
  ...
  RUST_LOG        optional (default info,agent_tui_voice=debug)
