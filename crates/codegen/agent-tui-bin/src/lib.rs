...
//! **ONE CORE** composition root for every product skin.
//!
//! There is a **single** linked binary (`agent-tui`). Product identity comes from:
//! 1. `AGENT_TUI_PRODUCT` / `AGENT_TUI_PRODUCT_FILE` (if already set), else
//! 2. argv[0] basename (`grok`, `lazartui`, `codex`, `claude`, `hermes`,
//!    `agent-multi` / …) when the binary is installed under multiple names
//!    via **symlinks** (zero core duplication on disk).
//!
//! See `docs/CORE_AND_ADDONS.md` and `scripts/link-product-bins.sh`.

/// Map install name → product preset (`product_profile` / `AGENT_TUI_PRODUCT`).
pub fn product_preset_for_argv0(argv0: &str) -> Option<&'static str> {
    ...
    match name {
        "agent-tui" | "agent_tui" => None, // platform default (no forced product)
        "agent-multi" | "agent_multi" | "agent-all" | "agent_all" => Some("all"),
        "grok" => Some("grok"),
        "lazartui" | "lazar" => Some("lazar"),
        "codex" => Some("codex"),
        "claude" => Some("claude"),
        "hermes" => Some("hermes"),
        _ => None,
...
#[cfg(test)]
mod product_skin_tests {
    use super::product_preset_for_argv0;

    #[test]
    fn argv0_maps_product_skins() {
        assert_eq!(product_preset_for_argv0("agent-tui"), None);
        assert_eq!(product_preset_for_argv0("/usr/bin/agent-tui"), None);
        assert_eq!(product_preset_for_argv0("grok"), Some("grok"));
        assert_eq!(product_preset_for_argv0("./target/debug/grok"), Some("grok"));
        assert_eq!(product_preset_for_argv0("lazartui"), Some("lazar"));
        assert_eq!(product_preset_for_argv0("hermes"), Some("hermes"));
        assert_eq!(product_preset_for_argv0("agent-multi"), Some("all"));
        assert_eq!(product_preset_for_argv0("codex.exe"), Some("codex"));
    }
}
...
/// Apply product skin from env or argv[0] **before** pager/product_profile init.
pub fn apply_product_from_invocation() {
    if !env_blank("AGENT_TUI_PRODUCT") || !env_blank("AGENT_TUI_PRODUCT_FILE") {
        // Explicit product wins; still apply lazar home isolation when product is lazar.
        apply_lazar_config_home_if_needed();
        return;
    }
    let argv0 = std::env::args().next().unwrap_or_default();
    if let Some(preset) = product_preset_for_argv0(&argv0) {
        // SAFETY: process start, single-threaded before core init.
        unsafe {
            std::env::set_var("AGENT_TUI_PRODUCT", preset);
        }
    }
    apply_lazar_config_home_if_needed();
}

fn apply_lazar_config_home_if_needed() {
    let product = std::env::var("AGENT_TUI_PRODUCT").unwrap_or_default();
    if product.trim().to_ascii_lowercase() != "lazar" {
        return;
    }
    if !env_blank("AGENT_TUI_HOME") || !env_blank("GROK_HOME") {
        return;
    }
    ...
    let lazar_cfg = home.join(".lazar");
    let lazar_home = std::env::var_os("LAZAR_HOME")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| home.join("lazar"));
    if lazar_cfg.is_dir() || lazar_home.is_dir() {
        unsafe {
            std::env::set_var("AGENT_TUI_HOME", &lazar_cfg);
        }
    }
}
...
use agent_tui_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use agent_tui_pager::client_identity::PAGER_CLIENT_VERSION;
use agent_tui_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use agent_tui_shell::agent::config::Config as AgentConfig;
use agent_tui_shell::leader::{
...
use agent_tui_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_agent_endpoint_args(
    agent_args: &agent_tui_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.agent_tui_api_base_url {
        config.endpoints.agent_tui_api_base_url = v.clone();
    }
}
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use agent_tui_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(agent_tui_telemetry::sampling_log::layer())
        .with(agent_tui_telemetry::instrumentation::layer())
        .with(agent_tui_telemetry::hooks_log::layer())
        .with(agent_tui_telemetry::otel_layer::build_otel_layer(
            agent_tui_telemetry::otel_layer::OtelClientInfo {
                client_name: "grok-pager",
                client_version: agent_tui_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            agent_tui_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    agent_tui_telemetry::debug_log::install_firehose(registry, app_entrypoint);
...
/// `grok setup`: rendering + exit codes only; fetch logic lives in `agent_tui_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use agent_tui_shell::managed_config::{self, SetupOutcome};
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = agent_tui_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !agent_tui_shell::util::is_grok_process(pid) {
        ...
        if let Err(e) = agent_tui_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(agent_tui_shell::env::GrokBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, agent_tui_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = agent_tui_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = agent_tui_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&agent_tui_shell::leader::ControlPayload>,
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&agent_tui_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<agent_tui_shell::util::config::RemoteSettings> {
    join_early_prefetch(agent_tui_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = agent_tui_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
...
async fn workspace_control(
    ...
    let raw_config = agent_tui_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use agent_tui_shell::auth::ensure_authenticated;
    agent_tui_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    ...
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        agent_tui_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
...
async fn run_agent_command(
    agent_args: Box<agent_tui_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use agent_tui_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        agent_tui_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = agent_tui_telemetry::otel_layer::otel_guard();
    agent_tui_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => agent_tui_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Grok Build (pager) - v{}",
            agent_tui_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                agent_tui_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(agent_tui_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = agent_tui_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = agent_tui_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    let requested_confinement = agent_tui_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    if use_leader {
        ...
        let env_urls = agent_tui_shell::leader::LeaderEnvUrls::from(&agent_config.grok_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = agent_tui_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            agent_tui_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(agent_tui_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = agent_tui_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = agent_tui_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !agent_tui_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<agent_tui_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(agent_tui_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<agent_tui_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(agent_tui_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    agent_tui_shell::heap_profile::install(agent_tui_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "grok {}\n",
        agent_tui_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = agent_tui_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
pub fn run() {
    agent_tui_telemetry::startup::mark_process_start();
    if let Some(code) = agent_tui_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = agent_tui_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    agent_tui_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    agent_tui_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        agent_tui_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        agent_tui_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    agent_tui_pager::memory_trace::start(agent_tui_pager::memory_trace::default_dir());
    raise_fd_limit();
    if let Err(e) = agent_tui_config::validate_requirements() {
    ...
    let _sentry_guard = agent_tui_telemetry::sentry::init(agent_tui_telemetry::sentry::Config {
        client: "grok-pager",
    ...
    let crashed = agent_tui_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if args.chat() {
        unsafe {
            std::env::set_var(agent_tui_shell::agent::chat_modes::GROK_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(agent_tui_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    ...
    if let Some(Command::Completions { shell }) = &args.command {
        agent_tui_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return agent_tui_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        agent_tui_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        agent_tui_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    agent_tui_shell::config::apply_sandbox(
        None,
    ...
    agent_tui_shell::http::set_client_name(if is_interactive {
        agent_tui_workspace::permission::ClientType::GrokPager
    } else {
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": agent_tui_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
            ...
            Command::Inspect { json } => {
                ...
                agent_tui_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return agent_tui_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = agent_tui_telemetry::otel_layer::otel_guard();
                return agent_tui_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = agent_tui_telemetry::otel_layer::otel_guard();
                let config = agent_tui_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return agent_tui_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return agent_tui_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            Command::DiskUsage(disk_usage_args) => {
                init_tracing_simple("cli");
                let _otel_guard = agent_tui_telemetry::otel_layer::otel_guard();
                return agent_tui_pager::disk_usage_cmd::run(disk_usage_args);
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return agent_tui_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return agent_tui_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return agent_tui_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return agent_tui_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return agent_tui_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                agent_tui_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                agent_tui_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = agent_tui_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                agent_tui_shell::auth::run_cli_logout(&config)?;
                agent_tui_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = agent_tui_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(agent_tui_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == agent_tui_pager::headless::OutputFormat::Plain
        {
            args.output_format = agent_tui_pager::headless::OutputFormat::Json;
        }
        return agent_tui_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            agent_tui_pager::headless::HeadlessOptions {
    ...
    let result = agent_tui_pager::app::run(args, bg_update_rx).await;
    agent_tui_sandbox::flush();
    match result {
        Ok(true) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = agent_tui_shell::env::GrokBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                agent_tui_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("GROK_NPM_REGISTRY"))
        .ok()
        .or_else(agent_tui_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = agent_tui_shell::config::load_effective_config_disk_only()
        && let Some(ch) = agent_tui_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn is_managed_install(exe: Option<std::path::PathBuf>, grok_home: &std::path::Path) -> bool {
    ...
    let managed = agent_tui_config::grok_application_in(grok_home);
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in agent_tui_shell::leader::discover_leaders().await {
        if d.classification != agent_tui_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match agent_tui_shell::leader::LeaderClient::connect(
            socket_path,
            "grok-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(agent_tui_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(agent_tui_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            agent_tui_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: agent_tui_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            agent_tui_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!agent_tui_shell::heap_profile::prof_available());
            return;
        }
        assert!(agent_tui_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(agent_tui_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(agent_tui_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        agent_tui_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    fn workspace_command_gate_resolution() {
        use agent_tui_shell::util::config::RemoteSettings;
