...
use agent_tui_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
/// `grok setup`: render command outcomes; fetch logic lives in `agent_tui_shell::managed_config`.
...
/// Shared entry for the one core binary (and any argv[0] product alias).
///
/// Product profile is resolved once on first use inside the pager — call
/// [`apply_product_from_invocation`] first.
pub fn run() {
    ...
    agent_tui_pager::memory_trace::start(
        agent_tui_shell::util::grok_home::grok_home().join("memtrace"),
    );
    raise_fd_limit();
...
async fn async_main() -> Result<()> {
            ...
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        agent_tui_update::channel_name().unwrap_or("unknown"), }
                    );
    ...
    if let Some(prompt) = headless_prompt {
        ...
        if json_schema.is_some() {
            if args.output_format == agent_tui_pager::headless::OutputFormat::Plain {
