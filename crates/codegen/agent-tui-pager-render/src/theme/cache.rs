...
//! `agent_tui_shell::util::config::set_theme()` (and friends), invoked
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `agent_tui_shell::util::config::set_theme()` (and
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = agent_tui_config::load_effective_config_disk_only() else {
