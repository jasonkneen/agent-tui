...
//! frame diff (see `agent_tui_ratatui_inline::Terminal::flush_with_links`).
...
mod tests {
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/agent-tui-pager/src/render` should
        // NOT produce a link for the `/agent-tui-pager/src/render` substring.
        let line = make_line("find crates/codegen/agent-tui-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
