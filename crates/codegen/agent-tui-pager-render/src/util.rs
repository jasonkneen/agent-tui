...
pub use agent_tui_config::grok_home;
...
///
/// Derived from resolved [`grok_home()`] vs `agent_tui_config::default_grok_home()`,
/// not from whether `GROK_HOME` is set in the environment.
pub fn display_grok_home_prefix() -> String {
    if grok_home() == agent_tui_config::default_grok_home() {
        "~/.grok".to_string()
    } else {
        "$GROK_HOME".to_string()
    }
