...
fn run_tmux_bounded(
    ...
    let group = agent_tui_tty_utils::ProcessGroup::new()
        .and_then(|mut group| {
...
fn terminate_tmux_tree(group: &agent_tui_tty_utils::ProcessGroup, child: &mut std::process::Child) {
    terminate_owned_group(group);
    let _ = child.wait();
}
...
fn terminate_owned_group(group: &agent_tui_tty_utils::ProcessGroup) {
...
fn build_tmux_command(command: TmuxCommand<'_>) -> Command {
    ...
    cmd.stdin(Stdio::null()).envs(agent_tui_tty_utils::pager_env());
...
mod tests {
    ...
    fn command_protocol_uses_exact_argv_and_pager_env() {
        ...
        for (request, args) in cases {
            ...
            let actual: HashMap<OsString, Option<OsString>> = cmd
                ...
                .collect();
            let expected: HashMap<OsString, Option<OsString>> = agent_tui_tty_utils::pager_env()
                .into_iter()
    ...
    fn empty_group_teardown_does_not_wait_out_the_grace() {
        let group = agent_tui_tty_utils::ProcessGroup::new().expect("group");
    ...
    fn group_that_ignores_sigterm_waits_the_grace_and_is_killed() {
        let mut group = agent_tui_tty_utils::ProcessGroup::new().expect("group");
