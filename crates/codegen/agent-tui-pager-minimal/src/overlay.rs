...
//! state is a no-op.
//!
//! [`Terminal::set_viewport_height`]: agent_tui_ratatui_inline::Terminal::set_viewport_height
...
use agent_tui_pager::app::agent_view::AgentView;
use agent_tui_pager::app::app_view::{ActiveView, AppView};
use agent_tui_pager::appearance::LayoutConfig;
use agent_tui_pager::minimal_api;
use agent_tui_pager::render::SafeBuf as _;
use agent_tui_pager::theme::Theme;
use agent_tui_pager::views::prompt_widget::{PromptBg, PromptInfo, PromptStyle, PromptWidget};
use agent_tui_pager::views::question_view::{feedback_input, inline_text_width};
...
fn active(prompt: &PromptWidget, items_width: u16) -> Option<(Kind, u16)> {
    use agent_tui_pager::views::completion_dropdown::MAX_VISIBLE_ROWS;
    use agent_tui_pager::views::file_search::dropdown::MAX_DROPDOWN_ROWS as FILE_MAX;
    use agent_tui_pager::views::slash_dropdown::desired_item_rows;
...
fn compute_target(app: &mut AppView, term_h: u16, width: u16) -> u16 {
    ...
    let (input_mode, multiline) = app
        ...
        .unwrap_or_default();
    let theme = agent_tui_pager::theme::Theme::current();
    ...
    let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
        agent_tui_pager::views::btw_overlay::btw_panel_height(agent.btw_state.as_ref(), width)
    } else {
        0
    };
...
pub fn render(
    ...
    match kind {
        Kind::FileSearch => {
            agent_tui_pager::views::file_search::dropdown::render_dropdown(
                buf,
                items_rect,
                &prompt.file_search,
                theme,
        ...
        Kind::Slash => {
            ...
            agent_tui_pager::views::slash_dropdown::render_dropdown(
                buf, items_rect, &snap, hovered, theme,
            );
        }
        Kind::Completion => {
            agent_tui_pager::views::completion_dropdown::render_dropdown(
                buf,
                items_rect,
...
pub fn modal_height(modal: Modal, agent: &mut AgentView, screen_h: u16, content_w: usize) -> u16 {
    match modal {
        Modal::Permission => agent
            ...
            .front()
            .map(|p| {
                agent_tui_pager::views::permission_view::permission_view_height(
        ...
        Modal::Question => {
            let input_mode = minimal_api::question_view(agent).is_some_and(|qv| {
                qv.focus == agent_tui_pager::views::question_view::QuestionFocus::InputMode
            });
            ...
            minimal_api::question_view_mut(agent)
                .map(|qv| {
                    agent_tui_pager::views::question_view::question_view_height(
        ...
        Modal::Rewind => minimal_api::rewind_state(agent)
            .map(|rw| agent_tui_pager::views::rewind::rewind_overlay_height(&rw.phase, screen_h))
            .unwrap_or(0),
        Modal::Cancel => {
            if minimal_api::cancel_turn_view(agent).is_some() {
                agent_tui_pager::views::modal::cancel_turn_panel_height(screen_h)
            } else {
                0
            }
        }
    ...
    match modal {
        ...
        Modal::Rewind => {
            if let Some(rw) = minimal_api::rewind_state(agent) {
                agent_tui_pager::views::rewind::render_rewind_overlay(buf, area, &rw.phase, true);
            }
            None
        }
        Modal::Cancel => {
            ...
            let drawn = if let Some(ctv) = minimal_api::cancel_turn_view(agent) {
                agent_tui_pager::views::modal::render_cancel_turn_panel(
                    buf,
                    area,
                    ctv,
                    true,
...
fn render_permission(
    ...
    let result = agent_tui_pager::views::permission_view::render_permission_view(
        buf,
        area,
        perm,
        &followup,
...
fn render_question(
    ...
    use agent_tui_pager::views::question_view::{QUESTION_VIEW_HPAD, QuestionFocus};
    ...
    if let Some(qv) = minimal_api::question_view_mut(agent) {
        let vis = qv.questions.get(qv.active_tab).map(|question| {
            agent_tui_pager::views::question_view::visible_options_height(
                question,
                q_area.height,
                content_w,
                qv.focused_preview(),
    ...
    if let Some(qv) = minimal_api::question_view(agent) {
        agent_tui_pager::views::question_view::render_question_view(
            buf,
            q_area,
            qv,
    ...
    if input_mode {
        ...
        let freeform_sel = minimal_api::question_view(agent)
            ...
            .copied()
            .unwrap_or(false);
        let embed = agent_tui_pager::views::modal_window::embedded_row_style(theme, true);
        ...
        let marker = if is_multi {
            ...
            format!("({})", agent_tui_pager::glyphs::filled_dot())
        } else {
            "(\u{25cb})".to_string()
        };
        ...
        buf.set_span_safe(
            area.x + 9,
            row_y,
            &ratatui::text::Span::styled(agent_tui_pager::glyphs::prompt_arrow(), accent),
            2,
        );
        ...
        let editor = Rect {
            x: area.x + 11,
            y: row_y,
            width: agent_tui_pager::views::question_view::inline_text_width(area.width),
            height: input_h,
        };
        return agent
            .prompt
...
fn render_feedback_editor(
    ...
    for y in input_area.y..input_area.y.saturating_add(box_h) {
        if let Some(cell) = buf.cell_mut((area.x, y)) {
            cell.set_symbol(agent_tui_pager::glyphs::accent_bar());
...
mod tests {
    use super::*;
    use agent_tui_pager::views::suggestion_controller::CompletionItemParsed;
    ...
    fn question_input_agent(text: &str) -> AgentView {
        use agent_tui_pager::views::prompt_widget::StashedPrompt;
        use agent_tui_pager::views::question_view::{Question, QuestionOption, QuestionViewState};
