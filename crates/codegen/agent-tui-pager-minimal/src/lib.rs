...
//! terminal's *native* scrollback (via `agent_tui_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `agent-tui-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`agent_tui_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`agent-tui-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `agent-tui-pager`.
///
...
/// subsequent calls are ignored (see [`agent_tui_pager::minimal_hook`]).
pub fn install() {
    agent_tui_pager::minimal_hook::install(agent_tui_pager::minimal_hook::MinimalHooks { draw });
}
