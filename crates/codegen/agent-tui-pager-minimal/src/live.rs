...
use agent_tui_pager::views::prompt_widget::PromptStyle;
...
fn minimal_watchers(agent: &agent_tui_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            ...
            .values()
            .filter(|t| {
                t.is_monitor && t.status == agent_tui_pager::app::agent::BgTaskStatus::Running
            })
            .count(),
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let hint = if agent_tui_pager::app::minimal_auto_set_for_mouse_leak() {
