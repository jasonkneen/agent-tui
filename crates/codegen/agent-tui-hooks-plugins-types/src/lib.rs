...
//! `agent-tui-shell` and `agent-tui-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `agent-tui-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `agent-tui-agent`. Optional on [`PluginInfo`]
...
/// Hook event type.
///
/// Maps from `HookEventName` in `agent-tui-hooks`. The source type's
