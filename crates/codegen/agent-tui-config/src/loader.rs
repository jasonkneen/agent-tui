...
/// A hook's origin (held by `agent_tui_hooks::HookSpec::layer`). Defined here, not
/// in `agent-tui-hooks`, since the dep direction is `agent-tui-hooks -> agent-tui-config`;
...
/// (`agent_tui_shell::util::config::load_effective_config`) when remote campaigns
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match agent_tui_version::installed_semver() {
...
mod tests {
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        ...
        std::fs::write(
            &path,
            "api_key = \"agent-tui-secretmustnotleak\"\napi_key = \"agent-tui-secretmustnotleak2\"\n",
        )
        .unwrap();
        ...
        assert!(
            !msg.contains("agent-tui-secretmustnotleak"),
