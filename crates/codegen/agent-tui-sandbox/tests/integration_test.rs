//! Integration tests for agent-tui-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use agent_tui_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use agent_tui_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!agent_tui_sandbox::should_restrict_child_network());
}
