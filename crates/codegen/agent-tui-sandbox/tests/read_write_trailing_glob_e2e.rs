...
//!
//! ```text
//! cargo test -p agent-tui-sandbox --test read_write_trailing_glob_e2e -- --nocapture
//! ```
...
fn subprocess_entry() {
    ...
    let profile: agent_tui_sandbox::ProfileName = "cargo".parse().expect("profile name");
    let mut mgr = agent_tui_sandbox::SandboxManager::new(profile, &workspace);
