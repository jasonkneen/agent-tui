...
fn subprocess_profile_and_bwrap_reexec(profile: &agent_tui_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !agent_tui_sandbox::is_inside_bwrap() {
            ...
            match agent_tui_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = agent_tui_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_grok_home: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match agent_tui_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    ...
    let profile = agent_tui_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
