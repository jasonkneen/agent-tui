...
//!
//! ```rust,no_run
//! use agent_tui_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            agent_tui_config::ensure_grok_hook_slots(paths::grok_home().as_path())
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [agent_tui_config::GlobalHookSource {
            path: leaf.clone(),
            kind: agent_tui_config::GlobalHookSourceKind::ConfiguredSource,
        }];
