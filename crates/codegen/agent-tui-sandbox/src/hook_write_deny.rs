...
use agent_tui_config::{GlobalHookSource, missing_configured_sources, resolve_global_hook_sources};

#[cfg(unix)]
use agent_tui_config::validated_hook_json_files_for_sources;
#[cfg(target_os = "linux")]
use agent_tui_config::{ensure_grok_hook_slots, unique_ancestors_rootward};
...
impl From<agent_tui_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: agent_tui_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use std::os::unix::fs::MetadataExt;
    use agent_tui_config::GlobalHookSourceKind;
    for s in sources {
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use agent_tui_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
