...
//!
//! Two consumers read it independently. The hook matcher (`agent-tui-hooks`) needs the
//! Grok tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`agent-tui-agent`) needs the [`ToolKind`] a `tools:`
...
/// Every distinct Grok name the table references, for the `agent-tui-agent` drift-check
