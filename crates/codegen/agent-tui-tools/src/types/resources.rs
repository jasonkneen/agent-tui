...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, agent_tui_tool_runtime::ToolError> {
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), agent_tui_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn agent_tui_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn agent_tui_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, agent_tui_tool_runtime::ToolError>;
}
#[derive(Clone)]
...
///
/// Stamped onto [`agent_tui_tool_runtime::ToolCallContext::extensions`] by
...
/// depending on `agent-tui-mcp` directly.  Follows the same pattern as
