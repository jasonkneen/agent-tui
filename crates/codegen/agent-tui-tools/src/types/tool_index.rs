...
//!
//! `ToolSearchIndex` is defined in `agent-tui-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `agent-tui-shell`
...
/// in `agent-tui-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `agent-tui-shell` after MCP initialization.
#[derive(Clone)]
