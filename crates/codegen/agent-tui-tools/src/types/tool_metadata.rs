...
//! 1. `agent_tui_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! ## Context helpers
//!
//! Tools access session state through `agent_tui_tool_runtime::ToolCallContext`
...
///
/// Each tool struct implements this alongside `agent_tui_tool_runtime::Tool`.
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Model-safe fallback description for `agent_tui_tool_runtime::Tool::description()`
...
) -> Result<SharedResources, agent_tui_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
pub async fn resolve_cwd(
...
) -> Result<PathBuf, agent_tui_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<agent_tui_tool_runtime::Cwd>() {
    ...
    res.get::<crate::types::resources::Cwd>()
        .map(|c| c.0.clone())
        .ok_or_else(|| {
            agent_tui_tool_runtime::ToolError::custom("missing_cwd", "Cwd not available in Resources")
        })
}
...
pub fn test_ctx(resources: SharedResources) -> agent_tui_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(agent_tui_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = agent_tui_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| agent_tui_tool_protocol::ToolCallId::new_v7());
    let mut ctx = agent_tui_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(agent_tui_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &agent_tui_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
