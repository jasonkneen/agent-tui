...
//! (in `agent-tui-tools`) and the subagent coordinator (in `agent-tui-shell`).
//!
//! ## Resource types
//!
...
/// defense-in-depth check in `agent-tui-shell`'s subagent coordinator.
...
/// the cross-crate drain loop in `agent-tui-shell` relies on
...
///
/// Set by agent-tui-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by agent-tui-shell at turn start.
