...
async fn detect_continue_parent_work(
    ...
    agent_tui_tool_types::should_continue_parent_work(&asks, description, prompt)
}
...
async fn resolve_background_notice_names(resources: &SharedResources) -> (String, String, String) {
    let canonical = agent_tui_tool_types::BackgroundNoticeNaming::CANONICAL;
...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // in agent-tui-agent/src/builder.rs (live subagent roster). But a
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            let subagents: Vec<agent_tui_tool_types::SubagentDescriptor> =
                agent_tui_tool_types::BUILTIN_SUBAGENTS
                    .iter()
                    .map(|b| agent_tui_tool_types::SubagentDescriptor {
                        name: b.name.to_owned(),
                        description: b.description.to_owned(),
                        tools: Some(guard_kind_tokens(b.tools_template)),
                    })
                    .collect();
            agent_tui_tool_types::build_task_description(
                &subagents,
                &agent_tui_tool_types::TaskToolNaming {
...
impl agent_tui_tool_runtime::Tool for TaskTool {
    ...
    fn id(&self) -> agent_tui_tool_protocol::ToolId {
        agent_tui_tool_protocol::ToolId::new(TASK_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let tool_cancellation = ctx
            .get::<agent_tui_tool_runtime::Cancellation>()
        ...
        if input.run_in_background {
            ...
            let naming = agent_tui_tool_types::BackgroundNoticeNaming {
        ...
        if result.backgrounded {
            ...
            let text = agent_tui_tool_types::format_subagent_auto_backgrounded(
                &id,
                &input.subagent_type,
                &input.description,
                &naming,
...
mod tests {
    ...
    async fn auto_backgrounded_result_returns_task_id_text() {
        ...
        match result {
            ToolOutput::Text(text) => {
                ...
                assert!(
                    text.text
                        .contains(agent_tui_tool_types::BACKGROUND_SUBAGENT_CONTINUE_PARENT_WORK),
    ...
    fn task_tool_id_predicate_accepts_all_wire_spellings() {
        assert!(is_task_tool_id(
            agent_tui_tool_runtime::Tool::id(&TaskTool).as_str()
        ));
