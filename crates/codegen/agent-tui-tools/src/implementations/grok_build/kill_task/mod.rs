...
use agent_tui_tool_types::{KillTaskOutput, KillTaskResult, KillTaskToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for KillTaskTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            agent_tui_tool_types::build_kill_task_description(&agent_tui_tool_types::KillTaskToolNaming {
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        let task_tool = Expr::Value(ToolRequirement::Tool {
            ...
            id: agent_tui_tool_runtime::Tool::id(&TaskTool).to_string(),
            if_params: None,
        });
        let mut arms =
...
/// [`agent_tui_tool_types::build_kill_task_description`] builder so the CLI and
...
impl agent_tui_tool_runtime::Tool for KillTaskTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> agent_tui_tool_protocol::ToolId {
        agent_tui_tool_protocol::ToolId::new("kill_task").expect("valid tool id")
    }
    ...
    ) -> Result<KillTaskOutput, agent_tui_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn make_ctx_with_version(
        ...
        ctx.extensions
            .insert(agent_tui_tool_runtime::BehaviorVersion(version.to_owned()));
        ctx
    }
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(agent_tui_tool_runtime::Tool::id(&tool).as_str(), "kill_task");
