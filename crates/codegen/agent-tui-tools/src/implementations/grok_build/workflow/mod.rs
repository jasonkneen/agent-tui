...
pub use agent_tui_tools_api::slash_commands::WORKFLOW_TOOL_NAME;
...
impl agent_tui_tool_runtime::ToolOutput for WorkflowToolOutput {}
...
impl agent_tui_tool_runtime::Tool for WorkflowTool {
    type Args = WorkflowToolInput;
    type Output = WorkflowToolOutput;

    fn id(&self) -> agent_tui_tool_protocol::ToolId {
        agent_tui_tool_protocol::ToolId::new(WORKFLOW_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
    ...
    ) -> Result<WorkflowToolOutput, agent_tui_tool_runtime::ToolError> {
        ...
        match ack_rx.await {
            ...
            Ok(WorkflowLaunchAck::Rejected { code, detail }) => {
                Err(agent_tui_tool_runtime::ToolError::custom(code, detail))
            }
            Err(_) => Err(agent_tui_tool_runtime::ToolError::custom(
