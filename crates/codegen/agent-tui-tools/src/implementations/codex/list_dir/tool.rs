...
impl agent_tui_tool_runtime::Tool for CodexListDirTool {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = agent_tui_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = agent_tui_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
