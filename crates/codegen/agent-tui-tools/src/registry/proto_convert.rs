//! Conversion from the gRPC wire config types (`agent-tui-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`agent_tui_tools_api::config_validation`] so every consumer (this
...
pub use agent_tui_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`agent_tui_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: agent_tui_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let agent_tui_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = agent_tui_tools_api::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    agent_tui_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<agent_tui_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> agent_tui_tools_api::ToolConfigEntry {
        agent_tui_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
