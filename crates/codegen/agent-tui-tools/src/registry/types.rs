...
/// this crate: instead of `agent-tui-tools` referencing an out-of-tree tool
/// pack, the pack calls
        ...
        for tool_config in &config.tools {
            ...
            if let Some(desc) = &definition.function.description {
                definition.function.description = Some(truncation_config.interpolate_description(
                    ...
                    agent_tui_tool_types::max_wait_block_ms(),
                ));
            }
...
impl FinalizedToolset {
    ...
    async fn call_raw(
        ...
        if let Some(cancellation) = parent_ctx.get::<agent_tui_tool_runtime::Cancellation>() {
                ...
                match item {
                    ...
                    agent_tui_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                        let run_result = this
                            ...
                            .await;
                        yield agent_tui_tool_runtime::ToolStreamItem::Terminal(run_result);
                        return;
                    }
                }
            }
            yield agent_tui_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        if let Some(cancellation) = cancellation {
            ctx.extensions
                .insert(agent_tui_tool_runtime::Cancellation(cancellation));
        }
    ...
    fn capabilities_is_read_only_matches_metadata() {
        ...
        let mismatches: Vec<String> = builder
            ...
            .filter_map(|(name, entry)| {
                let lr = agent_tui_computer_hub_sdk::LocalRegistry::new();
                (entry.register_in_local)(&lr);
                let id = agent_tui_tool_protocol::ToolId::new(&entry.id)
