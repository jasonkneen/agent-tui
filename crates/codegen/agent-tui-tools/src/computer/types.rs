...
/// (`agent-tui-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (agent-tui-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in agent-tui-tools, spawns processes)
/// - `AcpTerminalBackend` (in agent-tui-shell, calls ACP protocol)
#[async_trait::async_trait]
