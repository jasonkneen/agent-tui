//! Installed Agent TUI version, lockstepped with shipping binaries.
//!
//! Also hosts product identity constants shared by the pager, shell, and bin.

use semver::Version;

/// User-facing product name (welcome screen, help, notifications).
pub const PRODUCT_DISPLAY_NAME: &str = "Agent TUI";
/// CLI / binary name.
pub const BINARY_NAME: &str = "agent-tui";
/// Short product token for terminal titles (compact).
pub const PRODUCT_TITLE_TOKEN: &str = "agent-tui";

pub const TEST_VERSION_ENV: &str = "AGENT_TUI_TEST_VERSION";
/// Legacy test override still accepted.
const LEGACY_TEST_VERSION_ENV: &str = "GROK_TEST_VERSION";

pub const VERSION: &str = match option_env!("AGENT_TUI_VERSION") {
    Some(v) => v,
...
/// [`TEST_VERSION_ENV`] (or legacy `GROK_TEST_VERSION`) override first, then
/// [`VERSION`]. Trimmed so non-semver-aware callers can pass the result
/// straight into parsing.
