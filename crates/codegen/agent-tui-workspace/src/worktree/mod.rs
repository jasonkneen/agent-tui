...
//!
//! Moved from `agent-tui-shell/src/session/worktree.rs` into the workspace
...
use agent_tui_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in agent-tui-workspace-types; re-exported for existing paths.
...
pub(crate) fn to_creation_mode(t: WorktreeType) -> agent_tui_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => agent_tui_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => agent_tui_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => agent_tui_fast_worktree::CreationMode::GitCheckout,
    }
}
...
pub use agent_tui_fast_worktree::META_KEY_LABEL;
...
/// `worktrees.db` (`agent_tui_fast_worktree::resolve_grok_home`), so checkout dirs and
/// the metadata DB always live under the same `.grok` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `agent_tui_config::grok_home()`,
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to agent_tui_fast_worktree::WorkingTreeMode
    ...
    // Use agent-tui-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(agent_tui_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        agent_tui_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        agent_tui_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        agent_tui_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        agent_tui_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        agent_tui_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        agent_tui_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use agent-tui-fast-worktree -- it handles copying from any worktree path
...
pub fn worktree_db_rebuild() -> Result<agent_tui_fast_worktree::RebuildReport> {
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = agent_tui_fast_worktree::ListFilter {
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[agent_tui_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
