...
//!
//! The store is rooted at [`agent_tui_config::user_grok_home`] — the **Option**
...
impl TrustStore {
    ...
    ///
    /// Resolves via [`agent_tui_config::user_grok_home`], never
    /// [`agent_tui_config::grok_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(agent_tui_config::user_grok_home())
    }
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match agent_tui_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_sources_from_user_grok_home() {
        ...
        assert_eq!(
            ...
            agent_tui_config::user_grok_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
