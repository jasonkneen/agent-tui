...
// permissions), consider extracting to a shared helper (e.g., in agent-tui-hooks
...
// =============================================================================
//
// `agent-tui-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in agent-tui-shell (inaccessible from here at runtime). They also
    ...
    let Some(config_path) = agent_tui_config::user_grok_home().map(|g| g.join("config.toml")) else {
        return false;
    };
