...
///
/// ```bash
/// XAI_API_KEY='agent-tui-some-key' cargo run --bin agent-tui-pager
/// ```
///
...
///   prefix: ["XAI_API_KEY=agent-tui-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "agent-tui-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "XAI_API_KEY='agent-tui-some-key' cargo run --bin agent-tui-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["XAI_API_KEY=agent-tui-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "agent-tui-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin agent-tui-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package agent-tui-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
