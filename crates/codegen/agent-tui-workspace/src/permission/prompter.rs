...
/// Canonical definition lives in `agent_tui_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `agent-tui-mcp` can
...
/// reached `agent_tui_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
