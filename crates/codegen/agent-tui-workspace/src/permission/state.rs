...
use agent_tui_tools::util::grok_home::grok_home;
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    agent_tui_config::sessions_cwd_dir(cwd.as_str())
}
...
async fn try_load_state(path: &std::path::Path) -> Option<PermissionState> {
    try_load_state_with_writer(path, |path, contents| {
        agent_tui_config::fs_atomic::write_atomically(path, contents, None)
    })
    .await
}
...
async fn persist_state_to_dir(
    ...
    let result = persist_state_to_path_with_writer(&path, state, move |path, contents| {
        agent_tui_config::create_dir_all_owner_only(&dir)?;
        agent_tui_config::fs_atomic::write_atomically(path, contents, None)
    })
    .await;
    if let Err(e) = result {
...
pub(crate) async fn persist_state(
    ...
    let ensured =
        tokio::task::spawn_blocking(move || agent_tui_config::ensure_sessions_cwd_dir(&cwd_str))
            .await
            .map_err(std::io::Error::other)
            .and_then(|r| r);
