...
///
/// ```
/// use agent_tui_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = agent_tui_tools::util::grok_home::grok_home().join("worktrees");
...
pub fn git_rewind_enabled() -> bool {
    agent_tui_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
mod tests {
    ...
    async fn get_worktree_info_tilde_collapses_home_prefix() {
        ...
        let fake_main = PathBuf::from(&home).join("agent-tui-fake-main-repo-for-wt-display");
        std::fs::write(
            clone.join(".git").join("grok-worktree-source"),
        ...
        assert_eq!(
            main_repo.as_deref(),
            Some("~/agent-tui-fake-main-repo-for-wt-display")
        );
    }
    #[tokio::test]
    ...
    fn register_db_worktree(home: &Path, wt: &Path, source: &Path, label: &str) {
        let db = agent_tui_fast_worktree::db::WorktreeDb::open(home).unwrap();
        db.register(&agent_tui_fast_worktree::db::WorktreeRecord {
            id: "db-wt".into(),
            path: dunce::canonicalize(wt).unwrap_or_else(|_| wt.to_path_buf()),
            source_repo: source.to_path_buf(),
            repo_name: "main-repo".into(),
            kind: agent_tui_fast_worktree::db::WorktreeKind::Session,
            ...
            status: agent_tui_fast_worktree::db::WorktreeStatus::Alive,
