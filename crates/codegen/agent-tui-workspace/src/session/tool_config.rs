...
use std::sync::Arc;
use agent_tui_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            agent_tui_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(agent_tui_tools::computer::local::LocalFs)
            as Arc<dyn agent_tui_tools::computer::types::AsyncFileSystem>;
        let notification_handle = agent_tui_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            agent_tui_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = agent_tui_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("agent-tui-workspace/{version}"),
    );
...
fn build_web_fetch_config() -> agent_tui_tools::implementations::grok_build::web_fetch::WebFetchConfig
{
    use agent_tui_tools::implementations::grok_build::web_fetch::{WebFetchConfig, WebFetchParams};
    ...
    if agent_tui_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
mod tests {
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use agent_tui_tools::types::resources::{State, WebCitationCounter};
