...
//! (the envelope types are canonical in `agent_tui_workspace_types::rpc`).
//!
//! `error_code` uses a non-wildcard match so the compiler enforces
//! coverage of new `WorkspaceError` variants.
use crate::error::WorkspaceError;
pub use agent_tui_workspace_types::rpc::{RpcEnvelope, RpcError};
...
pub fn error_code(err: &WorkspaceError) -> &'static str {
    match err {
        ...
        WorkspaceError::TurnActive(_) => agent_tui_workspace_types::rpc::envelope::TURN_ACTIVE,
...
pub fn rpc_error_to_workspace(err: RpcError) -> WorkspaceError {
    if let Some(kind) =
        agent_tui_workspace_types::rpc::export_github::ExportGithubError::from_wire_code(&err.code)
    {
...
mod tests {
    ...
    fn export_github_codes_round_trip_typed() {
        for kind in agent_tui_workspace_types::rpc::export_github::ExportGithubError::ALL {
