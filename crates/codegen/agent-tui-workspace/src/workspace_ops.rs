...
pub use agent_tui_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use agent_tui_workspace_types::rpc::code_nav::{
...
pub use agent_tui_workspace_types::rpc::export_github::ExportGithubReq;
pub use agent_tui_workspace_types::rpc::fs::{
...
pub use agent_tui_workspace_types::rpc::git::{
...
pub use agent_tui_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use agent_tui_workspace_types::rpc::hunks::{
...
pub use agent_tui_workspace_types::rpc::repos::{
    ProvisionedRepo, RepoManifest, ReposListReq, ReposListResponse,
};
pub use agent_tui_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use agent_tui_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use agent_tui_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use agent_tui_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use agent_tui_workspace_types::rpc::worktree::{
...
pub use agent_tui_workspace_types::rpc::{RpcActivityClass, WorkspaceRpc};
...
fn repos_manifest_search_dirs(start: &std::path::Path) -> Vec<std::path::PathBuf> {
    let rel = agent_tui_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH;
    #[allow(deprecated)]
    ...
    if let Some(user_home) = agent_tui_config::user_grok_home() {
...
fn hook_registry_to_wire(
    registry: &agent_tui_hooks::discovery::HookRegistry,
...
fn wire_to_hook_registry(
...
) -> WorkspaceResult<agent_tui_hooks::discovery::HookRegistry> {
    ...
    let mut registry: agent_tui_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
///   session's [`FinalizedToolset`](agent_tui_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn server_version(&self) -> Option<String> {
        match self {
            Self::Local { .. } => Some(agent_tui_version::VERSION.to_owned()),
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<agent_tui_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](agent_tui_tools::registry::types::FinalizedToolset)
...
mod tests {
    ...
    /// request types are defined in `agent-tui-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/agent-tui-main");
        let window_b = std::path::PathBuf::from("/repos/agent-tui-main-2");
        assert_eq!(
    ...
    async fn repos_list_reads_real_manifest_empty_one_and_many() {
        ...
        assert_eq!(
            empty.version,
            agent_tui_workspace_types::rpc::repos::REPOS_MANIFEST_VERSION
        );
        ...
        std::fs::write(
            tmp.path()
                .join(agent_tui_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
    ...
    async fn repos_list_walks_up_from_rewritten_cwd() {
        ...
        std::fs::write(
            sandbox_ws.join(agent_tui_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
            one.to_json_bytes().unwrap(),
        )
        .unwrap();
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            agent_tui_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = agent_tui_hooks::config::HookSpec {
            ...
            event: agent_tui_hooks::event::HookEventName::PreToolUse,
        ...
        let mut registry = agent_tui_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use agent_tui_hooks::event::HookEventName as E;
    ...
    /// can't depend on `agent-tui-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use agent_tui_hooks::config::HookSpec;
        ...
        let spec = HookSpec {
            ...
            source_dir: std::path::PathBuf::from("/home/u/.grok/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
            layer: agent_tui_hooks::config::HookProvenance::Managed,
        };
        assert_eq!(
