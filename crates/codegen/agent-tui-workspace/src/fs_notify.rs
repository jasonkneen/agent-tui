...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> agent_tui_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => agent_tui_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => agent_tui_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => agent_tui_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => agent_tui_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => agent_tui_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](agent_tui_workspace_types::WorkspaceEvent::FsChanged)
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    agent_tui_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    agent_tui_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(agent_tui_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            agent_tui_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    ...
    let graph_kind = match kind {
        ...
        agent_tui_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        agent_tui_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn workspace_event_kind_round_trip() {
        use agent_tui_workspace_types::FsEventKind as WsKind;
        assert_eq!(
