...
use url::Url;
use agent_tui_workspace::config::WorkspaceServerMetadata;
use agent_tui_workspace::daemonize;
use agent_tui_workspace::diag_server::{self, DiagHandle, ErrorClass};
use agent_tui_workspace::error::WorkspaceError;
use agent_tui_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
/// `/usr/local/bin/agent-tui-preview-proxy` child (see `cli.rs` for the proxy's
...
fn main() -> anyhow::Result<()> {
    ...
    builder
        .worker_threads(agent_tui_tty_utils::runtime::capped_worker_threads().get())
        .enable_all();
    let rt = agent_tui_tty_utils::runtime::build_with_blocking_pool(&mut builder)?;
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    {
        use agent_tui_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("GROK_SANDBOX_PROFILE").ok() {
            ...
            None if agent_tui_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = agent_tui_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = agent_tui_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = agent_tui_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let mut status_config = agent_tui_workspace::StatusConfig::from_env();
    ...
    agent_tui_workspace::init_metrics();
    let ws_handle = match agent_tui_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = agent_tui_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            agent_tui_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    fn classify_from_client_error_display_round_trip() {
        let handshake = WorkspaceError::HubError(
            agent_tui_computer_hub_sdk::ClientError::HandshakeAuthFailed { status: 401 }.to_string(),
        );
        ...
        let auth = WorkspaceError::HubError(
            agent_tui_computer_hub_sdk::ClientError::AuthError("token rejected".into()).to_string(),
        );
        assert_eq!(
        ...
        let network = WorkspaceError::HubError(
            agent_tui_computer_hub_sdk::ClientError::NetworkError("connection refused".into())
                .to_string(),
        );
        assert_eq!(
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        ...
        let args = Args::try_parse_from(["agent-tui-workspace-server", "--project-lsp-trusted", "true"])
            .unwrap();
        assert!(args.project_lsp_trusted);
    }
    #[test]
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["agent-tui-workspace-server", "--ready-file", "/tmp/x.ready"]).unwrap();
