...
use agent_tui_workspace::diag_server;
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let url = Url::parse(&args.hub_url).map_err(|e| anyhow::anyhow!("invalid --hub-url: {e}"))?;
    {
        ...
        if profile == ProfileName::Off {
            ...
            tracing::info!(
                ...
                agent_tui_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    ...
    let ws_handle = agent_tui_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
