...
//! spawns the unchanged `/usr/local/bin/agent-tui-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/agent-tui-preview-proxy";
...
impl PreviewArgs {
    ...
    /// `agent-tui-preview-proxy/src/cli.rs`). Absent options and a false
...
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT agent_tui_tty_utils::detach_command: the proxy must stay in
...
/// `agent-tui-preview-proxy`'s `/__control/activity` route).
...
/// the default in `agent-tui-preview-proxy/src/cli.rs`).
...
pub async fn supervise_preview_metrics(control_port: Option<u16>, shutdown: watch::Receiver<bool>) {
    scrape_metrics_loop(
        ...
        shutdown,
        |body| {
            if let Some(sink) = agent_tui_computer_hub_sdk::metric_donate::active_metrics_sink() {
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match agent-tui-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
