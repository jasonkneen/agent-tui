...
pub(crate) fn stream_responses_tracked<'a>(
    ...
    async_stream::stream! {
                ...
                ResponseStreamEvent::ResponseFailed(failed_event) => {
                    ...
                    let err = SamplingError::Api {
                        ...
                        error_code: response
                            .error
                            .as_ref()
                            .map(|e| agent_tui_sampling_types::ApiErrorCode::parse(&e.code)),
                    };
                    yield SamplingEvent::Failed {
                        request_id: request_id.clone(),
                        error: SamplingErrorInfo::from(&err),
                ...
                ResponseStreamEvent::ResponseError(error_event) => {
                    ...
                    let err = SamplingError::Api {
                        ...
                        error_code: error_event
                            .code
                            .as_deref()
                            .map(agent_tui_sampling_types::ApiErrorCode::parse),
                    };
                    yield SamplingEvent::Failed {
                        request_id: request_id.clone(),
                        error: SamplingErrorInfo::from(&err),
        ...
        let mut items = agent_tui_sampling_types::response_to_conversation_items(response);
        agent_tui_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn response_failed_yields_failed_500() {
        ...
        match events.last().unwrap() {
            SamplingEvent::Failed { error, .. } => {
                ...
                assert_eq!(
                    error.error_code,
                    Some(agent_tui_sampling_types::ApiErrorCode::Other(
                        "server_error".into()
                    ))
                );
            }
    ...
    async fn response_error_event_carries_code_into_failed() {
        let error_event = rs::ResponseStreamEvent::ResponseError(rs_types::ResponseErrorEvent {
            sequence_number: 0,
            code: Some(agent_tui_sampling_types::INVALID_IMAGE_ERROR_CODE.into()),
        ...
        match events.last().unwrap() {
            SamplingEvent::Failed { error, .. } => {
                assert_eq!(
                    error.error_code,
                    Some(agent_tui_sampling_types::ApiErrorCode::InvalidImage)
                );
            }
            other => panic!("expected Failed, got {other:?}"),
        }
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use agent_tui_sampling_types::doom_loop::{
