...
use agent_tui_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use agent_tui_sampling_types::ApiBackend;
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = agent_tui_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<agent_tui_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn create_response(
        ...
        agent_tui_sampling_types::patch_reasoning_text_types(&mut request_body);
        let SentRequest {
            builder,
            sent_bearer,
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = agent_tui_sampling_types::extra_tool_entries(&request.hosted_tools);
...
mod tests {
    use super::*;
    use indexmap::IndexMap;
    use agent_tui_sampling_types::types::ChatRequestMessage;
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = agent_tui_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
