...
//! it" buckets.
//!
//! `agent-tui-sampler` is intentionally decoupled from `agent-tui-shell`
...
/// tools live in `agent-tui-tools`
/// (`crates/codegen/agent-tui-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `agent_tui_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `agent_tui_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `agent-tui-shell/src/auth/manager.rs` to match, otherwise the
