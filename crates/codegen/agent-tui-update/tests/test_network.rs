...
use agent_tui_update::auto_update::{download_silent, download_with_progress};
use agent_tui_update::version::fetch_gcs_version_from_base;
...
async fn download_silent_writes_body_to_dest() {
    ...
    Mock::given(method("GET"))
        .and(path("/agent-tui-0.1.181-macos-aarch64"))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(body.clone()))
        .mount(&server)
        .await;

    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("agent-tui");
    let url = format!("{}/agent-tui-0.1.181-macos-aarch64", server.uri());
...
async fn download_silent_publishes_executable() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("agent-tui-0.1.181-linux-x86_64");
    download_silent(&format!("{}/bin", server.uri()), &dest)
        .await
        .unwrap();
...
async fn download_silent_to_nonexistent_parent_dir_fails() {
    ...
    let dest = tmp.path().join("missing-subdir").join("agent-tui");
    let err = download_silent(&format!("{}/x", server.uri()), &dest)
        .await
        .unwrap_err();
    let msg = format!("{err:#}").to_lowercase();
