...
use agent_tui_shell::env::GrokBuildEnvironment;
...
/// [`CLI_BASE_URLS`] with a test seam: `AGENT_TUI_CLI_BASE_URL` (legacy
/// `GROK_CLI_BASE_URL`) points fetches and downloads at one base.
...
pub(crate) fn cli_base_urls() -> Vec<String> {
    let override_base = std::env::var("AGENT_TUI_CLI_BASE_URL")
        .or_else(|_| std::env::var("GROK_CLI_BASE_URL"));
    if let Ok(base) = override_base {
        let base = base.trim();
        ...
        if !base.is_empty() {
            tracing::warn!(
                "AGENT_TUI_CLI_BASE_URL/GROK_CLI_BASE_URL ignored: only loopback bases are honored"
            );
        }
    }
...
impl UpdateConfig {
    pub fn from_environment(env: &GrokBuildEnvironment) -> Self {
        Self {
            ...
            auth_scope: agent_tui_shell::auth::GrokComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
/// gh-release paths.
///
/// Tries each base URL from [`cli_base_urls`] in order and stops at the first
...
pub use agent_tui_version::installed as get_installed_grok_version;
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = agent_tui_shell::util::grok_home::grok_application();
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(agent_tui_version::VERSION, &stable)
    })
}
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(agent_tui_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
