...
use agent_tui_shell::util::grok_home::{grok_application, grok_home};
pub use agent_tui_telemetry::events::CliUpdateTrigger;
...
async fn run_update_subcommand(
    ...
    // delay an update.
    if let Some(mode) = agent_tui_telemetry::client::current_mode() {
...
pub async fn run_install_script(
    ...
    agent_tui_telemetry::session_ctx::log_event(CliUpdate {
        outcome,
        trigger,
        from_version,
        to_version,
...
async fn refresh_deployment_config() {
    if !agent_tui_shell::managed_config::has_principal() {
        return;
    }
    if !agent_tui_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    agent_tui_shell::managed_config::clear_orphan();
    if !agent_tui_shell::config::is_managed_config_stale_for(
        &agent_tui_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match agent_tui_shell::managed_config::sync().await {
