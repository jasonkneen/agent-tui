...
Sample hooks for Grok. Copy to `~/.agent-tui/hooks/` to enable globally, or to `<project>/.grok/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.agent-tui/hooks/bin
cp examples/hooks/safe-shell.json ~/.agent-tui/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.agent-tui/hooks/bin/
chmod +x ~/.agent-tui/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.agent-tui/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.agent-tui/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.agent-tui/hooks/bin/
chmod +x ~/.agent-tui/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.agent-tui/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.agent-tui/hooks/bin
cp examples/hooks/session-log.json ~/.agent-tui/hooks/
cp examples/hooks/bin/session-log.sh ~/.agent-tui/hooks/bin/
chmod +x ~/.agent-tui/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.agent-tui/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.agent-tui/hooks/bin
cp examples/hooks/tool-logger.json ~/.agent-tui/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.agent-tui/hooks/bin/
chmod +x ~/.agent-tui/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.agent-tui/hooks/`. The hook stops running on the next session.