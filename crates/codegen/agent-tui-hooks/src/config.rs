...
pub use agent_tui_config::HookProvenance;
...
pub fn parse_hooks_from_config_layers(
    layers: &[agent_tui_config::HookConfigLayer],
) -> (Vec<HookSpec>, Vec<HookError>) {
    let home = agent_tui_config::user_grok_home();
...
mod tests {
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> agent_tui_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        agent_tui_config::HookConfigLayer::new(
            agent_tui_config::HookProvenance::Managed,
            source_name,
            hooks,
        )
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        ...
        use agent_tui_config::HookProvenance::{Managed, User};
