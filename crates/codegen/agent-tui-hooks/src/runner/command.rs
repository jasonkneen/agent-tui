...
pub async fn run_command_hook(
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = agent_tui_config::shell::shell_command_argv(&command_str);
    ...
    // `agent_tui_tools::util::detach_command`: Unix uses the same setsid /
...
mod tests {
    ...
    fn make_scoped_ctx(scope: agent_tui_tools::util::ProcessScope) -> RunContext<'static> {
        RunContext {
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = agent_tui_tools::util::ProcessScope::new();
