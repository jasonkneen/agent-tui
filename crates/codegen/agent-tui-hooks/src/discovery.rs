...
fn load_hooks_from_directory(dir: &Path) -> (Vec<HookSpec>, Vec<HookError>) {
    ...
    // hooks still load. (Sandbox fail-closed listing lives in agent_tui_config.)
    ...
    for entry in entries {
        ...
        if !agent_tui_config::is_direct_hook_json_name(name) || !path.is_file() {
            continue;
        }
...
fn is_valid_hook_file(path: &Path) -> bool {
    ...
    agent_tui_config::is_direct_hook_json_name(name) && path.is_file()
}
