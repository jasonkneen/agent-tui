...
//! image bytes.
//!
//! Both the TUI ([`agent_tui_pager::prompt_images`]) and the server-side
...
//! [`agent_tui_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `agent-tui-tools`).
...
/// [`AttachedImages`](agent_tui_tools::types::resources::AttachedImages))
...
///   separator — see [`agent_tui_pager::prompt_images::display_text`].
...
/// [`agent_tui_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    agent_tui_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `agent_tui_pager::prompt_images::display_text`. A
