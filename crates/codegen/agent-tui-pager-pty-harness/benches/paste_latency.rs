//! `paste-latency` — clipboard paste latency benchmark for `agent-tui-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p agent-tui-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p agent-tui-pager-pty-harness --bench paste_latency -- \
...
use agent_tui_pager_pty_harness::{
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for agent-tui-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
