...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    if !output.status.success() {
        bail!(
            "failed to build agent-tui-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_agent-tui-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p agent-tui-bin` (the composition-
///    root package that owns the `agent-tui-pager` binary)
