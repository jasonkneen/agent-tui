//! Unified PTY harness for agent-tui-pager.
//!
...
//!    `agent-tui-pager/tests/scenarios/`) — assert screen contents and
...
impl PtyHarness {
    ...
    /// Spawn from a canonical [`agent_tui_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &agent_tui_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use agent_tui_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
