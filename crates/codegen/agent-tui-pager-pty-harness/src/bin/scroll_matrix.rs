//! `scroll-matrix` — scroll validation matrix sweep for agent-tui-pager.
//!
...
use agent_tui_pager_pty_harness::pager_binary;
use agent_tui_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against agent-tui-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_agent-tui-pager,
