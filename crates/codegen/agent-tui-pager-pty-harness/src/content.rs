...
//! [`MockInferenceServer`] from `agent-tui-test-support` and provides the
...
pub use agent_tui_test_support::mock_server::LogEntry;
pub use agent_tui_test_support::mock_server::MockModelEntry as MockModel;
pub use agent_tui_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use agent_tui_test_support::sse;
pub use agent_tui_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    ///
    /// Mirrors `agent_tui_test_support::env::test_env_cmd_tokio`.
