...
Agent TUI supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.agent-tui/hooks/`
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.agent-tui/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `agent-tui plugin marketplace add <source>`)
...
Sources land in `~/.agent-tui/config.toml`:
...
Or in `~/.agent-tui/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../agent-tui-hooks/examples/README.md) — ready-to-use sample hooks
