...
- **Shell builtins** -- handled by the agent backend (agent-tui-shell)
- **Pager builtins** -- handled by the TUI frontend (agent-tui-pager)
...
choice persists to `[ui].screen_mode` in `config.toml`, so future plain `agent-tui`
