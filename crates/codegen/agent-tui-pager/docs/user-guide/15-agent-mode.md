...
Agent mode runs Agent TUI as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`agent-tui -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `agent-tui agent` command and apply to every mode. Pass them before the mode name, for example `agent-tui agent --model grok-build stdio`. The `stdio` subcommand itself takes no options.
...
```bash
agent-tui agent serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
agent-tui agent headless --grok-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, Agent TUI defines extension methods under the `x.ai/` prefix for SpaceXAI-specific functionality. These cover:
