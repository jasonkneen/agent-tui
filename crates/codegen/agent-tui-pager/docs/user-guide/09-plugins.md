...
If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, Agent TUI discovers the components above from their standard directories.
...
```bash
agent-tui plugin list [--json] [--available]   # List installed plugins (--available requires --json)
agent-tui plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
agent-tui plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
agent-tui plugin update [<name>]               # Omit the name to update all plugins
agent-tui plugin enable <name>
agent-tui plugin disable <name>
agent-tui plugin details <name>                # Show the plugin's component inventory
agent-tui plugin validate [<path>]             # Validate plugin.json (default: current directory)
agent-tui plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `agent-tui plugin install <source>` without `--trust` and Agent TUI prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
agent-tui plugin marketplace list [--json]
agent-tui plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
agent-tui plugin marketplace remove <url>      # Git URL or local path of a configured source
agent-tui plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
agent-tui plugin marketplace add my-org/team-plugins
agent-tui plugin marketplace list
agent-tui plugin install my-org/team-plugins --trust
agent-tui plugin list
agent-tui plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `agent-tui plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.agent-tui/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `agent-tui plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.agent-tui/pager.toml`:
...
Place this file at `~/.agent-tui/settings.json` or `~/.claude/settings.json`.
...
Agent TUI trusts plugins from `~/.agent-tui/plugins/` automatically. Project plugins in `.grok/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
agent-tui plugin install <source> --trust
```
...
Run `agent-tui inspect` to see every discovered plugin and what it provides:

```bash
agent-tui inspect          # Show plugins with their skills, agents, hooks, and MCP servers
agent-tui inspect --json   # Emit machine-readable JSON
```
