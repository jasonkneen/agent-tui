...
Hooks let you run a script or send an HTTP request at key moments in a Agent TUI session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Agent TUI calls when a specific lifecycle event occurs. Hooks can:
...
1. Create the hooks directory:
   ...
   ```sh
   mkdir -p ~/.agent-tui/hooks
   ```

2. Create a hook file, e.g. `~/.agent-tui/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Agent TUI session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Agent TUI session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.agent-tui/hooks/*.json` | Always | Personal hooks |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.agent-tui/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.agent-tui/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.agent-tui/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.
...
Agent TUI accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Agent TUI skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Agent TUI maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
Agent TUI sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
| `GROK_SESSION_ID`     | The unique identifier of the current Agent TUI session. |
...
When a hook originates from a plugin, Agent TUI additionally injects the following variables:
...
Press `r` in the Hooks tab to reload all hooks from disk. Agent TUI re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.agent-tui/hooks/`) run with your user permissions -- treat them like shell scripts.
