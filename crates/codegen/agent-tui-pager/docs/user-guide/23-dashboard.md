...
- **`agent-tui dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.agent-tui/config.toml` under `[keybindings]` like every other shortcut.
...
The state icon matches Agent TUI's sibling views (
`tasks_pane`):
...
rebound via `~/.agent-tui/config.toml`.
...
process. The plan's Phase 4 ("supervisor / `agent-tui --bg`") would list
