...
MCP (Model Context Protocol) servers extend Agent TUI with external tool integrations. They let Agent TUI interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Agent TUI over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Agent TUI's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.agent-tui/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

Agent TUI spawns a local process and communicates over stdin/stdout:
...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Agent TUI-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.agent-tui/config.toml`) **or repo-level**
...
agent-tui mcp list
agent-tui mcp list --json          # Machine-readable output
...
agent-tui mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
agent-tui mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
agent-tui mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
agent-tui mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
agent-tui mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
agent-tui mcp remove github

# Forget locally stored OAuth credentials for a configured server
agent-tui mcp logout linear

# If its configuration was already removed, provide the previous server URL
agent-tui mcp logout linear --url https://mcp.linear.app/mcp

# Diagnose a server's configuration and connectivity
agent-tui mcp doctor               # Check every configured server
agent-tui mcp doctor github        # Check one server
agent-tui mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `agent-tui mcp add` writes to `~/.agent-tui/config.toml` (`--scope user`). Use `--scope project` to write to `.grok/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `agent-tui mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`agent-tui mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

Removing a server deliberately retains its locally stored OAuth credentials because another scope or project may use the same server name and URL. Run `agent-tui mcp logout <name>` before removal to forget those credentials, or pass the former URL with `--url` after removal. Logout removes the local credential entry; it does not revoke access at the provider.
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Agent TUI handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Agent TUI's own OAuth client with the provider.

Agent TUI walks from the current directory up to the git repo root, loading `.grok/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.agent-tui/config.toml` | All projects | Lowest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Agent TUI reads most other config sections only from `~/.agent-tui/config.toml`.
...
You can enable or disable MCP servers during a session without restarting Agent TUI.
...
Agent TUI loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Agent TUI config | `~/.agent-tui/config.toml`, `.grok/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.agent-tui/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `agent-tui inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Agent TUI handles the credential flow automatically. When an MCP server requests OAuth credentials, Agent TUI opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. Agent TUI stores the resulting tokens under `~/.agent-tui/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Agent TUI expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, Agent TUI captures the process's standard error to `~/.agent-tui/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.agent-tui/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `agent-tui inspect` to see all loaded MCP servers and their sources:

```bash
agent-tui inspect          # Human-readable
agent-tui inspect --json   # Machine-readable
```
