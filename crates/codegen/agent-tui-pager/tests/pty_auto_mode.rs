...
//!
//! Uses `agent-tui-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//!
//! Run with:
//! `cargo test -p agent-tui-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Duration;

use agent_tui_pager_pty_harness::{PtyHarness, pager_binary};
use agent_tui_test_support::TestSandbox;
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    // layer below requirements; "1"/"0" parse to on/off (agent_tui_config::
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (agent-tui-pager-pty-harness)");
