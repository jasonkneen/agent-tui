//! End-to-end tests for the built `agent-tui-pager` binary, driven through a PTY.
//!
//! Mirrors `agent-tui-shell/tests/test_built_binary_e2e.rs`: every test is
...
//! 3. `CARGO_BIN_EXE_agent-tui-pager` (set by `cargo test` for in-tree runs)
...
//! lives in the [`agent_tui_pager_pty_harness`] crate, not here, so its deps
//! never land in the production pager binary.
//!
//! Run locally:
//! ```bash
//! cargo test -p agent-tui-pager --test pty_e2e -- --ignored --nocapture
//! ```
//!
