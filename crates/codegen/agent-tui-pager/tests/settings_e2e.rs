...
fn make_state() -> SettingsModalState {
    ...
    agent_tui_pager::app::set_voice_mode_enabled_for_test(true);
...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    agent_tui_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use agent_tui_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    agent_tui_pager::appearance::cache::set_keep_text_selection(
        agent_tui_pager::appearance::TextSelection::Flash,
    );
    agent_tui_pager::appearance::cache::set_show_thinking_blocks(true);
    agent_tui_pager::appearance::cache::set_prompt_suggestions(true);
    agent_tui_pager::appearance::cache::set_group_tool_verbs(true);
    agent_tui_pager::appearance::cache::set_page_flip_on_send(true);
    agent_tui_pager::appearance::cache::set_combine_queued_prompts(false);
    agent_tui_pager::appearance::cache::set_scroll_mode(
        agent_tui_pager::appearance::ScrollMode::Auto,
    );
    agent_tui_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    agent_tui_pager::appearance::cache::set_scroll_lines(3);
    ...
    for meta in reg.all() {
        ...
        let default_value = agent_tui_pager::settings::default_value_for(meta);
...
// `crates/codegen/agent-tui-pager/src/app/dispatch.rs::tests` (next to
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use agent_tui_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use agent_tui_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use agent_tui_pager::views::settings_modal::ResetConfirmOverlay;
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    agent_tui_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use agent_tui_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use agent_tui_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use agent_tui_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use agent_tui_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use agent_tui_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        agent_tui_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        agent_tui_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_takes_no_arguments() {
    use agent_tui_pager::slash::commands::builtin_commands;
    use agent_tui_pager::slash::registry::CommandRegistry;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use agent_tui_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use agent_tui_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use agent_tui_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use agent_tui_pager::appearance::TextSelection;
    ...
    agent_tui_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    agent_tui_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    agent_tui_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    agent_tui_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    agent_tui_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    agent_tui_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    agent_tui_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    agent_tui_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
