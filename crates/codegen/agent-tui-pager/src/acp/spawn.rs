...
pub async fn spawn_grok_shell(
    ...
    cancel: &CancellationToken,
    memory_config: Option<agent_tui_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    agent_tui_shell::agent::app::apply_otel_config(&auth_manager, &agent_config.grok_com_config);
...
async fn spawn_agent_thread_direct(
    ...
    let rt = tokio::task::spawn_blocking(|| {
        ...
        agent_tui_tty_utils::runtime::build_with_blocking_pool(builder.enable_all())
    })
    .await
