...
use agent_tui_telemetry::startup;
pub use agent_tui_telemetry::startup::{
...
use agent_tui_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<agent_tui_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<agent_tui_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<agent_tui_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    startup::set_auth_mode(agent_tui_shell::managed_config::classify_auth_mode());
    let mut agent_config = AgentConfig::new_from_toml_cfg(raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    agent_tui_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = agent_tui_shell::util::grok_home::grok_home().join("config.toml");
...
mod tests {
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use agent_tui_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use agent_tui_shell::agent::auth_method::{GROK_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};
