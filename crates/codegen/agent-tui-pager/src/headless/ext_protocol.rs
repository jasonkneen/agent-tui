...
use agent_tui_acp_lib::{AcpArgsBox, AcpResult};
...
fn decode_session_notification(method: &str, params: &str) -> ExtEvent {
    ...
    enum XaiUpdate {
        ...
        ResponseCompleted {
            ...
            #[serde(default)]
            usage: Option<agent_tui_shell::extensions::notification::ResponseUsage>,
            #[serde(default)]
            signature: Option<String>,
            #[serde(default)]
    ...
    let agent_tui_notif = match serde_json::from_str::<XaiNotif>(params) {
        Ok(n) => n,
        Err(e) => {
            tracing::warn!(
                method,
