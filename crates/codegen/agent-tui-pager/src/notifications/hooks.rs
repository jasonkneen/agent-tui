...
fn execute_hook(
    ...
    let group = match agent_tui_tty_utils::global_process_scope().enroll_std(&child) {
        ...
        Err(error) => {
            ...
            if !matches!(
                ...
                agent_tui_tty_utils::spawn_child_reaper("notification-hook-reaper", child, None)
            {
    ...
    match agent_tui_tty_utils::wait_child_bounded(&mut child, timeout) {
        ...
        Err(error) if agent_tui_tty_utils::is_child_wait_identity_uncertain(&error) => {
...
fn kill_tree_and_reap(mut child: Child, group: Arc<agent_tui_tty_utils::ProcessGroup>, command: &str) {
...
fn abandon_child(child: Child, group: Option<Arc<agent_tui_tty_utils::ProcessGroup>>, command: &str) {
    if let Err((error, child, group)) =
        agent_tui_tty_utils::spawn_child_reaper("notification-hook-reaper", child, group)
    {
