...
//! render mode.
//!
//! `agent-tui-pager` cannot depend on `agent-tui-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`agent-tui-bin`) wires it up once at
//! startup by calling `agent_tui_pager_minimal::install()`. When nothing has
