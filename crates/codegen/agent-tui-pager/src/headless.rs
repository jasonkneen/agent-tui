...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<agent_tui_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use agent_tui_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<agent_tui_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = agent_tui_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut agent_tui_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    agent_tui_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !agent_tui_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !agent_tui_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &agent_tui_tools::types::compat::CompatConfig::default());
...
pub async fn run_single_turn(
    ...
    agent_tui_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = agent_tui_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = agent_tui_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        agent_tui_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    if track_active {
        let _ = agent_tui_shell::active_sessions::register(
            agent_tui_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(agent_tui_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(agent_tui_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    if track_active {
        ...
        let _ = agent_tui_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = agent_tui_shell::sampling::error::prompt_usage_from_error(&err)
...
mod tests {
    ...
    use super::*;
    use agent_tui_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            agent_tui_workspace::permission::types::PatternMode::Domain
        );
