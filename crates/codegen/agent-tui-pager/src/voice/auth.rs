...
use agent_tui_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<agent_tui_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        agent_tui_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
