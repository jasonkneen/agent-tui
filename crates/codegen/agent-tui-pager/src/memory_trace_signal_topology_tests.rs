...
use agent_tui_test_support::{TestProcess, TestProcessConfig, TestSandbox};
...
async fn memtrace_wait_survives_outer_sigchld_errno_clobber() {
    ...
    let child_deadline = agent_tui_test_support::scaled(CHILD_DEADLINE);
...
fn run_isolated_topology() {
    ...
    let notifier = std::thread::spawn(move || {
        ...
        let handler_deadline = agent_tui_test_support::scaled(HANDLER_DEADLINE);
    ...
    let sample_deadline = agent_tui_test_support::scaled(CHILD_DEADLINE);
...
fn wait_for_sampler_pthread() -> usize {
    let deadline = agent_tui_test_support::scaled(HANDLER_DEADLINE);
