...
pub(crate) async fn run(
    ...
    let voice_mode_enabled = crate::app::resolve_voice_mode_enabled(
        agent_tui_config::env_bool("GROK_VOICE_MODE"),
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            app.voice_config = agent_tui_voice::VoiceConfig::from_config_table(table);
        }
    }
    ...
    {
        use agent_tui_shell::util::config::{resolve_announcements, resolve_tips};
