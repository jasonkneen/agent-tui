...
use agent_tui_acp_lib::{AcpClientMessage, acp_send};
...
/// [`decide`](agent_tui_workspace::folder_trust::decide) precedence.
///
...
fn seed_trust_state(
    app: &mut AppView,
    remote: Option<&agent_tui_shell::util::config::RemoteSettings>,
) {
    use std::io::IsTerminal;
    use agent_tui_workspace::folder_trust::{
        TrustOutcome, decide, decide_inputs_with_interactive, feature_enabled,
    };
    use agent_tui_workspace::trust::workspace_key;
...
pub(crate) async fn run(
    ...
    pending_startup: agent_tui_telemetry::startup::PendingStartup,
    ...
    agent_tui_telemetry::startup::enter(agent_tui_telemetry::startup::StartupPhase::AppInit);
    ...
    let launch_auto = agent_tui_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = agent_tui_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(agent_tui_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = agent_tui_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match agent_tui_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = agent_tui_config::env_bool("GROK_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    app.privacy_banner_acked = agent_tui_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            agent_tui_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = agent_tui_config::env_bool("GROK_PLUGIN_CTA")
    ...
    app.session_picker_grouped = std::env::var("GROK_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            agent_tui_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    agent_tui_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<agent_tui_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == agent_tui_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = agent_tui_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = agent_tui_shell::config::load_merged_requirements();
    let user_config = agent_tui_shell::config::load_from_disk().ok();
    let managed_config = agent_tui_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match agent_tui_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = agent_tui_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat =
        agent_tui_workspace::foreign_sessions::EnabledForeignSessionSources {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            let endpoints_base =
                agent_tui_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .agent_tui_api_base_url;
            app.voice_config =
                agent_tui_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    ...
    app.zdr_access_enabled = agent_tui_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        agent_tui_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        agent_tui_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        agent_tui_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        agent_tui_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        app.active_announcements = agent_tui_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let grok_home = agent_tui_tools::util::grok_home::grok_home();
            app.tip = agent_tui_shell::util::tips::pick_and_advance(&app.tips, &grok_home);
        }
    ...
    let hints = agent_tui_shell::util::config::resolve_hints(
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        agent_tui_shell::util::config::clamped_display_permission_mode(
            agent_tui_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        agent_tui_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = agent_tui_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = agent_tui_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<agent_tui_voice::VoiceEvent>>;
    ...
    if let Some(action) = startup_action {
        ...
        app.finish_startup(agent_tui_telemetry::startup::StartupOutcome::Ok);
    }
    ...
    loop {
        ...
        if let Err(e) = run_pending_suspends(
            ...
            app.finish_startup(agent_tui_telemetry::startup::StartupOutcome::Error);
            return Err(e);
        }
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(agent_tui_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = agent_tui_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
                                        &agent_tui_tools::types::compat::CompatConfig::default(),
                                    );
...
pub(crate) fn load_initial_ui_config() -> agent_tui_shell::agent::config::UiConfig {
    use agent_tui_shell::agent::config::UiConfig;
    let Ok(root) = agent_tui_shell::config::load_effective_config() else {
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && agent_tui_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)
