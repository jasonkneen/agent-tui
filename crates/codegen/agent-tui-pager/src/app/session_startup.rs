...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = agent_tui_shell::util::grok_home::grok_home().join("sessions");
    let encoded = agent_tui_shell::util::grok_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    agent_tui_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = agent_tui_shell::session::persistence::list_summaries(Some(cwd)).await?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &agent_tui_shell::agent::config::Config,
) -> std::sync::Arc<agent_tui_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(agent_tui_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if agent_tui_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = agent_tui_shell::session::resolve_local_session(session_id, cwd) {
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        agent_tui_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    use agent_tui_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use agent_tui_shell::session::restore::{RestoreSessionOpts, restore_session_with_storage};
    ...
    let storage_client = agent_tui_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: agent_tui_shell::session::restore::ProgressCallback = Box::new(move |event| {
    ...
    let recovered_local_id = agent_tui_shell::session::find_local_child_for_remote(session_id, cwd);
...
mod tests {
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = agent_tui_shell::util::grok_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = agent_tui_shell::util::grok_home::grok_home()
            .join("sessions")
            .join(&encoded);
    ...
    #[cfg(feature = "local-workspace")]
    fn advertised_tools_env() -> agent_tui_test_support::EnvGuard {
        agent_tui_test_support::EnvGuard::set(
    ...
    fn resolve_local_workspace_empty_cli_attach_falls_back_to_env() {
        let _env = advertised_tools_env();
        let _sid = agent_tui_test_support::EnvGuard::set(
    ...
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = agent_tui_test_support::EnvGuard::set(
            GROK_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable = agent_tui_test_support::EnvGuard::set(GROK_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        let _tools = agent_tui_test_support::EnvGuard::set(
            GROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
            "workspace.fs_list",
        );
        let _allow =
            agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV);
        let home = tempfile::tempdir().unwrap();
        let home_str = home.path().to_str().unwrap();
        let _home = agent_tui_test_support::EnvGuard::set("HOME", home_str);
        let _userprofile = agent_tui_test_support::EnvGuard::set("USERPROFILE", home_str);
        let err =
    ...
    fn resolve_local_workspace_refuses_uncheckable_toolset() {
        let _tools =
            agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV);
        let tmp = tempfile::tempdir().unwrap();
        let err =
    ...
    fn local_workspace_non_tty_requires_ack() {
        let _ack = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_LOCAL_WORKSPACE_ACK_ENV);
        let home = tempfile::tempdir().unwrap();
        let _home =
            agent_tui_test_support::EnvGuard::set("GROK_HOME", home.path().to_str().unwrap());
