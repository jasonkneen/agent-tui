    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    agent_tui_telemetry::session_ctx::log_event(
                        agent_tui_telemetry::events::PromptSuggestion {
                            action: agent_tui_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::PromptSuggestion {
                        action: agent_tui_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
