...
impl AgentView {
    ...
    pub(super) fn push_promo_cta_link_span(
        &self,
        link_spans_out: &mut Vec<agent_tui_ratatui_inline::LinkSpan>,
        banner_announcements: &[agent_tui_announcements::RemoteAnnouncement],
    ...
    fn push_cta_link_span(
        ...
        {
            link_spans_out.push(agent_tui_ratatui_inline::LinkSpan {
                row: rect.y,
                col_start: rect.x,
...
mod link_click_tests {
    ...
    fn open_prompt_dropdown_suppresses_announcement_cta_click_target() {
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (80, 30);
        let promo = [agent_tui_announcements::RemoteAnnouncement {
    ...
    fn header_upgrade_cta_rect_and_ctrl_o_override() {
        ...
        let reg = ActionRegistry::defaults();
        let cta = || {
            Some(agent_tui_announcements::AnnouncementCta {
                label: Some("Upgrade Account".into()),
                url: Some("https://x.ai/promo".into()),
                caption: None,
            })
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (120, 30);
        let pinned = [agent_tui_announcements::RemoteAnnouncement {
            id: Some("promo-pin".into()),
            severity: Some("promo".into()),
            message: Some("ZZPROMO".into()),
            dismissible: Some(false),
        ...
        let mut agent = make_agent();
        agent.last_terminal_size = (120, 30);
        let dismissible = [agent_tui_announcements::RemoteAnnouncement {
