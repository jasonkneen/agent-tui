...
use agent_tui_telemetry::startup::{self, StartupOutcome, StartupPhase};
use actions::{
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = agent_tui_shell::active_sessions::register(agent_tui_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = agent_tui_tools::types::compat::CompatConfig::default();
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = agent_tui_shell::util::config::worktree_type();
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            agent_tui_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &grok_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || agent_tui_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            ...
            use agent_tui_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = agent_tui_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = agent_tui_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = agent_tui_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        agent_tui_shell::session::restore::ProgressCallback,
                    > = {
                        use agent_tui_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
                    ...
                    match restore_session_with_storage(
                            ...
                            &session_id,
                            &cwd_str,
                            agent_tui_shell::session::restore::RestoreSessionOpts {
                                turn_override: None,
                                progress,
                                restore_code: true,
                            },
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = agent_tui_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use agent_tui_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = agent_tui_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use agent_tui_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            agent_tui_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            agent_tui_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    agent_tui_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = agent_tui_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = agent_tui_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            serde_json::from_value::<
                                Vec<
                                    agent_tui_tools::implementations::skills::types::SkillInfo,
                                >,
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: agent_tui_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use agent_tui_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = agent_tui_shell::agent::auth_method::has_xai_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use agent_tui_shell::session::ClientType;
            use agent_tui_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = agent_tui_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    agent_tui_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            agent_tui_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionMetaFromDisk {
            ...
            tasks
                .spawn(async move {
                    ...
                    let path = agent_tui_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    ...
                    let (title, last_turn_summary) = tokio::task::spawn_blocking(move || -> Option<
                        ...
                        > {
                            ...
                            let summary: agent_tui_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent, nonce } => {
            ...
            tasks
                .spawn(async move {
                    use agent_tui_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "x.ai/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !agent_tui_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let grok_home = agent_tui_shell::util::grok_home::grok_home();
                            let store = agent_tui_shell::auth::read_auth_json(
                                    &grok_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = agent_tui_shell::auth::GrokComConfig::default()
                                .auth_scope();
                            let auth = agent_tui_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    agent_tui_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            agent_tui_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<agent_tui_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
        "x.ai/session/usage",
        serde_json::value::to_raw_value(
                &serde_json::json!({
    ...
    let parsed: agent_tui_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = agent_tui_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
fn format_session_info(
    ...
    let model_display = agent_tui_shell::session::model_display_name(
    ...
    let model_hash_line = if agent_tui_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = agent_tui_sandbox::profile_name()
        .map(|profile| format!("\n  Sandbox: {profile}"))
        .unwrap_or_default();
    ...
    let conversation_line = info
        ...
        .map(|id| format!("\n  Conversation ID: {id}"))
        .unwrap_or_default();
    let version_display = agent_tui_version::display_version(
