...
fn parse_kill_outcome_reads_result_envelope() {
    use agent_tui_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use agent_tui_workspace::session::git::RestoreDegree;
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use agent_tui_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        agent_tui_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        agent_tui_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: agent_tui_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> agent_tui_shell::session::SessionInfoResponse {
    use agent_tui_shell::session::acp_types::{ContextInfo, SessionInfoData};
    agent_tui_shell::session::SessionInfoResponse {
