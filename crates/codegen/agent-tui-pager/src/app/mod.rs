...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    agent_tui_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use agent_tui_shell::agent::config::{BoolFlag, ConfigSource};
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `agent-tui-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`agent-tui-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&agent_tui_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<agent_tui_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<agent_tui_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
async fn bounded_connect(
    ...
    let context = || startup_failure::Context {
        target,
        attempt,
        version: agent_tui_version::display_version_with_commit(
            env!("VERSION_WITH_COMMIT"),
            agent_tui_update::channel_label(),
        ),
        log_path: agent_tui_telemetry::unified_log::path(),
    };
    tokio::select! {
        biased;
...
pub async fn run(
    ...
    let grok_com_config = match agent_tui_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
        Ok(c) => c.grok_com_config,
        Err(e) => {
            tracing::warn!(error = %e, "failed to parse config for auth refresh, using defaults");
            agent_tui_shell::auth::GrokComConfig::default()
        }
    };
    let refreshed_auth = tokio::time::timeout(
        agent_tui_shell::http::STARTUP_AUTH_REFRESH_TIMEOUT,
        agent_tui_shell::auth::try_ensure_fresh_auth(&grok_com_config),
    )
    .await
    .unwrap_or(None);
    let early_prefetch = match refreshed_auth {
        Some(auth) => agent_tui_shell::agent::models::start_early_prefetch_with_auth(Some(auth)),
        None => agent_tui_shell::agent::models::start_early_prefetch(Some(grok_com_config.clone())),
    };
    ...
    agent_tui_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(agent_tui_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = agent_tui_shell::session::persistence::list_summaries(None).await?;
    ...
    let pending_startup = agent_tui_telemetry::startup::PendingStartup::new();
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = agent_tui_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
