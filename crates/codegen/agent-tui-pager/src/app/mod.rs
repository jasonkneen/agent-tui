...
pub async fn run(
    ...
    let grok_com_config =
        match agent_tui_shell::agent::config::Config::new_from_toml_cfg(&raw_config) {
    ...
    let refreshed_auth = agent_tui_shell::auth::try_ensure_fresh_auth(&grok_com_config).await;
    let early_prefetch =
        agent_tui_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    if args.minimal || args.fullscreen {
        ...
        if config_screen_mode != Some(want) {
            tokio::spawn(async move {
                if let Err(e) =
                    agent_tui_shell::util::config::set_screen_mode(want.to_string()).await
                {
