#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn queue_changed_still_adopts_running_prompt_not_terminal_in_replay() {
        ...
        app.agents.get_mut(&id).unwrap().session.loading_replay = true;
        let _ = handle_ext_notification(
            &agent_tui_turn_completed_notif("sess-1", "p-old", "end_turn", true),
            &mut app,
        );
        app.agents.get_mut(&id).unwrap().session.loading_replay = false;
    ...
    fn viewer_stop_hooks_after_marker_attach_to_it() {
        ...
        let _ = handle_ext_notification(
            &agent_tui_turn_completed_notif("sess-view-hooks", "pid-v", "end_turn", false),
            &mut app,
        );
        assert_eq!(
        ...
        let _ = handle_ext_notification(
            &agent_tui_hook_execution_notif("sess-view-hooks", "stop", false),
            &mut app,
        );
        ...
        let _ = handle_ext_notification(
            &agent_tui_hook_execution_notif("sess-view-hooks", "stop_failure", false),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
