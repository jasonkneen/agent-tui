#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use agent_tui_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use agent_tui_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(agent_tui_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
    ...
    fn summary_generated_ext(
        ...
        let meta = if title_is_manual {
            Some(agent_tui_shell::extensions::notification::title_is_manual_meta())
        } else {
            None
        };
    ...
    fn auto_title_notification_strips_controls_and_caps() {
        use agent_tui_shell::session::persistence::MAX_TITLE_SCALARS;
        let mut app = make_app_with_agent("sess-1");
        let dirty = format!(
