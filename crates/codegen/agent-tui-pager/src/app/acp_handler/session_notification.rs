use super::*;
use agent_tui_shell::sampling::error::format_rate_limited_user_message;
    ...
    let changed = match session_notif.update {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        agent_tui_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        agent_tui_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        agent_tui_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
        ...
        XaiSessionUpdate::SessionSummaryGenerated { session_summary } => {
            let title_is_manual = session_notif.meta.as_ref().and_then(|v| {
                v.get(agent_tui_shell::extensions::notification::TITLE_IS_MANUAL_META_KEY)
                    .and_then(|v| v.as_bool())
            });
            match title_is_manual {
                Some(true) => {
                    if let Some(clean) =
                        agent_tui_shell::session::persistence::sanitize_and_cap_title(
                            &session_summary,
                        )
                    {
                ...
                other => {
                    ...
                    if let Some(clean) =
                        agent_tui_shell::session::persistence::sanitize_and_cap_title(&decoded)
                    {
                        agent.generated_session_title = Some(clean);
        ...
        XaiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if agent_tui_shell::agent::chat_modes::process_chat_mode_enabled() {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[agent_tui_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &agent_tui_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && agent_tui_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use agent_tui_tools::types::SessionMode;
