...
pub(super) fn handle_models_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    if let Ok(model_state) = serde_json::from_str::<acp::SessionModelState>(notif.params.get()) {
        ...
        app.models.update_catalog(app_models.available.clone());
        let stale = app
            .models
            .current
            .as_ref()
            .is_none_or(|id| !app.models.available.contains_key(id));
        if stale && let Some(id) = app_models.current {
...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(campaigns) = update.campaigns.clone() {
        let rs = agent_tui_shell::util::config::RemoteSettings {
            campaigns,
            ..Default::default()
        };
        agent_tui_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    }
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        agent_tui_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = agent_tui_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = agent_tui_shell::config::load_effective_config().ok();
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            agent_tui_config::env_bool("GROK_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(agent_tui_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(agent_tui_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        agent_tui_shell::config::load_merged_requirements(),
        agent_tui_shell::config::load_from_disk().ok(),
        agent_tui_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = agent_tui_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = agent_tui_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = agent_tui_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use agent_tui_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use agent_tui_shell::util::config::resolve_slash_command_tags;
        let effective_config = agent_tui_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = agent_tui_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        agent_tui_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<agent_tui_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[agent_tui_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = agent_tui_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = agent_tui_announcements::filter_expired(merged);
    ...
    if agent_tui_announcements::prune_hidden_announcement_ids(
...
pub(super) fn pick_random_announcement(
    announcements: &[agent_tui_announcements::RemoteAnnouncement],
) -> Option<agent_tui_announcements::RemoteAnnouncement> {
...
/// `agent-tui-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `agent-tui-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(default)]
    campaigns: Option<Vec<agent_tui_shell::util::config::CampaignOverride>>,
    #[serde(default)]
