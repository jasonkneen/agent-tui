...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = agent_tui_update::channel_label();
        agent_tui_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            agent_tui_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
...
impl PagerArgs {
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                agent_tui_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                agent_tui_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<agent_tui_sandbox::ProfileName>().ok()
                    != s.parse::<agent_tui_sandbox::ProfileName>().ok() =>
            {
