...
//! inside [`agent_tui_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `agent-tui-pager __mermaid-render` (see
...
//!    via [`agent_tui_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`agent_tui_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `agent-tui-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, agent_tui_mermaid::MermaidError> {
