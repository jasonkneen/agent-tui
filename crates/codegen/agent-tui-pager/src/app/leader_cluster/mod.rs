...
//!
//! ```bash
//! cargo test -p agent-tui-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
use agent_tui_shell::agent::mvp_agent::MvpAgent;
...
impl PagerLeaderCluster {
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (agent_tui_version), not
            // this crate's version: a reconnecting client evicts strictly-older
            // leaders, and "evict" here would signal THIS test process.
            leader_binary_version: agent_tui_version::VERSION.to_string(),
        });
        ...
        // agent-tui-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: agent-tui-shell/tests/test_leader_soak.rs ("Real agent
