...
pub(super) fn set_plan_mode(
    ...
    let mode_id = acp::SessionModeId::new(if new {
        agent_tui_tools::types::SessionMode::Plan.as_id()
    } else {
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    if previous_state != new {
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: agent_tui_telemetry::events::YoloTrigger::Pager,
        });
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(agent_tui_tools::types::SessionMode::Plan.as_id()),
            }]
        }
