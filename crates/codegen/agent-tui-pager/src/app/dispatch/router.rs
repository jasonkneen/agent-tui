...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, agent_tui_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(agent_tui_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::SwitchModel { model_id, effort } => {
            ...
            match crate::runtime_backend::active() {
                ...
                | crate::runtime_backend::RuntimeBackend::Hermes => {
                    return super::settings::setters::set_default_model(app, model_id);
                }
        ...
        Action::SetCodingDataSharing { opted_in } => set_coding_data_sharing(
            app,
            opted_in,
            agent_tui_telemetry::events::CodingDataConsentSource::Settings,
        ),
            ...
            match crate::runtime_backend::set_active(backend) {
                Ok(()) => {
                    let note = match backend {
                        ...
                        crate::runtime_backend::RuntimeBackend::Claude => {
                            "Runtime: Claude — loading models (Claude Code harness)…".to_string()
                        }
                        crate::runtime_backend::RuntimeBackend::Lazar => {
