...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                agent_tui_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(agent_tui_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                agent_tui_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn confirmation_required_builds_plugins_confirmation_with_confirmed_true() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(agent_tui_hooks_plugins_types::ActionOutcome {
                status: agent_tui_hooks_plugins_types::OutcomeStatus::ConfirmationRequired,
    ...
    match &modal.modal_message {
        Some(ModalMessage::Confirmation {
            ...
            assert_eq!(
                action,
                &ConfirmationAction::Plugins(agent_tui_hooks_plugins_types::PluginsAction::Uninstall {
                    plugin_id: "user/ab12/gone".into(),
                    confirmed: true,
                })
            );
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = agent_tui_shell::agent::config::ModelSwitchIncompatibleAgentError {
...
fn session_success_arms_finish_startup_obligation() {
    ...
    for result in results {
        let mut app = test_app_with_agent();
        app.agents.get_mut(&id).unwrap().session.session_id = None;
        app.pending_startup = Some(agent_tui_telemetry::startup::PendingStartup::new());
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = agent_tui_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    let settings = agent_tui_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = agent_tui_shell::util::config::RemoteSettings {
...
fn test_gate() -> agent_tui_shell::auth::GateInfo {
    agent_tui_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(agent_tui_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(agent_tui_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
