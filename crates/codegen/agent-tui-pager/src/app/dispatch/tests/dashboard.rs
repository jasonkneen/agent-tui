...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(agent_tui_shell::sampling::types::ReasoningEffort::High),
        ...
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    ...
                    && *kind == agent_tui_shell::session::unified_list::SessionKind::Build
        )),
    ...
    assert!(
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    && title == "Chat title"
                    && *kind == agent_tui_shell::session::unified_list::SessionKind::Chat
        )),
