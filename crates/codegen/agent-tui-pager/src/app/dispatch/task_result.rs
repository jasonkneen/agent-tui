...
use super::queue::{maybe_drain_queue, note_peek_page_flip, push_and_page_flip};
use super::rewind::{
...
/// Finish a Codex/Claude/Lazar/Hermes external-runtime turn: inject assistant
/// text (or error), close the turn, drain the local prompt queue.
    ...
    match result {
        ...
        TaskResult::SessionMetaFromDisk {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                if let Some((raw, is_manual)) = title
                    && let Some(t) =
                        agent_tui_shell::session::persistence::sanitize_and_cap_title(&raw)
                {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    grok_home: agent_tui_tools::util::grok_home::grok_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                agent_tui_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
