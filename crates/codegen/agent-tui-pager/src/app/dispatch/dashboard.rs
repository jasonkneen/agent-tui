...
fn stage_dashboard_model(
    ...
    effort: Option<agent_tui_shell::sampling::types::ReasoningEffort>,
) {
    let display = app
        .models
        .available
...
pub(super) fn dispatch_dashboard_permission_select(
        ...
        .filter(|_| option_id.0.as_ref() == "allow-always-mcp")
    {
        let selection = match scope.selected {
            crate::views::permission_view::McpScope::Tool => {
                agent_tui_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                }
            }
            crate::views::permission_view::McpScope::Server => match &scope.server_prefix {
                Some(prefix) => agent_tui_workspace::permission::McpScopeSelection::Server {
                    server: prefix.clone(),
                },
                None => agent_tui_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                },
            },
        };
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(agent_tui_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())
