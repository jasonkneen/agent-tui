...
pub(super) fn plugin_cta_candidates(
    response: agent_tui_hooks_plugins_types::MarketplaceListResponse,
) -> (Vec<agent_tui_hooks_plugins_types::MarketplacePluginEntry>, bool) {
    let mut candidates = Vec::new();
    let mut official_source_present = false;
    for source in response.sources {
        let is_official = source.source_name == agent_tui_plugin_marketplace::OFFICIAL_SOURCE_NAME
...
pub(super) fn cta_install_relative_path(
    candidates: &[agent_tui_hooks_plugins_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn plugin_cta_phase_for(
    ...
    use agent_tui_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(agent_tui_telemetry::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == agent_tui_hooks_plugins_types::OutcomeStatus::Success => {
...
pub(super) fn handle_cta_plugin_reload_done(
    ...
    let reload_result = match result {
        Ok(outcome) if outcome.status == agent_tui_hooks_plugins_types::OutcomeStatus::Success => Ok(()),
        Ok(outcome) => Err(crate::app::effects::sanitize_user_error(&outcome.message)),
        Err(e) => Err(e),
    };
    match reload_result {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(agent_tui_telemetry::events::ExtensionsModalOpened {
                    trigger: agent_tui_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = agent_tui_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(agent_tui_telemetry::events::PluginCtaImpression {
