...
pub(super) fn dispatch_import_claude(app: &mut AppView) -> Vec<Effect> {
    ...
    let plan = agent_tui_shell::claude_import::scan_importable_settings(&cwd);

    if plan.is_empty() {
        agent_tui_shell::claude_import_state::mark_dismissed(&cwd);
        // Always write the [claude_compat] imported = true marker so the user's
        // opt-in is recorded even on an empty plan.
        if let Err(e) = agent_tui_shell::claude_import::mark_claude_imported() {
...
pub(super) fn dispatch_import_claude_confirm(app: &mut AppView) -> Vec<Effect> {
    ...
    if selected_count > 0 {
        match agent_tui_shell::claude_import::apply_import(&filtered, &cwd) {
            Ok(result) => {
    ...
    // the slash command.
    agent_tui_shell::claude_import_state::mark_imported(&cwd);
    if let Err(e) = agent_tui_shell::claude_import::mark_claude_imported() {
        tracing::warn!(error = %e, "Failed to write Claude import marker");
    }
    app.has_claude_import = false;
