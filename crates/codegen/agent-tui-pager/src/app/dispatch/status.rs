...
fn log_coding_data_consent_selected(
    source: agent_tui_telemetry::events::CodingDataConsentSource,
    opted_in: bool,
    previous_opted_in: bool,
) {
    use agent_tui_telemetry::events::{CodingDataConsentChoice, CodingDataConsentSelected};
    agent_tui_telemetry::session_ctx::log_event(CodingDataConsentSelected {
        source,
...
pub(in crate::app::dispatch) fn dispatch_privacy_banner_opt_in(app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = set_coding_data_sharing(
        app,
        true,
        agent_tui_telemetry::events::CodingDataConsentSource::PrivacyBanner,
    );
