...
use agent_tui_shell::session::persistence::MAX_TITLE_SCALARS as MAX_RENAME_SCALARS;
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<agent_tui_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    agent_tui_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
fn config_path() -> Option<PathBuf> {
    let home = agent_tui_shell::util::grok_home::grok_home();
...
mod tests {
    ...
    fn rename_at_cap_drops_extra_char() {
        assert_eq!(
            MAX_RENAME_SCALARS,
            agent_tui_shell::session::persistence::MAX_TITLE_SCALARS
        );
        assert_eq!(agent_tui_shell::session::persistence::MAX_TITLE_SCALARS, 100);
    ...
    fn completion_reports_full_miss_for_unreadable_file_url() {
        ...
        let completion = state.complete_clipboard_attachment_paste(
            completion_ctx(None, false),
            crate::app::actions::ProbedAttachment::NoRaster,
            Some("file:///definitely/missing/agent-tui-primary-paste.png".to_owned()),
        );
