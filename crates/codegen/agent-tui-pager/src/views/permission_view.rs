...
use agent_tui_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: agent_tui_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `agent-tui-shell`, NOT duplicated locally.
...
pub(crate) fn preview_command_text(state: &PermissionViewState) -> String {
    match state.bash_highlights.as_ref() {
        Some(h) => agent_tui_workspace::permission::bash_command_splitting::unwrap_command_wrappers(
            &h.highlighted_words,
        )
        .join(" "),
...
fn render_pattern_preview_line(
    ...
    match edit.trimmed() {
        ...
        Some(pattern) => {
            if agent_tui_workspace::permission::bash_pattern_matches_command(pattern, command) {
                spans.push(Span::styled(
            ...
            if agent_tui_workspace::permission::bash_pattern_is_broad(pattern) {
...
mod tests {
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p agent-tui-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/agent-tui-pager
