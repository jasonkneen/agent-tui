...
use agent_tui_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use agent_tui_shell::agent::config::AgentSelectionConfig;
use agent_tui_tools::implementations::skills::discovery::extract_first_paragraph;
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> agent_tui_ratatui_textarea::SingleLineViewport {
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = agent_tui_agent::discovery::discover(cwd);
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match agent_tui_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => agent_tui_config::grok_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".grok").join("personas"),
    }
}
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    agent_tui_shell::config::load_effective_config()
        .ok()
        .and_then(|root| agent_tui_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in agent-tui-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    agent_tui_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        agent_tui_agent::config::PromptMode::Extend => "extend",
        agent_tui_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use agent_tui_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
