...
fn hook_row_label(hook: &agent_tui_hooks_plugins_types::HookInfo) -> String {
    let matcher = hook
        .matcher
        .as_deref()
        .map(|m| format!(" /{m}"))
...
fn is_official_marketplace_source(source: &agent_tui_hooks_plugins_types::MarketplaceScanResult) -> bool {
    source.source_name == agent_tui_plugin_marketplace::OFFICIAL_SOURCE_NAME
        || agent_tui_plugin_marketplace::is_official_source_url(&source.source_url_or_path)
}
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> agent_tui_telemetry::events::ExtensionsModalTab {
        use agent_tui_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<agent_tui_shell::util::config::McpServerConfig>,
    },
...
pub enum ConfirmationAction {
    ...
    Hooks(agent_tui_hooks_plugins_types::HooksAction),
    ...
    Plugins(agent_tui_hooks_plugins_types::PluginsAction),
    /// Replay a marketplace action (uninstall plugin or remove source).
    Marketplace(agent_tui_hooks_plugins_types::MarketplaceAction),
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use agent_tui_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
fn classify_hook_source(source_dir: &str) -> HookSourceMeta {
    let grok = agent_tui_config::grok_home();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            agent_tui_tools::types::config_source::ConfigSource::User { path } => {
            ...
            agent_tui_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            agent_tui_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
mod tests {
    ...
    fn make_skill(
        ...
        agent_tui_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: agent_tui_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = agent_tui_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(agent_tui_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    agent_tui_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    agent_tui_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn skills_groups_then_az_by_label() {
        ...
        project_z.scope = agent_tui_tools::implementations::skills::types::SkillScope::Local;
        project_z.config_source = Some(
            agent_tui_tools::types::config_source::ConfigSource::Project {
        ...
        project_a.scope = agent_tui_tools::implementations::skills::types::SkillScope::Repo;
    ...
    fn hooks_group_and_row_order() {
        ...
        h_stop.event = agent_tui_hooks_plugins_types::HookEvent::Stop;
        let mut h_pre = make_hook("pre-hook", "/tmp/hooks-src", false);
        h_pre.event = agent_tui_hooks_plugins_types::HookEvent::PreToolUse;
        h_pre.matcher = Some("Bash".into());
        let mut h_notify = make_hook("notify-hook", "/tmp/hooks-src", false);
        h_notify.event = agent_tui_hooks_plugins_types::HookEvent::Notification;
