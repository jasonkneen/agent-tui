...
pub fn entry_title(agent: &AgentView) -> String {
    ...
    if let Some(title) = agent.generated_session_title.as_deref() {
        ...
        if !trimmed.is_empty() {
            let clean =
                agent_tui_tools::implementations::skills::skill::extract_skill_display_text(trimmed);
...
/// [`agent_tui_shell::session::persistence::is_forbidden_title_char`]; persist
...
pub(crate) fn sanitize_display_text(s: &str) -> Cow<'_, str> {
    use agent_tui_shell::session::persistence::is_forbidden_title_char;
