...
//! the generic [`CodeBlockSpan`](agent_tui_markdown::CodeBlockSpan) API) and
...
use agent_tui_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](agent_tui_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](agent_tui_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a agent_tui_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
/// dark. The render worker maps this to `agent_tui_mermaid::MermaidTheme`; it
