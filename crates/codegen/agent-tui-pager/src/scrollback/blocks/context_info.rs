...
use agent_tui_shell::session::{ContextInfo, count_detail};
...
impl ContextInfoBlock {
    ...
    fn build_lines(&self, theme: &Theme, bar: BarLayout) -> Vec<Line<'static>> {
        ...
        // (routed from agent-tui-shell's model config resolution). This makes
...
mod tests {
    use super::*;
    use agent_tui_shell::session::TokenUsageCategory;
