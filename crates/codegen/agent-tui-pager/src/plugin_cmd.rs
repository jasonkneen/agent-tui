...
//!
//! Business logic lives in `agent_tui_shell::plugin` (shared orchestration)
//! and lower crates (`agent-tui-agent`, `agent-tui-plugin-marketplace`). This
...
use agent_tui_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use agent_tui_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use agent_tui_plugin_marketplace::SourceKind;
use agent_tui_plugin_marketplace::git::SourceCacheLease;
use agent_tui_shell::plugin::{self, RepoUpdateOutcome, UninstallError};
...
fn installed_plugins(
    repos: &[(
        &str,
        &agent_tui_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = agent_tui_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = agent_tui_plugin_marketplace::load_sources(&config);
    sources.extend(agent_tui_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in agent_tui_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                agent_tui_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &agent_tui_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = agent_tui_plugin_marketplace::git::default_cache_root();
            agent_tui_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> agent_tui_telemetry::events::InstallKind {
    if is_git {
        agent_tui_telemetry::events::InstallKind::Git
    } else {
        agent_tui_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: agent_tui_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = agent_tui_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use agent_tui_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &agent_tui_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            agent_tui_telemetry::session_ctx::log_event(
                agent_tui_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
...
fn cmd_enable(name: &str) -> Result<()> {
    ...
    if let Err(e) = agent_tui_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    agent_tui_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = agent_tui_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    agent_tui_shell::config::add_disabled_plugin(name)
...
fn marketplace_add(
    ...
    use agent_tui_shell::plugin::MarketplaceAddInput;
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        agent_tui_plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
...
fn find_removal_source<'a>(
    sources: &'a [agent_tui_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a agent_tui_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        agent_tui_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &agent_tui_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match agent_tui_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use agent_tui_plugin_marketplace::MarketplaceSource;
    ...
    fn removal_fixture() -> Vec<MarketplaceSource> {
        vec![
            ...
            MarketplaceSource {
                name: "official".into(),
                kind: SourceKind::Git {
                    url: "https://github.com/agent-tui-org/plugin-marketplace.git".into(),
                    branch: None,
                },
            },
            MarketplaceSource {
    ...
    fn find_removal_source_matches_by_url_ignoring_git_suffix() {
        ...
        let found = find_removal_source(
            &sources,
            "https://github.com/agent-tui-org/plugin-marketplace",
            Path::new("/"),
        )
        .unwrap();
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        let msg = trust_prompt(
            ...
            "sentry@agent-tui-org/plugin-marketplace",
        );
        assert!(
            msg.starts_with(
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  grok plugin install sentry@agent-tui-org/plugin-marketplace --trust"
        ));
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = agent_tui_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = agent_tui_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
