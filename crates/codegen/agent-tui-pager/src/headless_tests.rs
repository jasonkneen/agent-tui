...
fn drain_records_task_backgrounded_delivered_at_exit() {
    let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<agent_tui_acp_lib::AcpClientMessage>();
    let payload = serde_json::json!({
        "sessionId": "sess-1",
    ...
    tx.send(agent_tui_acp_lib::AcpClientMessage::ExtNotification(
...
fn handler_answers_ext_method_instead_of_dropping() {
    ...
    use agent_tui_tools::implementations::grok_build::ask_user_question::AskUserQuestionExtResponse;
    let raw = serde_json::value::to_raw_value(&serde_json::json!({})).unwrap();
    let (tx, mut rx) = tokio::sync::oneshot::channel();
    let msg = agent_tui_acp_lib::AcpClientMessage::ExtMethod(agent_tui_acp_lib::AcpArgs {
