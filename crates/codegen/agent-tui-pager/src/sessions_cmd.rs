...
use agent_tui_shell::auth::{AuthManager, try_ensure_fresh_auth};
use agent_tui_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let client = agent_tui_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = agent_tui_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                agent_tui_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
            )
            .await;
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use agent_tui_shell::session::merge::REMOTE_TIMEOUT;
            use agent_tui_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = agent_tui_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
