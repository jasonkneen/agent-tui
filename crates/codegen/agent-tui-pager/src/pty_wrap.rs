...
///
/// Delegates to [`agent_tui_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = agent_tui_shell::util::clipboard::set_text(text) {
...
fn host_clipboard_image_frame() -> Vec<u8> {
    let image = agent_tui_pager_render::clipboard::system_clipboard_get_image();
