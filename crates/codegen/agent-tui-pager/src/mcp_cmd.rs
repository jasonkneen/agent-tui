...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = agent_tui_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    let disabled = agent_tui_shell::util::config::disabled_mcp_server_names(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    agent_tui_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => agent_tui_shell::util::config::user_config_path(),
        McpScope::Project => {
            agent_tui_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn select_remove_site(
    ...
    use agent_tui_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    agent_tui_shell::util::config::user_config_path(),
                )
            })
        })
}
...
fn mcp_server_is_known(name: &str, cwd: &Path) -> bool {
    ...
    agent_tui_shell::util::config::cli_known_mcp_server_names(cwd).contains(name)
}

fn available_mcp_server_names(cwd: &Path) -> Vec<String> {
    let mut names: Vec<String> = agent_tui_shell::util::config::cli_known_mcp_server_names(cwd)
        .into_iter()
        .collect();
    names.sort();
    names
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    let was_disabled = agent_tui_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);

    let modified =
        agent_tui_shell::util::config::save_mcp_server_enabled_in(name, enabled, &cwd).await?;

    let now_disabled = agent_tui_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);
    ...
    let user_config = agent_tui_shell::util::config::user_config_path();
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        agent_tui_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = agent_tui_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        agent_tui_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = agent_tui_shell::util::config::user_config_path();
