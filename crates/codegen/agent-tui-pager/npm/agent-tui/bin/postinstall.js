#!/usr/bin/env node
// Runs once after npm install/update. Reads the agent-tui binary from the
// matching per-platform optional dependency (@agent-tui/agent-tui-<platform>)
// and installs it to ~/.agent-tui/bin/ using versioned filenames:
//
//   Unix:    agent-tui-<version>  +  agent-tui  (symlink)
//   Windows: agent-tui-<version>.exe  +  agent-tui.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.agent-tui', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@agent-tui/agent-tui: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@agent-tui/agent-tui-${key}`;
    try {
...
if (!version) {
    console.error('@agent-tui/agent-tui: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@agent-tui/agent-tui: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@agent-tui/agent-tui: failed to update ${canonicalPath}: ${e2.message}`);
...
// Uses an exact prefix match + hyphen + digit to avoid agent-tui-* matching agent-tui-pager-*.
...
if (!platformDir) {
    console.error(`@agent-tui/agent-tui: platform package @agent-tui/agent-tui-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @agent-tui/agent-tui');
    process.exit(0);
}

installBinary('agent-tui', platformDir, `grok${EXE}`);
cleanupOldVersions('agent-tui');
cleanupOldVersions('agent-tui-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.agent-tui');
    ...
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @agent-tui:registry',
...
// Set AGENT_TUI_INSTALL_COMPLETIONS=1 to auto-generate to ~/.agent-tui/completions.
const GROK_PATH = path.join(CANONICAL_DIR, `grok${EXE}`);
if (process.env.AGENT_TUI_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.agent-tui', 'completions');
        ...
        console.log('Completions generated to ~/.agent-tui/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
