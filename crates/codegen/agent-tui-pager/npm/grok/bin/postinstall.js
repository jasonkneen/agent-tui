...
// matching per-platform optional dependency (@agent-tui-official/grok-<platform>)
...
if (!SUPPORTED.has(key)) {
    console.error(`@agent-tui-official/grok: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@agent-tui-official/grok-${key}`;
    try {
...
if (!version) {
    console.error('@agent-tui-official/grok: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (!fs.existsSync(versionedPath) && !writeVendorBinary(brPath, rawPath, versionedPath)) {
        console.error(`@agent-tui-official/grok: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@agent-tui-official/grok: failed to update ${canonicalPath}: ${e2.message}`);
    ...
    if (!fs.existsSync(canonicalPath)) {
        console.error(`@agent-tui-official/grok: ${canonicalName} did not resolve after install`);
        return false;
    }
...
if (!platformDir) {
    console.error(`@agent-tui-official/grok: platform package @agent-tui-official/grok-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @agent-tui-official/grok');
    process.exit(0);
}
    ...
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @agent-tui-official:registry',
