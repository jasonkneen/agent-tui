...
pub fn estimate_system_message_tokens(item: &ConversationItem) -> u64 {
    match item {
        ConversationItem::System(s) => agent_tui_token_estimation::estimate_tokens(&s.content),
        _ => 0,
    }
}
...
pub fn estimate_tool_definition_tokens(td: &agent_tui_sampling_types::ToolDefinition) -> u64 {
    ...
    ((name_len + desc_len + params_len) as u64) / agent_tui_token_estimation::BYTES_PER_TOKEN
}

/// Sum [`estimate_tool_definition_tokens`] across a slice.
pub fn estimate_tool_definitions_tokens(tds: &[agent_tui_sampling_types::ToolDefinition]) -> u64 {
    tds.iter().map(estimate_tool_definition_tokens).sum()
}

/// Bytes/4 estimate for a single [`ConversationItem`].
///
/// Images are counted at [`agent_tui_token_estimation::IMAGE_TOKEN_ESTIMATE`] each.
...
pub fn estimate_item_tokens(item: &ConversationItem) -> u64 {
    ...
    match item {
        ...
        ConversationItem::User(u) => {
            ...
            (bytes as u64) / agent_tui_token_estimation::BYTES_PER_TOKEN
                + agent_tui_token_estimation::estimate_image_tokens(images)
        }
        ...
        ConversationItem::ToolResult(tr) => agent_tui_token_estimation::estimate_tokens(&tr.content),
        ConversationItem::BackendToolCall(b) => {
            agent_tui_token_estimation::estimate_tokens(&b.text_summary())
        }
        ConversationItem::Reasoning(r) => {
            ...
            let text_bytes = agent_tui_sampling_types::reasoning_item_text(r).len();
            let enc_bytes = r.encrypted_content.as_deref().map(str::len).unwrap_or(0);
            ((text_bytes + enc_bytes) as u64) / agent_tui_token_estimation::BYTES_PER_TOKEN
        }
    }
}

/// Estimate token footprint: text bytes / 4, images at the per-image
/// constant defined by [`agent_tui_token_estimation::IMAGE_TOKEN_ESTIMATE`].
pub fn estimate_conversation_tokens(items: &[ConversationItem]) -> u64 {
    items.iter().map(estimate_item_tokens).sum()
}

/// grok-build's [`ItemTokenCounter`](agent_tui_compaction::ItemTokenCounter)
...
impl agent_tui_compaction::ItemTokenCounter<ConversationItem> for EstimatedItemTokenCounter {
...
mod tests {
    ...
    fn estimated_item_token_counter_matches_estimate_item_tokens() {
        use agent_tui_compaction::ItemTokenCounter;
    ...
    fn estimate_tool_definition_tokens_counts_name_desc_params() {
        ...
        let td = agent_tui_sampling_types::ToolDefinition::function(
            "search",
            Some("find a file"),
            serde_json::json!({}),
        );
    ...
    fn estimate_tool_definitions_tokens_sums_across_slice() {
        let a = agent_tui_sampling_types::ToolDefinition::function(
            "a",
            None::<&str>,
            serde_json::json!({}),
        );
        let b = agent_tui_sampling_types::ToolDefinition::function(
            "b",
            None::<&str>,
            serde_json::json!({}),
        );
