...
//!   here would force every consumer of `agent-tui-workspace-types`
...
//! (`agent-tui-workspace`) wraps this in its own `Request<T>` that
//! adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `agent-tui-workspace-types`
