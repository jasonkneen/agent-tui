...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] agent_tui_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `agent-tui-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `agent_tui_tools::ToolError`) happens
