//! Wire types for the `agent-tui-workspace` API.
//!
...
//! `agent-tui-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `agent-tui-workspace-types` (instead of `agent-tui-workspace` or
/// `agent-tui-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `agent_tui_workspace::permission`
