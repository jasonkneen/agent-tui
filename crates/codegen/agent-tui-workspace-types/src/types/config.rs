...
//! agent-session config types in `agent-tui-config` and friends.
...
/// `agent-tui-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `agent_tui_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
