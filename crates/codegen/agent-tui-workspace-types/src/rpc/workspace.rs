...
/// over `agent_tui_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `agent_tui_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `agent_tui_tools`'s `TodoState`). `status`
...
/// in `agent-tui-workspace/src/hub_server.rs`.
...
pub struct WorkspaceInfo {
    ...
    pub shell: String,
    pub cwd: String,
    /// Server binary version (`agent_tui_version::VERSION`); `None` on
