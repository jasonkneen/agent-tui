...
//!
//! `agent_tui_hooks` pulls in `git2`/`reqwest`/`agent-tui-tools`, too heavy for
...
/// Wire mirror of `agent_tui_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `agent_tui_hooks::config::HookSpec`.
///
...
/// `agent-tui-hooks`, so a server-side test (`agent-tui-workspace`'s
...
/// Wire mirror of `agent_tui_hooks::event::HookEventName`.
///
