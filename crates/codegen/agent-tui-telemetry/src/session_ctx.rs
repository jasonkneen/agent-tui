...
//!
//! Extracted from `agent-tui-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `agent-tui-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `agent-tui-workspace` (remote sampler / workspace server).
    Workspace,
}
