...
//! targets `agent_tui_hooks` or `agent_tui_agent::plugins` at key lifecycle
...
/// Filters to hooks (`agent_tui_hooks`) and plugins (`agent_tui_agent::plugins`) targets.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "agent_tui_hooks=debug,agent_tui_agent::plugins=debug",
    );
