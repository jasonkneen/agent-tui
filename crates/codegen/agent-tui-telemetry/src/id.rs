...
//!
//! Extracted from `agent-tui-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$GROK_HOME` is resolved through `agent-tui-config::grok_home`.
...
fn load_or_compute_agent_id() -> String {
    let cache_path = agent_tui_config::grok_home().join("agent_id");
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    agent_tui_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
