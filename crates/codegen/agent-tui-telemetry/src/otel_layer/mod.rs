...
//!
//! `agent-tui-pager` uses this module to set up OTLP
...
///
/// Built by the binaries (`agent-tui-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `agent_tui_shell::auth::credential_provider::build_otel_credential_provider`.
...
fn build_export_headers(
    ...
    snapshot: &agent_tui_auth::CredentialSnapshot,
...
mod tests {
    ...
    use std::sync::Mutex;
    use agent_tui_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
