...
use agent_tui_config::grok_home;
...
///
/// Lives here (not in `agent-tui-pager`) so the firehose directives below and
...
/// safe. See `agent-tui-pager/src/tracing.rs` for the consumer side.
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,agent_tui_pager=debug,agent_tui_shell=debug,agent_tui_tools=debug,agent_tui_telemetry=debug,agent_tui_agent=debug,agent_tui_mcp=debug,agent_tui_acp_lib=debug,sampling_log=off";
...
mod tests {
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "agent_tui_shell", "inside session");
            });
            tracing::info!(target: "agent_tui_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `agent_tui_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "agent_tui_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "agent_tui_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "agent_tui_shell", "one first");
                tracing::info!(target: "agent_tui_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "agent_tui_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
