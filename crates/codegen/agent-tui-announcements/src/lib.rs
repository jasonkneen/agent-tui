...
//!
//! This crate provides the common logic used by `agent-tui-shell` and
//! `agent-tui-pager` for handling announcements (banner notifications).
...
fn announcements_state_path() -> PathBuf {
    agent_tui_tools::util::grok_home::grok_home().join("announcements.json")
}
