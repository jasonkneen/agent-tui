...
fn local_grok_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("agent-tui-pager{}", std::env::consts::EXE_SUFFIX))
}
...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "agent-tui-pager", "--bin", "agent-tui-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build agent-tui-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build agent-tui-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "agent-tui-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve grok binary: `GROK_BINARY` env (CI) or a locally built `agent-tui-pager` binary.
pub fn grok_binary() -> PathBuf {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_agent-tui-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
