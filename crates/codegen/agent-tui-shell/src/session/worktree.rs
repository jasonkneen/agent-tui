...
//!
//! Core worktree lifecycle logic lives in [`agent_tui_workspace::worktree`].
...
use std::path::Path;
use agent_tui_workspace::session::git::find_git_root_from_path;
pub use agent_tui_workspace::worktree::*;
...
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| agent_tui_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
async fn cleanup_worktree_on_failure(source_cwd: &str, worktree_path: &str) {
    ...
    if is_jj {
        ...
        if let Ok(root) = find_git_root_from_path(std::path::Path::new(source_cwd)) {
            let _ = agent_tui_workspace::session::git::git_cli(&root, &["worktree", "prune"]).await;
        }
    }
}
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> agent_tui_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return agent_tui_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    agent_tui_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<agent_tui_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared
/// [`agent_tui_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &agent_tui_workspace::session::git::CheckoutSessionOutcome,
    kind: agent_tui_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = agent_tui_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub async fn resume_session_in_worktree(
    ...
    ops: &agent_tui_workspace::WorkspaceOps,
    ...
    use agent_tui_workspace::session::git::effective_worktree_path;
    tracing::info!(
    ...
    let restore_degree = if codebase_ok {
        Some(agent_tui_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &agent_tui_workspace::WorkspaceOps,
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| agent_tui_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if agent_tui_workspace::session::git::should_warn_registry_disabled(
                is_jj,
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(agent_tui_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(agent_tui_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(agent_tui_workspace::session::git::RestoreDegree::HeadOnly)
        );
