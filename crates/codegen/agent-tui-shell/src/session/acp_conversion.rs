//! ACP conversion functions for `agent-tui-tools`'s `ToolOutput`.
//!
//! These standalone functions convert `agent_tui_tools::types::output::ToolOutput`
...
pub(crate) fn acp_tool_update(
    ...
    match output {
        ToolOutput::ReadFile(read_file_output) => {
            let (content, status) = match read_file_output {
                ...
                ReadFileOutput::ImageContent(image_content) => {
                    ...
                    // `agent-tui-tools` stay free of an
        ...
        ToolOutput::WebFetch(web_fetch_output) => {
            use agent_tui_tools::types::output::WebFetchOutput;
        ...
        ToolOutput::Todo(todo_output) => {
            use agent_tui_tools::types::output::TodoWriteOutput;
        ...
        ToolOutput::AskUserQuestion(ask) => {
            let message = match ask {
                agent_tui_tools::types::output::AskUserQuestionOutput::UserAnswered { message } => {
                    message.clone()
                }
                agent_tui_tools::types::output::AskUserQuestionOutput::QuestionsSent {
                    message,
                    ..
                } => message.clone(),
            };
        ...
        ToolOutput::EnterPlanMode(enter) => {
            let message = match enter {
                agent_tui_tools::types::output::EnterPlanModeOutput::Entered { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
        ...
        ToolOutput::ExitPlanMode(exit) => {
            let message = match exit {
                agent_tui_tools::types::output::ExitPlanModeOutput::PlanReady {
                    message, ..
                } => message.clone(),
                agent_tui_tools::types::output::ExitPlanModeOutput::EmptyPlan {
                    message, ..
                } => message.clone(),
            };
            Some(acp::ToolCallUpdate::new(
...
///
/// This converts `agent-tui-tools`' TodoItem (which has `id`, `content: Option<String>`,
...
mod tests {
    ...
    fn test_acp_tool_update_todo_returns_completed() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            ...
            todos: vec![],
            state: agent_tui_tools::implementations::grok_build::todo::TodoState::default(),
        }));
        let update = acp_tool_update(&output, "call-1", None, None).unwrap();
        assert_eq!(update.fields.status, Some(acp::ToolCallStatus::Completed));
    }
    ...
    fn test_turn_end_plan_cleanup_preserves_semantics_and_priority() {
        use crate::tools::todo::plan_entry_from_todo_item;
        use agent_tui_tools::implementations::grok_build::todo::{
    ...
    fn test_acp_plan_update_todo() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            summary_for_prompt: "tasks".to_string(),
            todos: vec![
                agent_tui_tools::implementations::grok_build::todo::TodoItem {
                    content: "Task 1".to_string(),
                    priority:
                        agent_tui_tools::implementations::grok_build::todo::TodoPriority::Medium,
                    status:
                        agent_tui_tools::implementations::grok_build::todo::TodoStatus::Completed,
                    meta: None,
                },
            ],
            state: agent_tui_tools::implementations::grok_build::todo::TodoState::default(),
