...
//!
//! Receives [`agent_tui_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`agent_tui_mcp::liveness`]),
//! - the [`agent_tui_mcp::servers::GrokClientHandler`] (server-pushed
...
pub enum McpServerStatus {
    /// Client is in [`agent_tui_mcp::servers::ClientStateKind::Ready`]
...
/// [`agent_tui_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && agent_tui_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`agent_tui_acp_lib::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use agent_tui_mcp::servers::{McpClient, McpState};
    ...
    fn discard_gateway() -> agent_tui_acp_lib::AcpAgentGatewaySender {
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = agent_tui_acp_lib::AcpAgentGatewaySender::new(gw_tx);
