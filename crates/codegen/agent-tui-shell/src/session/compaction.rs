...
use agent_tui_sampling_types::{ApiBackend, ConversationItem};
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                agent_tui_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            agent_tui_telemetry::session_ctx::log_event(
                agent_tui_telemetry::events::AutoCompactSuppressed {
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            agent_tui_sampler::SamplingErrorKind::Auth,
        ))
    }
    ...
    async fn run_compact_inner(
        ...
        trigger: agent_tui_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        let (cancel, _cancel_scope) = self.compaction.cancel.enter();
        ...
        let trigger_str = match trigger {
            agent_tui_telemetry::events::CompactionTrigger::Manual => "manual",
            agent_tui_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = agent_tui_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            agent_tui_hooks::event::HookEventName::PreCompact,
            agent_tui_hooks::event::HookPayload::PreCompact {
        ...
        let effective_tool_defs: Vec<agent_tui_sampling_types::ToolDefinition> = self
        ...
        let compaction_tools: Vec<agent_tui_sampling_types::ToolSpec> = effective_tool_defs
            ...
            .collect();
        let compaction_hosted_tools: Vec<agent_tui_sampling_types::HostedTool> =
        ...
        let fr_config = agent_tui_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match agent_tui_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(agent_tui_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(agent_tui_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(agent_tui_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            agent_tui_telemetry::session_ctx::log_event(
                                agent_tui_telemetry::events::CompactionRetryDegraded {
                                    trigger,
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                ...
                                agent_tui_tools::computer::types::TaskKind::Bash => {
                    ...
                    let connected_mcp_servers = {
                        ...
                        use agent_tui_tools::implementations::search_tool::{
                    ...
                    let todos = {
                        ...
                        use agent_tui_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn agent_tui_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn agent_tui_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        agent_tui_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: agent_tui_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            agent_tui_hooks::event::HookEventName::PostCompact,
            agent_tui_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &agent_tui_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                agent_tui_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<agent_tui_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use agent_tui_workspace::file_system::MockFs;
    use agent_tui_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        SessionActor {
            ...
            vcs_kind: agent_tui_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: agent_tui_sampler::SamplerHandle::noop(),
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use agent_tui_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn api_error_with_context_window(context_window: u64) -> agent_tui_sampler::SamplingErrorInfo {
        agent_tui_sampler::SamplingErrorInfo {
            kind: agent_tui_sampler::SamplingErrorKind::Api,
            ...
            credential: agent_tui_sampling_types::SentCredential::Unknown,
        }
    }
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = agent_tui_sampler::SamplingErrorInfo {
                    kind: agent_tui_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
