...
fn test_actor_inner(
    ...
    let sampling_client = OaiCompatClient::new(agent_tui_sampler::SamplerConfig::default()).unwrap();
    let mut summary =
...
fn save_session_titles(
    server: &agent_tui_test_support::MockInferenceServer,
    session_id: &str,
) -> Vec<SaveTitle> {
...
fn request_path_summary(server: &agent_tui_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .iter()
