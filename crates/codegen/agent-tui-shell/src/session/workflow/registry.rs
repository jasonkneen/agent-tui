...
use agent_tui_workflow::{WorkflowMeta, extract_meta};
...
pub(crate) enum ResolveError {
    ...
    Meta(#[from] agent_tui_workflow::MetaError),
}

pub(crate) fn project_root(session_cwd: &Path) -> PathBuf {
    agent_tui_workspace::session::git::find_git_root_from_path(session_cwd)
...
mod tests {
    ...
    fn inline_resolution_validates_name_and_size() {
        ...
        assert!(matches!(
            ...
            Err(ResolveError::Meta(agent_tui_workflow::MetaError::InvalidName))
        ));
        assert!(matches!(
