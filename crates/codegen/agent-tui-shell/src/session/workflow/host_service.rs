...
use agent_tui_workflow::{AgentOpts, AgentResult, BudgetState, HostError, WorkflowHostRequest};
...
pub(crate) const WORKFLOW_MAX_AGENT_RUNS: u32 =
    (agent_tui_workflow::MAX_AGENT_BUDGET as u32) * (SCHEMA_CONTRACT_RETRIES + 1);
...
impl HostService {
        ...
        match self.agent_slots.try_acquire() {
            ...
            Err(tokio::sync::TryAcquireError::NoPermits) => {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::SubagentLimitHit::workflow_run_concurrent(
    ...
    async fn spawn_agent(&self, mut opts: AgentOpts) -> Result<AgentResult, HostError> {
        ...
        let isolation = opts
            .isolation_worktree
            .then_some(agent_tui_tool_types::SubagentIsolationMode::Worktree);
    ...
    async fn git_diff_since(&self, commit: &str) -> Result<String, HostError> {
        ...
        agent_tui_tty_utils::detach_command(&mut cmd);
...
mod tests {
    ...
    async fn a_run_keeps_at_most_max_concurrent_agents_live() {
        ...
        let succeed = |spawn: agent_tui_tools::implementations::grok_build::task::types::SubagentSpawnRequest| {
            spawn
                .respond_with(|request| {
                    agent_tui_tools::implementations::grok_build::task::types::SubagentResult {
                        success: true,
