...
use agent_tui_workflow::{Journal, WorkflowOutcome, WorkflowRunParams};
...
pub(crate) const WORKFLOW_DEFAULT_AGENT_BUDGET: u64 = agent_tui_workflow::DEFAULT_AGENT_BUDGET;
...
impl WorkflowManager {
    ...
    pub(crate) fn launch(
        ...
        let (run_id, journal, state) = match &spec.resume_run_id {
            Some(run_id) => {
                ...
                if existing.status
                    ...
                    && existing.agents_used >= agent_tui_workflow::MAX_AGENT_BUDGET
                {
                    return Err(LaunchError::NotResumable(
        ...
        let exec = tokio::task::spawn_blocking(move || {
            agent_tui_workflow::run_workflow(WorkflowRunParams {
                script,
                args,
                journal,
                host_tx,
            ...
            if !epoch_matches {
                ...
                log_run_ended(
                    RunEndMetadata {
                        ...
                        status: agent_tui_telemetry::events::WorkflowRunEndStatus::Superseded,
                        duration_ms: elapsed,
                        agents_used,
                        agent_budget,
                    },
            ...
            if state.is_none() {
                ...
                log_run_ended(
                    RunEndMetadata {
                        run_id: &watcher_run_id,
                        parent_session_id: &watcher_session_id,
                        status: agent_tui_telemetry::events::WorkflowRunEndStatus::Interrupted,
                        duration_ms: 0,
                        agents_used: 0,
                        agent_budget: None,
                    },
    ...
    pub(crate) fn test_bundle_with_session_dir(
        ...
        let notify = super::notify::WorkflowNotifySender::new(
            ...
            agent_tui_acp_lib::AcpAgentGatewaySender::new(gateway_tx),
            persist_tx,
            store.clone(),
        );
    ...
    fn cancel_children_for_run(&self, run_id: &str) -> bool {
        ...
        self.subagent_event_tx
            .send(
                agent_tui_tools::implementations::grok_build::task::types::SubagentEvent::Cancel(
                    agent_tui_tools::implementations::grok_build::task::types::SubagentCancelRequest {
                        parent_session_id: Some(self.session_id.clone()),
                        target: agent_tui_tools::implementations::grok_build::task::types::SubagentCancelTarget::WorkflowRunId(
                            run_id.to_owned(),
                        ),
                        respond_to,
                    },
...
fn log_run_started(
    ...
    use agent_tui_telemetry::events::{WorkflowRunStarted, WorkflowSourceKind};
    agent_tui_telemetry::session_ctx::log_event(WorkflowRunStarted {
...
struct RunEndMetadata<'a> {
    ...
    status: agent_tui_telemetry::events::WorkflowRunEndStatus,
    duration_ms: u64,
    agents_used: u64,
...
fn log_run_ended(episode: RunEndMetadata<'_>, stats: &super::host_service::WorkflowAgentStats) {
    agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::WorkflowRunEnded {
...
) -> agent_tui_telemetry::events::WorkflowRunEndStatus {
    use crate::session::workflow::tracker::WorkflowRunStatus as S;
    use agent_tui_telemetry::events::WorkflowRunEndStatus as E;
    match status {
        S::Active => E::Active,
...
mod tests {
    ...
    type CancelLog = Arc<
        parking_lot::Mutex<
            Vec<agent_tui_tools::implementations::grok_build::task::types::SubagentCancelTarget>,
        >,
    >;
    ...
    ) -> agent_tui_tools::implementations::grok_build::task::types::SubagentSpawnRequest {
        use agent_tui_tools::implementations::grok_build::task::types::SubagentEvent;
    ...
    fn complete_spawn(
        req: agent_tui_tools::implementations::grok_build::task::types::SubagentSpawnRequest,
    ) {
        use agent_tui_tools::implementations::grok_build::task::types::SubagentResult;
        let id = req.id.clone();
    ...
    async fn workflow_spawns_await_to_completion() {
        ...
        assert_eq!(
            ...
            agent_tui_tools::implementations::grok_build::task::types::ModelOverrideProvenance::Tool,
