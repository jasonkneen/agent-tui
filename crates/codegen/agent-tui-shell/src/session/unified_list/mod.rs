...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source_workspace_dir: Some("/Users/me/agent-tui-src".into()),
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = agent_tui_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = agent_tui_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = agent_tui_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        let _chat_off = agent_tui_test_support::EnvGuard::unset(GROK_CHAT_MODE_ENV);
        let _desktop_off =
            agent_tui_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                agent_tui_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = agent_tui_test_support::EnvGuard::set(GROK_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        {
            let _on = agent_tui_test_support::EnvGuard::set(GROK_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = agent_tui_workspace::session::git::normalize_repo_url(this_repo).unwrap();
        ...
        let rows = relax_rows(
            ...
            vec![
                ...
                summary("theirs", Some("git@github.com:agent-tui-org/other.git"), 9),
            ],
            30,
            facet_registry(),
        )
