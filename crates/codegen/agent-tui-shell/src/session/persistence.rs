...
pub enum PersistenceMsg {
    ...
    AppendCwdSwitchAndAck {
        ...
        respond_to: tokio::sync::oneshot::Sender<
            Result<agent_tui_chat_state::StrictAppendAck, agent_tui_chat_state::StrictAppendError>,
        >,
    },
...
pub use agent_tui_shared::session::session_dir;
...
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = agent_tui_config::grok_home().join("logs").join("mcp");
...
pub(crate) fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "agent_tui_shell::session::persistence",
