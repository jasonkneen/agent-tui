...
//! both matched by the agent ([`agent_tui_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use agent_tui_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use agent_tui_telemetry::events::ClientHookGateOutcome;
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            agent_tui_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == agent_tui_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                agent_tui_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    agent_tui_telemetry::session_ctx::log_event(
                        agent_tui_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> agent_tui_hooks::dispatcher::StopDispatchResult {
        use agent_tui_hooks::result::HookRunResult;

        let mut out = agent_tui_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = agent_tui_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                agent_tui_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use agent_tui_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    agent_tui_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
