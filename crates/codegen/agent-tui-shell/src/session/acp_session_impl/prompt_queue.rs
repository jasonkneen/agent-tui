...
impl SessionActor {
    ...
    /// [`agent_tui_prompt_queue::combine_prefix_len`]. `skip_ids` holds rows under
    ...
    pub(super) fn combine_front_pending_inputs(
        ...
        use agent_tui_prompt_queue::{CombineGate, combine_prefix_len};
    ...
    fn combine_gate(item: &InputItem) -> agent_tui_prompt_queue::CombineGate<'_> {
        ...
        let text = item
            ...
            .map(|m| m.text.as_str())
            .unwrap_or("");
        agent_tui_prompt_queue::CombineGate {
    ...
    fn joined_text_blocks(blocks: &[acp::ContentBlock]) -> String {
        use agent_tui_prompt_queue::join_texts;
    ...
    fn append_text_to_prompt(item: &mut InputItem, extra: &str) {
        use agent_tui_prompt_queue::TEXT_SEPARATOR;
    ...
    fn stamp_combined_display_texts_meta(item: &mut InputItem) {
        use agent_tui_prompt_queue::stamp_combined_display_texts;
