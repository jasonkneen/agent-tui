...
//!
//! Cases run in a re-exec'd child (the `agent-tui-gix-status` pattern) so
...
use agent_tui_tty_utils::runtime::MAX_BLOCKING_THREADS;
...
fn thread_count() -> Option<u64> {
    agent_tui_tty_utils::sample_process_resources().threads
}

/// Child: session runtime must not 16-wide pre-warm (cap proof lives in
/// `agent-tui-tty-utils`). `spawn_blocking` must still run.
