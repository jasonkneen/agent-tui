...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &agent_tui_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                agent_tui_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                agent_tui_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        agent_tui_telemetry::session_ctx::log_event(
            agent_tui_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: agent_tui_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            agent_tui_telemetry::session_ctx::log_event(
                agent_tui_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
        ...
        tracing::info!(target: agent_tui_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        agent_tui_telemetry::session_ctx::log_event(
            agent_tui_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => agent_tui_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => agent_tui_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => agent_tui_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => agent_tui_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::MemoryFlushed {
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    agent_tui_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                agent_tui_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = agent_tui_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = agent_tui_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                agent_tui_sampler::collect_response(events).await
            }
        };
