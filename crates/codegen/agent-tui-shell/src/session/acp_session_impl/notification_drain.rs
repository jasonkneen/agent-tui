...
impl SessionActor {
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in agent_tui_tools::implementations::grok_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
            Self::push_pending_notification(
    ...
    fn notification_blocks(
        ...
        use agent_tui_tools::implementations::grok_build::monitor::types::MonitorEventNotification;
        ...
        if let (Some(index), Some(batch)) = (
            monitor_section_idx,
            agent_tui_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,
