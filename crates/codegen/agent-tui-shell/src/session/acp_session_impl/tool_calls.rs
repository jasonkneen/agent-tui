    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if !plan_file_auto_approve {
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (agent_tui_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    agent_tui_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    agent_tui_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    agent_tui_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            agent_tui_telemetry::session_ctx::log_event(
                agent_tui_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
