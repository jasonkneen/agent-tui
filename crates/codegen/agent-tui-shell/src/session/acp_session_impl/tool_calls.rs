...
fn record_tool_span_outcome(
    span: tracing::Span,
    result: &Result<ToolRunResult, agent_tui_tool_runtime::ToolError>,
) -> bool {
...
pub(super) fn is_file_backed_exit_plan_kind(
    kind: Option<agent_tui_tools::types::tool::ToolKind>,
) -> bool {
    matches!(kind, Some(agent_tui_tools::types::tool::ToolKind::ExitPlan))
}
...
fn split_exit_plan_tail(
    ...
    kind_of: impl Fn(&str) -> Option<agent_tui_tools::types::tool::ToolKind>,
) -> (
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        agent_tui_tools::normalization::merge_tool_meta(
            &toolset,
        ...
        while let Some((idx, mut result, mut duration_ms)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => agent_tui_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && agent_tui_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(agent_tui_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(agent_tui_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            agent_tui_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            agent_tui_hooks::event::HookEventName::PostToolUseFailure,
                            agent_tui_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    agent_tui_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    agent_tui_hooks::event::HookEventName::PostToolUse,
                    agent_tui_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                ...
                .await;
            let (ext_file_path, ext_parameters) = if agent_tui_telemetry::external::is_active() {
            ...
            agent_tui_telemetry::session_ctx::log_event(
                agent_tui_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(agent_tui_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                agent_tui_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                agent_tui_hooks::event::HookEventName::PreToolUse,
                None,
                agent_tui_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    agent_tui_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let agent_tui_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                agent_tui_workspace::permission::AccessKind::Read(p) => (
                    agent_tui_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                agent_tui_workspace::permission::AccessKind::Edit(p) => {
                    (agent_tui_telemetry::events::AccessKind::Edit, p.clone())
                }
                agent_tui_workspace::permission::AccessKind::Bash(cmd) => {
                    (agent_tui_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                agent_tui_workspace::permission::AccessKind::Grep { path, glob } => (
                    agent_tui_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                agent_tui_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (agent_tui_telemetry::events::AccessKind::Mcp, name.clone())
                }
                agent_tui_workspace::permission::AccessKind::WebFetch(u) => {
                    (agent_tui_telemetry::events::AccessKind::Web, u.clone())
                }
                agent_tui_workspace::permission::AccessKind::WebSearch(q) => {
                    (agent_tui_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            agent_tui_telemetry::session_ctx::log_event(
                agent_tui_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let path_context = Some(agent_tui_workspace::permission::types::RequestPathContext {
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        agent_tui_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        agent_tui_hooks::event::HookEventName::PermissionDenied,
                        agent_tui_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    pub(super) async fn request_plan_approval(
        ...
        agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use agent_tui_tools::implementations::grok_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                agent_tui_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
    ...
    async fn send_tool_call_start(
        ...
        let (title, kind, locations, content) = match tool_call_input {
                ...
                (
                    ...
                    vec![
                        acp::ToolCallLocation::new(read_file.path)
                            ...
                            .line(
                                agent_tui_tools::normalization::norm_offset_i64(read_file.offset)
                                    .map(|l| l as u32),
                            ),
                    ],
                    Vec::new(),
            ...
            ToolInput::Skill(skill) => {
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::SkillDispatched {
                        skill_name: skill.skill.clone(),
                        plugin_source: None,
                        trigger: agent_tui_telemetry::events::SkillTrigger::SkillTool,
                    },
                );
                tracing::info_span!(
                    "skill.activated",
    ...
    async fn skill_for_read_path(
        &self,
        path: &str,
    ) -> Option<agent_tui_tools::implementations::skills::types::SkillInfo> {
        if self.is_chat_kind {
            return None;
        }
        let read_path = agent_tui_tools::types::resources::resolve_model_path(
            self.tool_context.cwd.as_path(),
    ...
    fn emit_skill_md_read(&self, skill: agent_tui_tools::implementations::skills::types::SkillInfo) {
        ...
        .in_scope(|| {});
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::SkillDispatched {
            skill_name: skill.name,
            plugin_source: skill.plugin_name,
            trigger: agent_tui_telemetry::events::SkillTrigger::SkillMdRead,
        });
    }
    ...
    /// `agent_tui_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use agent_tui_telemetry::enums::PrCreationSource;
        use agent_tui_tools::util::git_detect;
        match &result.output {
            agent_tui_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    agent_tui_telemetry::session_ctx::log_event(
                        agent_tui_telemetry::events::PrMerged {},
                    );
                }
            }
            agent_tui_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: agent_tui_tools::util::git_detect::PrRef,
        source: agent_tui_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            agent_tui_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                agent_tui_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let agent_tui_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                agent_tui_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | agent_tui_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            agent_tui_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            agent_tui_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`agent_tui_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: agent_tui_sampler::SamplingEvent,
    ) {
        use agent_tui_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == agent_tui_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                agent_tui_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = agent_tui_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod exit_plan_tail_predicate_tests {
    ...
    use agent_tui_tools::types::ToolInput;
    use agent_tui_tools::types::tool::ToolKind;
    ...
    fn exit_plan_kind_is_file_backed_exit() {
        ...
        assert!(is_file_backed_exit_plan_input(&ToolInput::ExitPlanMode(
            agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeInput {}
        )));
    }
...
mod plan_mode_edit_gate_tests {
    ...
    use agent_tui_tools::types::ToolInput;
    use agent_tui_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use agent_tui_tools::implementations::grok_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use agent_tui_tools::implementations::opencode::write::WriteInput;
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use agent_tui_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use agent_tui_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeExtResponse;
