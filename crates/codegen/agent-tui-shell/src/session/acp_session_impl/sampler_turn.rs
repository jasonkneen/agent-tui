...
/// `SamplingError::is_auth_error` in agent-tui-sampling-types: 403 is
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<agent_tui_sampling_types::HostedTool>,
        agent_tui_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = agent_tui_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<agent_tui_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }
    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<agent_tui_sampling_types::HostedTool> {
    ...
    pub(crate) fn effective_tool_overrides(
        &self,
    ) -> Option<agent_tui_sampling_types::ToolOverrides> {
        if !self.backend_search_active() {
            return None;
        }
    ...
    /// inherits it.
    pub(crate) fn set_tool_overrides(&self, overrides: agent_tui_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<agent_tui_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| agent_tui_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<agent_tui_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, agent_tui_workspace::permission::ClassifierFailure>,
            >,
        )>();
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    let (sampling_client, model) = match &aux_classifier_sampler {
                        ...
                        None => {
                            let client = session
                                ...
                                .await
                                .map_err(|e| agent_tui_workspace::permission::ClassifierFailure::TransportError(
                                    e.to_string(),
                                ))?;
                            let model = session
                                .chat_state_handle
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            agent_tui_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            agent_tui_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            agent_tui_workspace::permission::classifier_output_json_schema(),
                        ),
                        ...
                        x_grok_req_id: Some(
                            format!("agent-tui-perm-auto-{}", uuid::Uuid::new_v4()),
                        ),
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| {
                            agent_tui_workspace::permission::ClassifierFailure::Timeout
                        })?
                        .map_err(|e| agent_tui_workspace::permission::ClassifierFailure::TransportError(
                            e.to_string(),
                        ))?;
        ...
        let clf =
            agent_tui_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<agent_tui_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(agent_tui_sampler::SamplingClient, String)> {
        ...
        let client = agent_tui_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<agent_tui_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            agent_tui_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `agent-tui-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: agent_tui_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use agent_tui_sampler::SamplingErrorKind;
        if self.tool_context.task_output_token_budget.is_some() {
        ...
        let client_version = agent_tui_version::VERSION;
        ...
        let error_type = if agent_tui_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = agent_tui_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = agent_tui_sampler::SamplingErrorInfo::from(&rich_err);
...
fn resolve_configured_cutoff(
    seed: Option<agent_tui_sampling_types::ToolOverrides>,
    base: Option<&agent_tui_sampling_types::ToolOverrides>,
) -> agent_tui_sampling_types::ToolOverrides {
    use agent_tui_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod configured_cutoff_tests {
    ...
    fn inherited_cutoff_agrees_with_the_wire_echo() {
        use agent_tui_sampling_types::{HostedTool, apply_tool_overrides};
