...
use agent_tui_tools::implementations::grok_build::LoopFireMode;
...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &agent_tui_tools::implementations::grok_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = agent_tui_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        agent_tui_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    agent_tui_telemetry::session_ctx::log_event(
                        agent_tui_telemetry::events::SlashCommandUsed {
                    ...
                    agent_tui_telemetry::session_ctx::log_event(
                        agent_tui_telemetry::events::SkillDispatched {
                            ...
                            trigger: agent_tui_telemetry::events::SkillTrigger::SlashCommand,
                        },
                    );
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        agent_tui_telemetry::session_ctx::log_event(
                            agent_tui_telemetry::events::PluginUsed {
        ...
        agent_tui_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        agent_tui_telemetry::session_ctx::begin_prompt_id();
        let origin = super::super::PromptOrigin::from_prompt_id(prompt_id);
        let mut chunk_meta = serde_json::Map::new();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let query = if send_now && !verbatim {
            format!("{}\n{query}", agent_tui_interjection_core::INTERJECTION_NOTE)
        } else {
            query
        };
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = agent_tui_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || agent_tui_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = agent_tui_telemetry::events::PromptSubmitted {
            ...
            agent_tui_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(agent_tui_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            agent_tui_hooks::event::HookEventName::UserPromptSubmit,
            agent_tui_hooks::event::HookPayload::UserPromptSubmit {
        ...
        match &result {
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    ...
                    self.dispatch_hook(
                        agent_tui_hooks::event::HookEventName::StopFailure,
                        agent_tui_hooks::event::HookPayload::StopFailure {
                            error: agent_tui_hooks::event::StopFailureKind::InvalidRequest,
                ...
                .await;
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::TurnCompleted {
                        outcome: agent_tui_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::TurnCompleted {
                        outcome: agent_tui_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                agent_tui_telemetry::session_ctx::log_session_event(
                    agent_tui_telemetry::events::ApiError {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::TurnCompleted {
                        outcome: agent_tui_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = agent_tui_tools::implementations::grok_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = agent_tui_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = agent_tui_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use agent_tui_tools::types::memory_backend::MemoryBackend as _;
        ...
        agent_tui_telemetry::session_ctx::log_event(
            agent_tui_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<agent_tui_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[agent_tui_sampling_types::conversation::ToolCall],
    ) -> bool {
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            if identical_tool_calls.run_len >= identical_tool_calls.hard_stop_threshold() {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::ActionStationarityStop {
                        true_noop,
                        run_len,
            ...
            request.x_grok_agent_id = Some(agent_tui_telemetry::id::agent_id());
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(agent_tui_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    agent_tui_sampling_types::ConversationItem::Assistant(_) => {
            ...
            if is_true_noop {
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::ShellTrueNoop {
