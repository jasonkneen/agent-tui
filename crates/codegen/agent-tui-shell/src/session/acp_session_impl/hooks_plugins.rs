...
impl SessionActor {
    ...
    pub(super) fn do_hooks_trust_project(cwd: &str) -> Result<std::path::PathBuf, String> {
        let root =
            agent_tui_workspace::session::git::find_git_root_from_path(std::path::Path::new(cwd))
                .map_err(|_| {
    ...
    pub(super) async fn reseed_mcp_output_cap(&self) {
        ...
        let existing = resources
            .get::<agent_tui_tools::types::resources::TruncationCfg>()
            .map(|c| c.0.clone());
        match (resolved, existing) {
            (resolved, Some(mut cfg)) => {
                if cfg.mcp_max_output_bytes != resolved {
                    cfg.mcp_max_output_bytes = resolved;
                    resources.insert(agent_tui_tools::types::resources::TruncationCfg(cfg));
                }
            }
            (Some(v), None) => {
                resources.insert(agent_tui_tools::types::resources::TruncationCfg(
    ...
    pub(super) async fn handle_hooks_action(
        ...
        match action {
            ...
            HooksAction::Disable { hook_name } => {
                match agent_tui_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::Enable { hook_name } => {
                match agent_tui_hooks::trust::enable_hook(&hook_name) {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        agent_tui_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        agent_tui_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        match action {
            ...
            PluginsAction::Install { source } => {
                ...
                let install_source =
                    agent_tui_agent::plugins::git_install::parse_install_source(&source, cwd);
                let registry = agent_tui_agent::plugins::InstallRegistry::load();
                match agent_tui_agent::plugins::git_install::install_from_source(
                    &install_source,
                    &registry,
                    crate::plugin::marketplace_require_sha(),
                ) {
                    Ok(result) => {
                        let repo = agent_tui_agent::plugins::git_install::build_installed_repo(
                            &result,
                            &install_source,
                        );
            ...
            PluginsAction::Uninstall {
                ...
                let mut registry = agent_tui_agent::plugins::InstallRegistry::load();
                match registry.find_plugin(plugin_name) {
                    ...
                    Some((repo_key, repo, _plugin)) => {
                        ...
                        if let Err(e) =
                            agent_tui_agent::plugins::git_install::remove_repo_path(&repo_path)
                        {
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                for (key, repo) in &repos_to_update {
                    match agent_tui_agent::plugins::git_install::update_repo(
                        key,
                        repo,
                        crate::plugin::marketplace_require_sha(),
                    ) {
                        Ok(status) => {
                            use agent_tui_agent::plugins::git_install::UpdateStatus;
                            match status {
        ...
        if let Some(ref pr) = plugin_registry_snapshot {
            for plugin in pr.active_plugins() {
                if let Some(ref hooks_path) = plugin.hooks_path {
                    let (specs, warnings) =
                        agent_tui_agent::plugins::hooks_adapter::parse_plugin_hooks(
                            hooks_path,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
                ...
                if let Some(ref inline_value) = plugin.inline_hooks {
                    let (specs, warnings) =
                        agent_tui_agent::plugins::hooks_adapter::parse_plugin_hooks_from_value(
                            inline_value,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
    ...
    pub(super) async fn reload_plugins_impl(
        self: &Arc<Self>,
        handle: &agent_tui_agent::plugins::SharedPluginRegistryHandle,
        force: bool,
    ) -> String {
        let session_cwd = std::path::Path::new(&self.session_info.cwd);

        let sid = self.session_info.id.0.as_ref();
        agent_tui_telemetry::unified_log::info("reload_plugins_impl: start", Some(sid), None);
    ...
    pub(crate) fn preserve_session_plugin_dirs(
        &self,
        incoming: Option<std::sync::Arc<agent_tui_agent::plugins::PluginRegistry>>,
    ...
    pub(super) async fn apply_plugin_registry_snapshot(
        self: &Arc<Self>,
        new_registry_snapshot: Option<std::sync::Arc<agent_tui_agent::plugins::PluginRegistry>>,
    ) -> (usize, bool, usize) {
        let sid = self.session_info.id.0.as_ref();
        let session_cwd = std::path::Path::new(&self.session_info.cwd);
        ...
        *self.plugin_registry.borrow_mut() = new_registry_snapshot.clone();
        ...
        if let Some(ref new_registry) = new_registry_snapshot {
            ...
            hooks_reloaded = new_specs.len();
            {
                ...
                if let Some(ref mut arc_reg) = *reg {
                    ...
                    let git_root =
                        agent_tui_workspace::session::git::find_git_root_from_path(session_cwd).ok();
                    let is_trusted =
                        crate::agent::folder_trust::resolve_and_record(session_cwd, None, false);
        ...
        let mcp_changed = if let Some(diff) = mcp_diff {
            ...
            {
                let _ = tx.send(agent_tui_mcp::servers::McpClientEvent::ConfigDiff {
