...
use agent_tui_sampling_types::SamplingError;
...
fn build_side_question_attempt(base: &ConversationRequest) -> ConversationRequest {
    ...
    request.x_grok_req_id = Some(format!("agent-tui-btw-{}", uuid::Uuid::new_v4()));
    request
}
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(
        ...
        let mut items: Vec<ConversationItem> =
            if sampling_client.api_backend().requires_reasoning_strip() {
                agent_tui_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
            } else {
                conversation
            };
        ...
        let base_request = self.parent_cached_request(AuxCall {
            ...
            req_id: format!("agent-tui-btw-{}", uuid::Uuid::new_v4()),
        });
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        tracing::debug!(
            raw_preview = %agent_tui_tools::util::truncate_str(raw.trim(), 60),
...
mod tests {
    ...
    fn side_question_attempts_get_fresh_request_ids() {
        ...
        assert!(a_id.starts_with("agent-tui-btw-"));
