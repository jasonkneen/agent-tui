...
impl SessionActor {
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    agent_tui_hooks::event::HookEventName::SubagentStart,
                    None,
                    agent_tui_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = agent_tui_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        agent_tui_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            ...
            None,
            agent_tui_hooks::event::HookPayload::Notification {
    ...
    pub(super) fn response_completed_update(
        &self,
        response: &agent_tui_sampling_types::ConversationResponse,
    ) -> XaiSessionUpdate {
        let usage =
            response
                .usage
