...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch, commit) = tokio::join!(
            agent_tui_workspace::session::git::get_worktree_info(cwd),
            agent_tui_workspace::session::git::get_branch(cwd),
            agent_tui_workspace::session::git::get_current_commit(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<agent_tui_tools::implementations::grok_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use agent_tui_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> agent_tui_hooks::event::StopFailureKind {
        use agent_tui_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &agent_tui_agent::plugins::install_registry::InstallError,
    ) -> String {
