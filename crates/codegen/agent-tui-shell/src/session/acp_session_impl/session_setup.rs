...
impl SessionActor {
    ...
    ) -> Option<agent_tui_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(crate) async fn slash_skills_for_resolve(
        &self,
    ) -> Vec<agent_tui_tools::implementations::skills::types::SkillInfo> {
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &agent_tui_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use agent_tui_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn agent_tui_auth::AuthCredentialProvider> = Arc::new(
        ...
        let (response, stamp) =
            match agent_tui_auth::execute_with_stamp(&middleware_client, built).await {
                Ok(r) => r,
                Err(e) => {
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = agent_tui_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(agent_tui_tools::types::resources::DenyReadGlobs(
    ...
    pub(super) async fn build_session_info(&self) -> SessionInfoData {
        ...
        let system_prompt_tokens = system_message
            .as_ref()
            .map(agent_tui_chat_state::estimate_system_message_tokens)
            .unwrap_or(0);
        ...
        let tool_definitions_tokens = agent_tui_chat_state::estimate_tool_definitions_tokens(&tool_defs);
        ...
        let free_tokens = agent_tui_token_estimation::free_tokens(context_window, total_tokens);
        let usage_pct = agent_tui_token_estimation::usage_percentage_u8(total_tokens, context_window);
