...
impl SessionActor {
            ...
            Err(e) => {
                ...
                if terminal {
                    ...
                    crate::session::telemetry::emit_mcp_connection_span(
                        ...
                        Some(elapsed_ms),
                        None,
                        Some(agent_tui_telemetry::events::McpErrorType::Auth.as_str()),
                    );
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(agent_tui_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(agent_tui_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        tokio::task::spawn_local(async move {
            ...
            > = std::collections::HashMap::new();
            {
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            let transport_enum = match server_transport_map
                                ...
                                .unwrap_or("unknown")
                            {
                                "stdio" => agent_tui_telemetry::events::McpTransport::Stdio,
                                "sse" => agent_tui_telemetry::events::McpTransport::Sse,
                                _ => agent_tui_telemetry::events::McpTransport::Http,
                            };
                            agent_tui_telemetry::session_ctx::log_event(
                                agent_tui_telemetry::events::McpServerConnected {
                                    server_name: server_name.clone(),
                                    tool_count,
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let detail = (!needs_auth).then(|| {
                                agent_tui_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::events::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<agent_tui_tools::types::tool_index::ServerSummary> {
        use agent_tui_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            agent_tui_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
