...
fn drop_cli_catchall_allows(
    rules: Vec<agent_tui_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<agent_tui_workspace::permission::types::PermissionRule>,
    Vec<agent_tui_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if agent_tui_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
/// process-lifetime (`agent_tui_tty_utils::runtime`).
pub(crate) fn build_session_runtime() -> std::io::Result<tokio::runtime::Runtime> {
    let mut builder = tokio::runtime::Builder::new_current_thread();
    agent_tui_tty_utils::runtime::apply_blocking_pool(builder.enable_all()).build()
}
#[cfg(all(test, unix))]
...
mod cli_catchall_drop_tests {
    ...
    use agent_tui_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use agent_tui_workspace::permission::rules::parse_permission_rule;
    use agent_tui_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    preloaded_skills: Option<Vec<agent_tui_tools::implementations::skills::types::SkillInfo>>,
    compat: CompatConfig,
    ...
    web_search_sampling_config: Option<agent_tui_sampler::SamplerConfig>,
    web_fetch_config: agent_tui_tools::implementations::grok_build::web_fetch::WebFetchConfig,
    image_gen_config: agent_tui_tools::implementations::grok_build::image_gen::ImageGenConfig,
    video_gen_config: agent_tui_tools::implementations::grok_build::video_gen::VideoGenConfig,
    app_builder_deployer_config: agent_tui_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: agent_tui_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<agent_tui_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<agent_tui_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<agent_tui_workspace::permission::PermissionHandle>,
    api_key_provider: Option<agent_tui_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<agent_tui_hooks::discovery::HookRegistry>>,
    workspace_ops: agent_tui_workspace::WorkspaceOps,
    cli_permission_rules: Vec<agent_tui_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn agent_tui_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        agent_tui_tools::implementations::grok_build::scheduler::types::SchedulerHandle,
    >,
    ...
    agent_tui_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(agent_tui_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            agent_tui_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
                project_trusted,
            )
            .await;
        let yolo_pin = agent_tui_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        agent_tui_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(agent_tui_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if agent_tui_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    agent_tui_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn agent_tui_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            agent_tui_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        agent_tui_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            agent_tui_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let chat_state_sampling_config = agent_tui_sampling_types::SamplingConfig {
    ...
    let task_completion_reservations =
        agent_tui_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        agent_tui_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        agent_tui_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        agent_tui_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn agent_tui_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn agent_tui_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn agent_tui_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(agent_tui_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(agent_tui_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = agent_tui_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        agent_tui_tools::implementations::grok_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn agent_tui_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target: agent_tui_telemetry::memory_log::TARGET,
                error = %e,
        ...
        let backend: std::sync::Arc<dyn agent_tui_tools::types::memory_backend::MemoryBackend> =
        ...
        agent_tui_telemetry::session_ctx::log_event(
            agent_tui_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        agent_tui_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    agent
        ...
        .await;
    let harness_metrics = if telemetry_enabled || agent_tui_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                agent_tui_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                agent_tui_telemetry::enums::PermissionMode::Auto
            } else {
                agent_tui_telemetry::enums::PermissionMode::Ask
            },
    ...
    let resolved_task_output =
        agent_tui_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        agent_tui_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    let scheduler_handle_for_handle = {
        ...
        res.get::<agent_tui_tools::implementations::grok_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let resolved_workspace_root = agent_tui_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = agent_tui_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<agent_tui_sampler::SamplingEvent>();
    let sampler_handle = agent_tui_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<agent_tui_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<agent_tui_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = agent_tui_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        agent_tui_tools::implementations::grok_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use agent_tui_tools::implementations::grok_build::workflow::WorkflowLaunchAck;
            while let Some((req, ack)) = workflow_launch_rx.recv().await {
                ...
                if input.validate_only {
                    ...
                    let agent_budget = input
                        .agent_budget
                        .unwrap_or(agent_tui_workflow::DEFAULT_AGENT_BUDGET);
                    tokio::spawn(async move {
                        let verdict = tokio::task::spawn_blocking(move || {
                            agent_tui_workflow::validate_script_with_agent_budget(
                                &script,
                                probe_args,
                                agent_budget,
                            )
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<agent_tui_sampling_types::ToolOverrides>,
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: agent_tui_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match agent_tui_workspace::session::git::discover_git_root(root) {
                agent_tui_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    agent_tui_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => agent_tui_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(agent_tui_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(agent_tui_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            agent_tui_tools::implementations::grok_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                agent_tui_tools::implementations::grok_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                agent_tui_telemetry::session_ctx::log_event(
                    agent_tui_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use agent_tui_tools::implementations::grok_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = agent_tui_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            agent_tui_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        agent_tui_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, agent_tui_agent::AgentBuildError>,
    >();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(agent_tui_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    agent_tui_sampling_types::conversation::ContentPart::Text { text }
...
mod resumed_prefix_fallback_tests {
    ...
    use agent_tui_sampling_types::conversation::ConversationItem;
    #[test]
