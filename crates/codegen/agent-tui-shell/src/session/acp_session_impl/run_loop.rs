...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<agent_tui_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            agent_tui_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : agent_tui_telemetry::memory_log::TARGET,
            ...
            agent_tui_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            .fire_hook(agent_tui_hooks::event::HookEventName::SessionEnd, None,
            agent_tui_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            agent_tui_hooks::dispatcher::dispatch_non_blocking(& registry,
            agent_tui_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(agent_tui_hooks::event::HookEventName::Stop,
            None, agent_tui_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            agent_tui_hooks::dispatcher::dispatch_non_blocking(& registry,
            agent_tui_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            agent_tui_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::SessionEnded
            ...
            .push_capped(agent_tui_tools::implementations::grok_build::task::types::MonitorEventNotification
            ...
            agent_tui_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(agent_tui_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            session.fire_hook(agent_tui_hooks::event::HookEventName::SessionStart, None,
            agent_tui_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            agent_tui_hooks::dispatcher::dispatch_non_blocking(& registry,
            agent_tui_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(agent_tui_hooks::event::HookEventName::SessionEnd, None,
            agent_tui_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            agent_tui_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(agent_tui_hooks::event::HookEventName::Stop,
            None, agent_tui_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            agent_tui_hooks::dispatcher::dispatch_non_blocking(& registry,
            agent_tui_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[agent_tui_sampling_types::ConversationItem],
    turn_number: usize,
