...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> agent_tui_hooks::runner::RunContext<'_> {
        agent_tui_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[agent_tui_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, agent_tui_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: agent_tui_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            agent_tui_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                agent_tui_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                ...
                agent_tui_hooks::result::HookRunResult::Failed {
                    ...
                    hook_name,
                    elapsed,
                    agent_tui_telemetry::events::HookOutcome::Error,
                ),
                agent_tui_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: agent_tui_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
