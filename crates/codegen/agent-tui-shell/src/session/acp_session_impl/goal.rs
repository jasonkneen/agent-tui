...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &agent_tui_tools::implementations::grok_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    fn record_goal_round_progress(&self, detail: &str, failed: bool) {
        let detail = agent_tui_tools::util::truncate_str(detail.trim(), 500).to_owned();
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        if agent_tui_agent::config::is_strict_harness_agent_type(&pair.agent_type)
