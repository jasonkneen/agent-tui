...
pub(crate) const LAZINESS_REQ_ID_PREFIX: &str = "agent-tui-laziness-";
        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let agent_tui_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = agent_tui_sampling_types::reasoning_item_text(r);
...
const CLASSIFIER_TURN_MAX_LEN: usize = agent_tui_workspace::permission::CLASSIFIER_TURN_MAX_LEN;
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<agent_tui_workspace::permission::ClassifierTurn> {
    use agent_tui_workspace::permission::ClassifierTurn;
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text = agent_tui_tools::util::truncate_str_with_marker(
                        &text,
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args = agent_tui_tools::util::truncate_str_with_marker(
                        &args,
                        CLASSIFIER_TURN_MAX_LEN,
                    )
                    .into_owned();
