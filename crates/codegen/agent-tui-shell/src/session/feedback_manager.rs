...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(agent_tui_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if telemetry_enabled {
        ...
        agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::UserFeedback {
...
impl FeedbackManager {
    ...
    pub(crate) fn set_upload_queue_stats(
        &self,
        stats: Arc<agent_tui_file_utils::queue::UploadQueueStats>,
    ) {
...
mod tests {
    ...
    fn test_shutdown_budgets_fit_under_session_flush_grace() {
        ...
        let _unset = agent_tui_test_support::env::EnvGuard::unset("GROK_SESSION_EXIT_DRAIN_SECS");
        assert!(
    ...
    fn test_nonempty_drain_budget_env_raises_cap() {
        {
            let _guard =
                agent_tui_test_support::env::EnvGuard::set("GROK_SESSION_EXIT_DRAIN_SECS", "7");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
        ...
        {
            let _guard =
                agent_tui_test_support::env::EnvGuard::set("GROK_SESSION_EXIT_DRAIN_SECS", "99");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
...
mod author_identity_tests {
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            agent_tui_test_support::env::EnvGuard::set("GROK_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            agent_tui_test_support::env::EnvGuard::set("GROK_TEST_WORK_NAME", "Ada Lovelace");
...
"#,
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = agent_tui_test_support::env::EnvGuard::set(
            "GROK_USER_METADATA",
