...
fn install_client_hook(
    actor: &SessionActor,
    event: agent_tui_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
fn spawn_deny_responder(
    gateway_rx: tokio::sync::mpsc::UnboundedReceiver<agent_tui_acp_lib::AcpClientMessage>,
    reason: &'static str,
) {
    ...
    tokio::task::spawn_local(async move {
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
                agent_tui_acp_lib::AcpClientMessage::ExtMethod(args) => {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                agent_tui_tools::registry::types::ToolConfig::for_tool::<
                    agent_tui_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        agent_tui_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
fn file_registry_with_stop_spec(
    ...
    let (mut registry, _) = agent_tui_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![agent_tui_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() =
                Some(std::sync::Arc::new(file_registry_with_stop_spec(
                    agent_tui_hooks::event::HookEventName::SubagentStop,
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                agent_tui_hooks::event::HookEventName::SubagentEnd,
                None,
                agent_tui_hooks::event::HookPayload::SubagentStop {
                    phase: agent_tui_hooks::event::SubagentStopPhase::Observe,
