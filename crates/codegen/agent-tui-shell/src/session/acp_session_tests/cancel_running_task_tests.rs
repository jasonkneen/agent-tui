        ...
        .run_until(async {
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(
                        agent_tui_sampling_types::UserItem {
                            content: vec![agent_tui_sampling_types::ContentPart::Text {
                                text: "hi".into(),
                            }],
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    agent_tui_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
