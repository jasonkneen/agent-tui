...
/// attribution.
fn auth_error() -> agent_tui_sampler::SamplingErrorInfo {
    agent_tui_sampler::SamplingErrorInfo {
        kind: agent_tui_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: agent_tui_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        agent_tui_chat_state::AuthType::SessionToken => "cached_token",
        agent_tui_chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
fn model_not_found_error() -> agent_tui_sampler::SamplingErrorInfo {
    agent_tui_sampler::SamplingErrorInfo {
...
fn unauthorized_401_error() -> agent_tui_sampler::SamplingErrorInfo {
    agent_tui_sampler::SamplingErrorInfo {
            kind: agent_tui_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=agent-tui-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
