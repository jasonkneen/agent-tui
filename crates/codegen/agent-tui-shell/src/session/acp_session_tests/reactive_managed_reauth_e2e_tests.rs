...
//! handshake against rmcp 2.1 in `agent-tui-mcp/tests/repro_sse_flood.rs`): a
...
use agent_tui_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use agent_tui_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};
