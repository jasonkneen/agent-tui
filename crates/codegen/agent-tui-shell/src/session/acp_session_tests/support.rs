...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> agent_tui_computer_hub_sdk::ObservabilityBridge {
    agent_tui_computer_hub_sdk::ObservabilityBridge::new(
        None,
        agent_tui_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> agent_tui_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<agent_tui_sampling_types::HostedTool>,
) -> agent_tui_agent::Agent {
    let base = test_agent_default().await;
    agent_tui_agent::Agent::new(
        base.definition().clone(),
        agent_tui_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        agent_tui_agent::ReminderPolicy::default(),
        agent_tui_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> agent_tui_agent::Agent {
    use agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalTool;
...
#[cfg(test)]
pub(crate) async fn test_grok_build_agent_with_todo() -> agent_tui_agent::Agent {
    use agent_tui_tools::implementations::grok_build::todo::TodoWriteTool;
    use agent_tui_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> agent_tui_agent::Agent {
    use agent_tui_tools::implementations::grok_build::enter_plan_mode::EnterPlanModeTool;
    use agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeTool;
    use agent_tui_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<agent_tui_tools::registry::types::ToolConfig>,
) -> agent_tui_agent::Agent {
    test_agent_from_config(
        agent_tui_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        agent_tui_agent::AgentDefinition::default_grok_build(),
        std::sync::Arc::new(agent_tui_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: agent_tui_agent::prompt::user_message::UserMessageTemplate,
) -> agent_tui_agent::Agent {
    let mut definition = agent_tui_agent::AgentDefinition::default_grok_build();
...
async fn test_agent_from_config(
    ...
    definition: agent_tui_agent::AgentDefinition,
    backend: std::sync::Arc<dyn agent_tui_tools::computer::types::TerminalBackend>,
) -> agent_tui_agent::Agent {
    use agent_tui_tools::computer::local::LocalFs;
    use agent_tui_tools::computer::types::AsyncFileSystem;
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<agent_tui_acp_lib::AcpClientMessage>,
...
pub(crate) async fn create_test_actor_with_terminal(
    ...
    let cwd = agent_tui_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(agent_tui_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(agent_tui_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(agent_tui_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: agent_tui_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
