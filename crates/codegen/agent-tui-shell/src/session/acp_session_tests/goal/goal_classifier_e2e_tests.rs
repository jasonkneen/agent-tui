...
use agent_tui_tools::implementations::grok_build::update_goal::{RejectReason, UpdateGoalInput};
...
fn seed_channel(actor: &SessionActor, cmds: Vec<UpdateGoalInput>) {
    ...
    *actor.goal_update_rx.borrow_mut() = Some(rx);
    for cmd in cmds {
        tx.send(agent_tui_tools::implementations::grok_build::update_goal::envelope_for_test(cmd))
            .unwrap();
    }
    drop(tx);
}
...
fn seed_channel_with_acks(
    ...
    tokio::sync::oneshot::Receiver<
        agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalAck,
    >,
> {
    let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
    *actor.goal_update_rx.borrow_mut() = Some(rx);
...
async fn goal_classifier_stall_early_exit_pauses_with_no_progress() {
    ...
    local
        .run_until(async {
            ...
            let (actor, tmp) = make_actor(Some(coord.tx.clone()), true).await;
            use agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalAck;
...
async fn update_goal_tool_blocks_until_classifier_verdict_when_enabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalAck::ClassifierAchieved
                { .. },),
...
async fn update_goal_tool_returns_immediately_when_classifier_disabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalAck::CompletedWithoutClassifier,)
            );
        })
        .await;
}
...
async fn update_goal_tool_returns_immediately_for_blocked_reason() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalAck::Accepted
                { .. },)
            );
        })
        .await;
...
async fn resolve_role_override_toolset_incapable_fails_open() {
    ...
    local
        .run_until(async {
            let mut summary =
                agent_tui_tools::implementations::grok_build::task::types::SubagentTypeSummary {
                    can_read: true,
                    ..Default::default()
                };
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                agent_tui_sampling_types::conversation::ConversationItem::assistant(SUMMARY),
            );
            let _ = actor.run_verification_stage_for_drain(1, 3).await;
            assert_eq!(
                actor
...
async fn reverification_replays_anchor_and_does_not_overwrite_it() {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                agent_tui_sampling_types::conversation::ConversationItem::assistant(NOTE),
            );
