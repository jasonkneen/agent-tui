...
use agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalInput;
...
async fn answer_strategist(
    ...
    req: Box<agent_tui_tools::implementations::grok_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        agent_tui_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
