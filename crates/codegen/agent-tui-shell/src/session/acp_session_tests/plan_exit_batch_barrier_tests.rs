...
use agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeExtRequest;
...
async fn seeded_active_plan_actor_with_edit_tools() -> (
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
    ...
    actor
        ...
        .borrow()
        .tool_bridge()
        .update_resource(agent_tui_tools::types::resources::PlanFilePath(
            plan_path.clone(),
        ))
        .await;
...
async fn mixed_permission_cancel_skips_exit_reverse_request() {
    ...
    local
        .run_until(async {
            ...
            use agent_tui_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeTool;
            use agent_tui_tools::registry::types::ToolConfig;
            use agent_tui_workspace::permission::{ClientType, spawn_permission_manager};
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
            ...
            let responder = tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        agent_tui_acp_lib::AcpClientMessage::RequestPermission(args) => {
                            let _ = args
                                .response_tx
