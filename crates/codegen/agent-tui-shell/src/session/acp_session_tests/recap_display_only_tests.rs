...
fn main_turn_input(items: Vec<ConversationItem>) -> Vec<serde_json::Value> {
    let request = agent_tui_sampling_types::ConversationRequest {
        items,
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = agent_tui_sampling_types::ApiBackend::Responses;
            // Low is not the model default, so a fallback would show up in the assert below.
            cfg.reasoning_effort = Some(agent_tui_sampling_types::ReasoningEffort::Low);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = agent_tui_sampling_types::ApiBackend::ChatCompletions;
            actor.chat_state_handle.update_sampling_config(cfg);
            ...
            assert!(
                req_id.starts_with("agent-tui-btw-"),
...
async fn manual_recap_over_budget_trims_persisted_request_and_is_display_only() {
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est = agent_tui_chat_state::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    ...
    use agent_tui_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use agent_tui_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    let msg = agent_tui_sampling_types::build_messages_request(&req);
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use agent_tui_sampling_types::HostedTool;
    use agent_tui_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = agent_tui_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use agent_tui_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use agent_tui_test_support::MockInferenceServer;
...
async fn side_question_trims_reasoning_orphaned_by_mid_turn_truncation() {
    use agent_tui_sampling_types::conversation::{AssistantItem, ToolCall};
    use agent_tui_test_support::MockInferenceServer;
