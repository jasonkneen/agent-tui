...
use agent_tui_test_support::{MockInferenceServer, MockModelEntry};
...
fn drain_gateway(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<agent_tui_acp_lib::AcpClientMessage>,
) -> XaiUpdates {
...
async fn session_token_actor(
    ...
    let sampling_cfg = agent_tui_sampler::SamplerConfig {
        base_url: server.url(),
    ...
    let agent_tui_updates = drain_gateway(gateway_rx);
