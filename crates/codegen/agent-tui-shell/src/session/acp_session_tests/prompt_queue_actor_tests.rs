...
fn x_search_cutoff_update() -> agent_tui_sampling_types::ToolOverridesUpdate {
    agent_tui_sampling_types::ToolOverridesUpdate {
        x_search: Some(Some(agent_tui_sampling_types::XSearchOptions {
            date_bound: Some(
                agent_tui_sampling_types::SearchDateBound::new(None, Some("2024-03-15".to_string()))
                    .unwrap(),
            ),
        })),
        web_search: None,
...
async fn tool_overrides_update_applies_at_promotion_never_at_enqueue() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
            let options = agent_tui_sampling_types::XSearchOptions {
                date_bound: Some(
                    agent_tui_sampling_types::SearchDateBound::new(
                        None,
            ...
            let set_update = || agent_tui_sampling_types::ToolOverridesUpdate {
                x_search: Some(Some(options.clone())),
                web_search: None,
            };
            let expected = agent_tui_sampling_types::ToolOverrides {
            ...
            actor.apply_tool_overrides_update(Some(agent_tui_sampling_types::ToolOverridesUpdate {
                x_search: Some(None),
                web_search: None,
            }));
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_backend_search(vec![agent_tui_sampling_types::HostedTool::XSearch {
                    options: None,
                }])
                .await;
            ...
            *actor.tool_overrides.borrow_mut() = Some(expected.clone());
            ...
            assert_eq!(
                ...
                vec![agent_tui_sampling_types::HostedTool::XSearch {
...
async fn agent_rebuild_republishes_the_configured_cutoff() {
    ...
    local
        .run_until(async {
            ...
            let seed = agent_tui_sampling_types::ToolOverrides {
                x_search: Some(agent_tui_sampling_types::XSearchOptions {
                    date_bound: Some(
                        agent_tui_sampling_types::SearchDateBound::new(
                            None,
            ...
            let mut seeded = agent_tui_agent::AgentDefinition::default_grok_build();
...
async fn set_tool_overrides_publishes_the_inheritance_cell_before_any_turn() {
    ...
    local
        .run_until(async {
            ...
            let cutoff = agent_tui_sampling_types::ToolOverrides {
                x_search: Some(agent_tui_sampling_types::XSearchOptions {
                    date_bound: Some(
                        agent_tui_sampling_types::SearchDateBound::new(
                            None,
