//! Session-level fs-watch policy over [`agent_tui_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `agent_tui_fsnotify`. This module
...
use agent_tui_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> agent_tui_workspace::file_system::FileIndexDelta {
    use agent_tui_workspace::file_system::FileIndexDelta;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = agent_tui_workspace::session::git::get_branch(&self.cwd).await;
        let commit = agent_tui_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = agent_tui_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = agent_tui_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: agent_tui_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                agent_tui_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || agent_tui_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = agent_tui_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(agent_tui_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: agent_tui_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
