...
use agent_tui_tty_utils::git_command;
...
pub fn cleanup_stale_pool_worktrees(source_git_root: Option<&Path>) {
    ...
    if has_adoptable {
        ...
        REGISTRATION_CLEANUP_ONCE.call_once(move || {
            let removed = agent_tui_fast_worktree::remove_stale_worktree_registrations_under(
                &root,
                &pool_base_directory(),
            );
            tracing::info!(
...
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(agent_tui_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(agent_tui_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                agent_tui_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || agent_tui_fast_worktree::remove_worktree(&path_owned)).await;
