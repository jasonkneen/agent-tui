...
//! [`agent_tui_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`agent_tui_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`agent_tui_agent::Agent`] owns an [`agent_tui_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`agent_tui_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use agent_tui_agent::config::AgentDefinition;
use agent_tui_agent::error::AgentBuildError;
use agent_tui_agent::prompt::context::PromptAudience;
use agent_tui_agent::prompt::skills::SkillsConfig;
use agent_tui_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use agent_tui_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use agent_tui_tools::implementations::grok_build::ask_user_question::types::UserQuestionRequest;
...
use agent_tui_tools::implementations::grok_build::task::types::{SubagentEvent, TaskModelValidator};
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<agent_tui_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<agent_tui_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use agent_tui_tools::implementations::grok_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(agent_tui_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(agent_tui_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(agent_tui_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use agent_tui_tools::implementations::grok_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            agent_tui_tools::computer::local::LocalTerminalBackend::new_local(
                agent_tui_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: agent_tui_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(agent_tui_tools::types::tool::ToolKind::Task)
