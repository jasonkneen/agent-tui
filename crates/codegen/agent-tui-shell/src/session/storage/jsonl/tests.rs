...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use agent_tui_paths::RelPathBuf;
    use agent_tui_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use agent_tui_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn test_xai_session_update_round_trip() {
    ...
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let agent_tui_notification = XaiSessionNotification {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(agent_tui_notification.clone())))
        .await
        .unwrap();
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    agent_tui_sandbox::set_configured_profile("workspace");
    let expected = agent_tui_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
/// path end-to-end (loader + serde + agent_tui_sampling_types::
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let agent_tui_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    ...
    let agent_tui_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
async fn retry_after_lost_ack_converges_memory_and_disk_to_authoritative_item() {
            ...
            let result = storage
                ...
                .map_err(|error| match error {
                    crate::session::storage::AppendCwdSwitchError::NotCommitted(
                        error,
                    ) => agent_tui_chat_state::StrictAppendError::NotCommitted(error),
                    crate::session::storage::AppendCwdSwitchError::Committed {
                        acknowledgement,
                        source,
                    } => {
                        agent_tui_chat_state::StrictAppendError::Committed {
                            acknowledgement,
                            source,
                        }
                    }
    ...
    let chat = agent_tui_chat_state::ChatStateActor::spawn(
        vec![],
    ...
    assert!(matches!(
            ...
            .await,
            Err(agent_tui_chat_state::StrictAppendError::Indeterminate(_))
        ));
    ...
    assert!(matches!(
            ...
            .await
            .unwrap(),
            agent_tui_chat_state::StrictAppendAck::AlreadyPresent(item)
...
async fn acknowledged_chat_append_preserves_existing_file_bytes_and_appends_once() {
    ...
    assert!(matches!(
            adapter
                ...
                .await
                .unwrap(),
            agent_tui_chat_state::StrictAppendAck::Appended
        ));
