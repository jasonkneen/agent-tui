...
impl RelocationStorage {
    ...
    fn target_parent(&self, cwd: &str) -> PathBuf {
        self.grok_home
            .join("sessions")
            .join(agent_tui_config::encode_cwd_dirname(cwd))
    }
    ...
    fn ensure_target_parent(&self, cwd: &str) -> Result<PathBuf> {
        ...
        agent_tui_config::set_dir_owner_only(&sessions);
        ...
        agent_tui_config::set_dir_owner_only(&dir);
    ...
    fn staging_dir(&self, journal: &RelocationJournal) -> PathBuf {
        self.grok_home
            .join("sessions")
            .join(agent_tui_config::encode_cwd_dirname(&journal.target_cwd))
