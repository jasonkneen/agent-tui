...
pub enum AppendCwdSwitchError {
    NotCommitted(io::Error),
    Committed {
        acknowledgement: agent_tui_chat_state::StrictAppendAck,
        source: io::Error,
    },
}
...
pub trait StorageAdapter: Send + Sync {
    ...
    ) -> Result<agent_tui_chat_state::StrictAppendAck, AppendCwdSwitchError> {
...
mod tests {
    ...
    fn agent_tui_envelope(session_update_json: &str) -> String {
        format!(
    ...
    #[test]
    fn agent_tui_rewind_marker_yields_rewind_to() {
        let line = agent_tui_envelope(
    ...
    #[test]
    fn agent_tui_rewind_to_zero_yields_rewind_to_zero() {
        let line = agent_tui_envelope(
    ...
    #[test]
    fn agent_tui_diff_review_yields_not_user() {
        let line = agent_tui_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    fn filter_rewind_ignores_a_malformed_middle_line() {
        ...
        let rewind_to_1 = agent_tui_envelope(
            r#"{"sessionUpdate":"rewind_marker","target_prompt_index":1,"created_at":"2024-01-01"}"#,
        );
    ...
    fn from_str_unknown_xai_variant_deserializes_via_envelope() {
        ...
        let line = agent_tui_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
    ...
    fn from_str_known_xai_variant_still_works() {
        let line = agent_tui_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Xai(notif) => {
                assert_eq!(
