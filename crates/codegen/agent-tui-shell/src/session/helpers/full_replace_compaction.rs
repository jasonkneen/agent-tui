...
//! (`agent_tui_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](agent_tui_compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](agent_tui_compaction::FullReplaceError).
...
use agent_tui_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use agent_tui_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    agent_tui_telemetry::session_ctx::log_event(CompactionRetryDegraded {
