...
use agent_tui_sampler::SamplerConfig as SamplingConfig;
use agent_tui_sampling_types::truncate_bytes;
...
use agent_tui_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use agent_tui_workspace::file_system::CodebaseIndexManager;
use agent_tui_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use agent_tui_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "agent_tui_session";
#[path = "compaction.rs"]
mod compaction;
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<agent_tui_sampling_types::ToolOverridesUpdate>,
...
impl agent_tui_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
...
pub(crate) struct SessionActor {
    ...
    /// `agent-tui-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<agent_tui_sampler::SharedAttributionCallback>,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<agent_tui_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<agent_tui_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<agent_tui_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<agent_tui_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<agent_tui_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<agent_tui_agent::Agent>,
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<agent_tui_tools::implementations::grok_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, agent_tui_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<agent_tui_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) vcs_kind: agent_tui_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<agent_tui_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<agent_tui_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `agent-tui-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: agent_tui_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`agent_tui_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`agent_tui_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: agent_tui_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == agent_tui_tools::implementations::grok_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == agent_tui_tools::implementations::grok_build::workflow::WORKFLOW_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<agent_tui_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &agent_tui_agent::PromptContext) {
...
) -> Option<agent_tui_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use agent_tui_tools::types::output::{MCPOutput, ToolOutput};
    use agent_tui_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl agent_tui_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use agent_tui_tools::registry::types::ToolConfig;
    use agent_tui_tools::tool_taxonomy::TOOL_META_KEY;
    use agent_tui_workspace::permission::PermissionCommand;
                        ...
                        } = cmd
                        {
                            *captured_in_task.lock().await = Some(tool_call_update);
                            let _ = respond_to.send(
                                agent_tui_workspace::permission::PermissionResolution {
...
mod managed_gateway_tool_tests {
    ...
    use agent_tui_tools::types::output::{MCPOutput, ToolOutput};
    use agent_tui_tools::types::tool::{ToolKind, ToolNamespace};
    use agent_tui_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<agent_tui_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
