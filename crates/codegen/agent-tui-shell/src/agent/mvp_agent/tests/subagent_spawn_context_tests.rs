...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = agent_tui_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                agent_tui_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    agent_tui_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                agent_tui_workspace::permission::AccessKind::Read(Some(".env".into())),
                agent_tui_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        agent_tui_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = agent_tui_sampling_types::ToolOverrides {
        x_search: Some(agent_tui_sampling_types::XSearchOptions {
            date_bound: Some(
                agent_tui_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
...
async fn subagent_spawn_context_inherits_parent_process_scope() {
    ...
    let mut handle = make_test_handle("test-model", false, None);
    let parent_scope = agent_tui_tty_utils::ProcessScope::new();
    handle.tool_context.process_scope = Some(parent_scope.clone());
    agent.insert_resident(&sid, handle);

    // Hold an owner Arc in the parent scope so live_count == 1.
    let owner = std::sync::Arc::new(agent_tui_tty_utils::ProcessGroup::new().expect("process group"));
