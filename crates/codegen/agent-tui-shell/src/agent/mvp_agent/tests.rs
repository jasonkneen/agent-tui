...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "agent-tui-not-a-jwt");
    assert_eq!(
...
fn harness_pair(id: &str) -> Vec<agent_tui_sampling_types::conversation::ConversationItem> {
    use agent_tui_sampling_types::ToolCall;
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = agent_tui_agent::AgentDefinition::from_json(&serde_json::json!(
...
fn inherited_harness_template_skips_nonstrict_model() {
    use agent_tui_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        agent_tui_agent::config::ModelOverride::Override("test-model-123".to_string()),
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use agent_tui_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = agent_tui_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: agent_tui_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: agent_tui_tools::implementations::grok_build::image_gen::ImageGenConfig::default(),
        video_gen_config: agent_tui_tools::implementations::grok_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: agent_tui_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            agent_tui_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(agent_tui_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: agent_tui_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        ...
        hook_registry: None,
        workspace_ops: agent_tui_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use agent_tui_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use agent_tui_tools::implementations::grok_build::video_gen::{
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = agent_tui_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = agent_tui_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(agent_tui_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<agent_tui_workspace::permission::PermissionEvent>();
    agent
        .session_registry
...
impl Drop for RestoreOtelGate {
    fn drop(&mut self) {
        agent_tui_telemetry::external::mark_external_otel_settings_resolved();
    }
}
...
async fn post_auth_settings_xai_upgrades_writeback_emits_and_opens_gate() {
    ...
    let server = agent_tui_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let agent_tui_auth = GrokAuth {
        oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
        ..GrokAuth::test_default()
    };
    assert!(agent_tui_auth.is_xai_auth(), "precondition: first-party xAI auth");
    let (agent, mut rx) =
        build_agent_with_auth_and_proxy(agent_tui_auth, server.url(), AgentMode::Leader);
    assert_eq!(
        agent.storage_mode(),
        StorageMode::Local,
        "precondition: leader boots in Local storage mode"
    );
    agent_tui_telemetry::external::suppress_external_otel_until_settings();
    assert!(!agent_tui_telemetry::external::is_settings_gate_open());
    ...
    assert!(
        agent_tui_telemetry::external::is_settings_gate_open(),
...
async fn post_auth_settings_failure_resolves_gate_onto_local_policy() {
    ...
    let (agent, _rx) = build_agent_with_auth_and_proxy(agent_tui_auth, server.url(), AgentMode::Leader);
    agent_tui_telemetry::external::suppress_external_otel_until_settings();
    assert!(!agent_tui_telemetry::external::is_settings_gate_open());
    agent.maybe_fetch_post_auth_settings().await;
    assert!(
...
async fn same_credential_refresh_does_not_flap_resolved_gate() {
    ...
    let (agent, _rx) =
        build_agent_with_auth_and_proxy(agent_tui_auth.clone(), server.url(), AgentMode::Leader);
    agent.otel_gate.set_resolved_for(&agent_tui_auth.user_id);
    agent_tui_telemetry::external::mark_external_otel_settings_resolved();
    assert!(agent_tui_telemetry::external::is_settings_gate_open());
    agent.refresh_remote_settings(&agent_tui_auth).await;
    assert!(
        agent_tui_telemetry::external::is_settings_gate_open(),
...
async fn settings_self_heal_refetches_after_token_rotation() {
    ...
    use crate::auth::{GrokAuth, XAI_OAUTH2_ISSUER};
    let _restore = RestoreOtelGate;
    let server = agent_tui_test_support::MockInferenceServer::start_with_required_auth(
        vec![agent_tui_test_support::MockModelEntry::new("grok-build")],
        "rotated-key",
    )
    .await
    .unwrap();
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.insert_resident(&sid, handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                agent_tui_subagent_resolution::config::SubagentRole {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use agent_tui_test_support::EnvGuard;
    use agent_tui_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROK_HOME", home.path());
    let _sim = EnvGuard::set(agent_tui_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
...
fn ann(id: &str) -> agent_tui_announcements::RemoteAnnouncement {
    agent_tui_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<agent_tui_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = agent_tui_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = agent_tui_announcements::RemoteAnnouncement {
