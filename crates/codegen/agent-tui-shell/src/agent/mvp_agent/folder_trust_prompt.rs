...
//! verdict is [`agent_tui_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = agent_tui_workspace::trust::workspace_key(cwd);
        ...
        self.session_registry.for_each_resident(|_, h| {
            if agent_tui_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a agent_tui_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a agent_tui_tools::types::CompatConfig,
