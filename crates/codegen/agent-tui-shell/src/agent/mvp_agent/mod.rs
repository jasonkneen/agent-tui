...
use agent_tui_telemetry::id::{agent_id, agent_instance_id};
use agent_tui_telemetry::session_ctx::log_event;
use agent_tui_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use agent_tui_workspace::permission::{ClientType, PermissionEvent};
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<agent_tui_workspace::envrc::EnvrcLoad>,
...
pub(crate) struct PromptResponseMeta {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<agent_tui_sampling_types::ToolOverrides>,
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a agent_tui_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[agent_tui_announcements::RemoteAnnouncement]>,
    last_emitted: &[agent_tui_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<agent_tui_announcements::RemoteAnnouncement>> {
    let current = agent_tui_announcements::filter_expired_at(
...
pub struct MvpAgent {
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        agent_tui_tools::implementations::grok_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: agent_tui_tools::implementations::grok_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: agent_tui_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<agent_tui_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<agent_tui_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &agent_tui_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<agent_tui_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !agent_tui_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = agent_tui_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use agent_tui_agent::config::is_strict_harness_agent_type;
    match (
...
impl MvpAgent {
    ...
    pub(super) fn spawn_auto_worktree_gc(&self) {
        ...
        tokio::task::spawn_blocking(move || {
            let opts = agent_tui_fast_worktree::AutoGcOptions::from_resolved(auto_gc_policy);
            if let Err(e) = agent_tui_fast_worktree::WorktreeDb::open_default()
                .and_then(|db| agent_tui_fast_worktree::maybe_auto_gc(&db, &opts))
            {
