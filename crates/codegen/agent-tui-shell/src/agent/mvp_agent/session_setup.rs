...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&agent_tui_sampling_types::ToolOverrides>,
) {
...
struct RestoredSignals {
    ...
    turn_count: u64,
    tool_call_count: u64,
    plan_mode_state: agent_tui_telemetry::events::PlanModeState,
...
impl RestoredSignals {
    fn read(
        ...
        use agent_tui_telemetry::events::PlanModeState as Reported;
        Self {
...
impl MvpAgent {
    pub(super) async fn new_session_inner(
        ...
        if product_analytics || agent_tui_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                ...
                agent_tui_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) =
            match agent_tui_workspace::session::git::discover_git_root(cwd.as_path()) {
    ...
    async fn attach_session(
        ...
        let git_root =
            agent_tui_workspace::session::git::find_git_root_from_path(cwd.as_path()).ok();
        ...
        let (model_state, response_meta) = self
            ...
            .await;
        agent_tui_telemetry::unified_log::info("session loaded", Some(session_id.0.as_ref()), None);
    ...
    async fn restore_session_code(
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            agent_tui_workspace::session::git::warn_registry_disabled_restore(session_id.0.as_ref());
        }
        let restore_checkout_allowed =
            agent_tui_workspace::session::git::restore_code_checkout_allowed(
                cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: agent_tui_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
    ...
    async fn build_attach_response_meta(
        ...
        if summary.head_commit.is_some()
            ...
            && summary.git_root_dir.as_deref().is_none_or(|root| {
                agent_tui_workspace::session::git::find_git_root_from_path(std::path::Path::new(
                    cwd.as_str(),
                ))
                .ok()
            ...
            })
        {
            ...
            let current_head =
                agent_tui_workspace::session::git::git_cli(cwd_path, &["rev-parse", "HEAD"])
                    .await
                    .ok();
