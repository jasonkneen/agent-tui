...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        agent_tui_telemetry::unified_log::info("agent initialized", None, None);
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    agent_tui_workspace::permission::cleanup_stale_permission_state(
        ...
        agent_tui_workspace::trust::migrate_legacy_hook_trust();
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    ...
                    serde_json::json!(
                        ...
                        default_auth_method_id_wire, (agent_tui_mcp::wire::MCP_SDK) :
                        ...
                        agent_tui_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(agent_tui_telemetry::events::Login {
    ...
    async fn new_session(
        ...
        agent_tui_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if !self.is_data_collection_disabled()
            || agent_tui_telemetry::external::is_active()
        {
            ...
            tokio::spawn(async move {
                let git = agent_tui_telemetry::context::collect_git_context(&cwd_str);
                let ev = agent_tui_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                agent_tui_telemetry::session_ctx::log_event_dual(internal_gate_open, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match agent_tui_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        agent_tui_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        agent_tui_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        agent_tui_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => agent_tui_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            agent_tui_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = agent_tui_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : agent_tui_workspace::session::git::RESTORE_CODE_LOG, session_id =
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use agent_tui_workspace::session::git::RestoreKind;
            let outcome = agent_tui_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = agent_tui_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    agent_tui_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = agent_tui_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = agent_tui_workspace::session::git::detect_head_divergence(
        ...
        if !self.is_data_collection_disabled() {
            log_event(agent_tui_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = agent_tui_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = agent_tui_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| agent_tui_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| agent_tui_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == agent_tui_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                agent_tui_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            agent_tui_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
