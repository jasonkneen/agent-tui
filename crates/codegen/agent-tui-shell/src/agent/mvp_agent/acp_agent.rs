...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        if agent_tui_telemetry::startup::agent_owned().is_some() {
        ...
        if let Some(auth) = self.auth_manager.current() {
            ...
            agent_tui_telemetry::unified_log::info(
                ...
                Some(
                    ...
                    "key_prefix": agent_tui_auth::bearer_suffix(&auth.key),
                    "rt_prefix": auth.refresh_token.as_deref().map(agent_tui_auth::bearer_suffix),
                }),
                ),
            );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let pre = self
            ...
            .current()
            .map(|a| (
                agent_tui_auth::bearer_suffix(&a.key).to_owned(),
                a
                    .refresh_token
                    .as_deref()
                    .map(|t| agent_tui_auth::bearer_suffix(t).to_owned()),
            ));
        ...
        let agent_tui_api_base_url = self.cfg.borrow().endpoints.agent_tui_api_base_url.clone();
        let has_byok = self
            .models_manager
            .models()
            .values()
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    ...
                    (agent_tui_mcp::wire::MCP_SDK): true,
                    ...
                    "agentVersion": agent_tui_version::VERSION,
                    "agentId": agent_id(),
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match agent_tui_sampling_types::ToolOverridesUpdate::parse(value) {
