...
use agent_tui_sampler::{AuthScheme, SamplerConfig};
use agent_tui_sampling_types::{
...
use agent_tui_tools::types::compat::{
...
pub struct EndpointsConfig {
    ...
    pub agent_tui_api_base_url: String,
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `agent_tui_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `agent_tui_api_base_url`.
    ...
    /// else `proxy_url` + `/deployment/config`. Never `agent_tui_api_base_url`, so the
    ...
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `agent_tui_api_base_url` fallback) so
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            ...
            agent_tui_api_base_url: std::env::var("GROK_XAI_API_BASE_URL")
...
pub use agent_tui_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use agent_tui_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(agent_tui_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: agent_tui_tools::types::compat::CompatCell,
...
pub(crate) use agent_tui_telemetry::config::env_telemetry_mode;
pub use agent_tui_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    pub(crate) fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                agent_tui_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub(crate) fn to_discovery_config(
        &self,
    ) -> agent_tui_agent::plugins::discovery::DiscoveryConfig {
        agent_tui_agent::plugins::discovery::DiscoveryConfig {
...
pub struct Config {
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<agent_tui_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `agent_tui_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<agent_tui_agent::config::AgentDefinition>,
    #[serde(skip)]
    ...
    pub subagents_limit_behavior:
        agent_tui_tools::implementations::grok_build::task::admission::LimitBehavior,
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, agent_tui_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, agent_tui_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<agent_tui_tools::implementations::grok_build::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<agent_tui_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<agent_tui_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub(crate) fn apply_to_definition(&self, def: &mut agent_tui_agent::config::AgentDefinition) {
    ...
    pub(crate) fn apply_to_subagent_definition(
        &self,
        def: &mut agent_tui_agent::config::AgentDefinition,
    ) {
...
pub use agent_tui_agent::config::AgentDefinition;
pub use agent_tui_agent::config::Effort;
pub use agent_tui_agent::config::PermissionMode;
pub use agent_tui_shared::ui_config::{ContextualHints, UiConfig};
...
pub struct AgentSelectionConfig {
    ...
    /// Looked up via `agent_tui_agent::discovery::by_name_in_cwd()`.
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(agent_tui_version::VERSION.to_string()),
            ...
            subagents_max_concurrent:
                agent_tui_tools::implementations::grok_build::task::admission::DEFAULT_MAX_CONCURRENT,
...
impl Config {
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<agent_tui_sampling_types::DoomLoopRecoveryPolicy> {
        use agent_tui_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
    ...
    /// `agent_tui_fast_worktree::maybe_auto_gc`, not here.
    pub(crate) fn resolve_worktree_auto_gc(&self) -> agent_tui_fast_worktree::ResolvedWorktreeAutoGc {
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use agent_tui_tools::implementations::grok_build::IMAGE_GEN_TOOL_NAME;
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use agent_tui_tools::implementations::grok_build::IMAGE_EDIT_TOOL_NAME;
    ...
    pub(crate) fn resolve_video_gen(&self) -> Resolved<bool> {
        use agent_tui_tools::implementations::grok_build::{
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        agent_tui_config::load_merged_requirements().as_ref(),
...
pub fn resolve_external_otel_config(
    client: agent_tui_telemetry::external::config::ExternalClientInfo,
) -> Option<agent_tui_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<agent_tui_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| agent_tui_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            agent_tui_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = agent_tui_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub(crate) fn apply_external_otel_remote_policy(
    ...
    let policy = agent_tui_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        agent_tui_telemetry::external::apply_remote_policy(policy);
    }
}
...
pub fn apply_remote_settings_side_effects(settings: Option<&crate::util::config::RemoteSettings>) {
    if let Some(s) = settings {
        ...
        agent_tui_config::signed_policy::apply_remote_managed_config_signature_verification(
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = agent_tui_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    agent_tui_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
    ...
    entries
        ...
        .map(|m| {
            ...
            let config = ModelEntryConfig {
                ...
                api_base_url: Some(endpoints.agent_tui_api_base_url.clone()),
                name: m.name,
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<agent_tui_workspace::permission::ClassifierPromptType>,
...
pub use agent_tui_telemetry::config::deployment_id_from_key;
...
mod tests {
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use agent_tui_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl agent_tui_sampler::BearerResolver for SessionResolver {
    ...
    fn shell_environment_policy_known_keys_track_the_policy_struct() {
        let agent_tui_tools::util::ShellEnvironmentPolicy {
            inherit: _,
            ignore_default_excludes: _,
            exclude: _,
            set: _,
            include_only: _,
        } = agent_tui_tools::util::ShellEnvironmentPolicy::default();
    ...
    async fn static_key_shadows_defined_provider_through_pipeline() {
        ...
        assert_eq!(creds.auth_type, agent_tui_chat_state::AuthType::ApiKey);
    ...
    async fn resolve_credentials_serves_cached_provider_token() {
        use agent_tui_chat_state::AuthType;
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: agent_tui_chat_state::AuthType::ApiKey,
    ...
    fn default_models_dual_endpoint_routing() {
        ...
        for (model_id, entry) in default_model_entries(&endpoints) {
            ...
            assert_eq!(
                api_key_creds.base_url, endpoints.agent_tui_api_base_url,
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        use agent_tui_chat_state::AuthType;
        use agent_tui_test_support::EnvGuard;
        let sentinel = "agent-tui-global-sentinel-key";
    ...
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("agent-tui-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: agent_tui_chat_state::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    ...
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        ...
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("agent-tui-secret"));
        let mut creds = api_key_creds("https://api.x.ai/v1");
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("agent-tui-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = agent_tui_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use agent_tui_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use agent_tui_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(agent_tui_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    /// agent-tui-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("XAI_API_KEY", "agent-tui-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    ...
    fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
        ...
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("XAI_API_KEY", "agent-tui-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("agent-tui-external-key"));
        assert_eq!(
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        ...
        let (_, models) = resolve_models_from_toml(
            ...
            agent_tui_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise agent_tui_api_base_url"
        );
    }
    ...
    /// `agent_tui_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    ...
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        ...
        let cfg = EndpointsConfig {
            agent_tui_api_base_url: inference.to_string(),
        ...
        assert_eq!(cfg.agent_tui_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `agent_tui_api_base_url`
    ...
    fn loader_managed_config_url_never_follows_inference_endpoint() {
        ...
        let cfg = Config::new_from_toml_cfg(
            &toml::from_str(
                r#"[endpoints]
                agent_tui_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
            )
            .unwrap(),
        )
    ...
    fn worktree_auto_gc_section_parses_from_toml() {
        unsafe {
            std::env::remove_var(agent_tui_fast_worktree::ENV_AUTO_GC);
            std::env::remove_var(agent_tui_fast_worktree::ENV_AUTO_GC_DRY_RUN);
            std::env::remove_var(agent_tui_fast_worktree::ENV_AUTO_GC_MAX_AGE);
        }
        let raw: toml::Value = toml::from_str(
            r#"
            [worktree.auto_gc]
        ...
        assert_eq!(
            p.max_age_by_kind
                .get(&agent_tui_fast_worktree::WorktreeKind::Subagent),
            Some(&Some(3600))
        );
        assert_eq!(
            p.max_age_by_kind
                .get(&agent_tui_fast_worktree::WorktreeKind::Manual),
            Some(&None)
        );
    }
...
"#,
    ...
    fn config_warns_on_section_typo() {
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "agent-tui-token-test"
        "#,
        )
        .unwrap();
    ...
    fn ext_client() -> agent_tui_telemetry::external::config::ExternalClientInfo {
        agent_tui_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
...
"#,
        ...
        agent_tui_config::apply_version_overrides(&mut value, &v).unwrap();
    ...
    fn remote_settings_disarm_managed_config_signatures() {
        ...
        assert!(agent_tui_config::signed_policy::verification_active());
    ...
    fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
        ...
        apply_remote_settings_side_effects(Some(&settings));
        assert!(
            !agent_tui_config::signed_policy::verification_active(),
