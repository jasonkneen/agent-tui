...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use agent_tui_tools::implementations::grok_build::task::MAX_SUBAGENT_DEPTH;
    ...
    let attribution_callback: Option<agent_tui_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        use agent_tui_tools::implementations::opencode;
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = agent_tui_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
