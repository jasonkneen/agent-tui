...
use agent_tui_tools::implementations::{grok_build, opencode};
pub(super) fn canonical_total_tokens(totals: &agent_tui_chat_state::UsageTotals) -> u64 {
    totals.total_tokens()
}
...
pub(super) async fn record_subagent_usage(
    ...
    by_model: Option<Vec<(String, agent_tui_chat_state::UsageTotals)>>,
...
/// `agent-tui-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    ...
    let mut effective_runtime = agent_tui_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = agent_tui_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        ...
        match tokio::task::spawn_blocking(move || {
            let mut builder = agent_tui_fast_worktree::WorktreeBuilder::new(&source_clone, &dest)
                .working_tree_mode(agent_tui_fast_worktree::WorkingTreeMode::PreserveWorkingTree)
    ...
    effective_runtime.capability_mode = agent_tui_subagent_resolution::intersect_capability_modes(
    ...
    agent_tui_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = agent_tui_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = agent_tui_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        agent_tui_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let initial_child_tokens = agent_tui_chat_state::estimate_conversation_tokens(&forked_conversation);
    ...
    let attribution_callback: Option<agent_tui_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = agent_tui_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<agent_tui_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                agent_tui_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == agent_tui_hooks::event::HookEventName::Stop {
                            s.event = agent_tui_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    agent_tui_agent::config::McpServerRef::Named(name) => {
                    ...
                    agent_tui_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            agent_tui_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    agent_tui_telemetry::session_ctx::log_event(agent_tui_telemetry::events::SubagentLaunched {
    ...
    let outcome = if result.success {
        agent_tui_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        agent_tui_telemetry::events::Outcome::Cancelled
    } else {
        agent_tui_telemetry::events::Outcome::Error
    };
