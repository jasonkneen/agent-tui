...
//! `agent-tui-tools` coordinator actor. This module keeps shell-specific
...
use agent_tui_agent::config::{McpInheritance, ModelOverride, PermissionMode};
use agent_tui_sampling_types::conversation::ConversationItem;
use agent_tui_subagent_resolution::ResumeSourceData;
use agent_tui_tools::implementations::grok_build::monitor::types::MonitorEventBuffer;
...
use agent_tui_tools::types::tool::ToolKind;
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn agent_tui_tools::implementations::lsp::LspBackend>>,
    ...
    /// (not an intermediate parent's) because agent-tui-tools task/coordinator.rs
    ...
    pub process_scope: Option<agent_tui_tty_utils::ProcessScope>,
    ...
    pub sampling_config: agent_tui_sampler::SamplerConfig,
    ...
    pub inherited_tool_overrides: Option<agent_tui_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn agent_tui_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<agent_tui_tools::notification::types::ToolNotificationHandle>,
    ...
    pub parent_scheduler_handle:
        Option<agent_tui_tools::implementations::grok_build::scheduler::types::SchedulerHandle>,
    ...
    pub web_search_sampling_config: Option<agent_tui_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: agent_tui_tools::implementations::grok_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: agent_tui_tools::implementations::grok_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: agent_tui_tools::implementations::grok_build::video_gen::VideoGenConfig,
    /// Resolved config for the deploy service.
    pub app_builder_deployer_config:
        agent_tui_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig,
    ...
    pub plugin_registry: Option<std::sync::Arc<agent_tui_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<agent_tui_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<agent_tui_hooks::discovery::HookRegistry>>,
    pub permission_handle: Option<agent_tui_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<agent_tui_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: agent_tui_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<agent_tui_sampler::SharedAttributionCallback>,
    ...
    pub parent_tool_definitions: Option<Vec<agent_tui_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<agent_tui_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: agent_tui_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: agent_tui_tools::types::compat::CompatConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut agent_tui_agent::config::AgentDefinition) {
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &agent_tui_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (agent_tui_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &agent_tui_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (agent_tui_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &agent_tui_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &agent_tui_sampler::SamplerConfig,
) {
...
fn session_bearer_resolver(
    ...
    base_url: &str,
) -> Option<agent_tui_sampler::SharedBearerResolver> {
...
async fn read_parent_sampling_config(
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let inherited = agent_tui_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(agent_tui_sampler::SamplerConfig, acp::ModelId)> {
    ...
    config.bearer_resolver = if !ctx.would_strip_fallback_key(config.api_key.as_deref())
        && resolved_auth_type == agent_tui_chat_state::AuthType::SessionToken
    {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[agent_tui_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<agent_tui_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<agent_tui_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        agent_tui_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[agent_tui_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<agent_tui_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        agent_tui_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &agent_tui_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<agent_tui_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = agent_tui_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def = agent_tui_subagent_resolution::discover_agent_definition(
        subagent_type,
        &resolution_context,
    )?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    agent_tui_subagent_resolution::available_agent_names(
        &agent_tui_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<agent_tui_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = agent_tui_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match agent_tui_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(agent_tui_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(agent_tui_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(agent_tui_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            agent_tui_subagent_resolution::ResolutionError::PersonaResolution(_)
            | agent_tui_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match agent_tui_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(agent_tui_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            agent_tui_subagent_resolution::ResolutionError::Unknown { .. }
            | agent_tui_subagent_resolution::ResolutionError::PersonaResolution(_)
            | agent_tui_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    agent_tui_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut agent_tui_agent::config::AgentDefinition,
) {
    let resolution_context = agent_tui_subagent_resolution::HarnessToolsetContext {
    ...
    agent_tui_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &agent_tui_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: agent_tui_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> agent_tui_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    agent_tui_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn inject_subagent_completed_prompt(
    ...
    task_completion_reservations: &Option<
        agent_tui_tools::reminders::task_completion::TaskCompletionReservations,
    >,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
    ...
    let summary =
        agent_tui_tools::implementations::grok_build::task::completion_summary(request, result);
    let message = agent_tui_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = agent_tui_tools::reminders::wrap_reminder(&message);
...
fn telemetry_owner_kind(
    request: &SubagentRequest,
) -> agent_tui_telemetry::events::SubagentOwnerKind {
    if request.owner.is_workflow() {
        agent_tui_telemetry::events::SubagentOwnerKind::Workflow
    } else if request.from_scheduler_loop() {
        agent_tui_telemetry::events::SubagentOwnerKind::SchedulerLoop
    } else {
        agent_tui_telemetry::events::SubagentOwnerKind::Task
    }
}
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &agent_tui_tools::implementations::grok_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
