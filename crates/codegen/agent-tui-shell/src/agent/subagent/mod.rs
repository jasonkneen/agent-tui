...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<agent_tui_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub parent_tool_snapshot: Option<Vec<agent_tui_sampling_types::ToolSpec>>,
...
use agent_tui_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    ...
    use agent_tui_agent::config::ModelOverride;
...
fn filter_pool_by_inheritance(
    ...
    use agent_tui_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    ...
    let mut def = agent_tui_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || agent_tui_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = agent_tui_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
fn resolve_subagent_permission_mode(
    ...
    use agent_tui_agent::config::PermissionMode;
    if is_plugin {
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<agent_tui_tools::reminders::task_completion::AutoWakeDeliveredIds>,
