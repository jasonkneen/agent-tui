...
use agent_tui_tools::implementations::grok_build::task::backend::ChannelBackend;
#[test]
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = agent_tui_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = agent_tui_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = agent_tui_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    agent_tui_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            agent_tui_subagent_resolution::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
async fn read_parent_sampling_config_live_never_strips_a_fallback_key() {
    ...
    chat.update_credentials(agent_tui_chat_state::Credentials {
        api_key: Some("agent-tui-env-fallback".to_string()),
        auth_type: agent_tui_chat_state::AuthType::SessionToken,
        alpha_test_key: None,
        client_version: None,
    });
    ...
    assert_eq!(config.api_key.as_deref(), Some("agent-tui-env-fallback"));
}
...
async fn read_parent_sampling_config_fallback_never_strips_a_fallback_key() {
    ...
    ctx.sampling_config.model = "grok-4.5".to_string();
    ctx.sampling_config.base_url = "https://api.x.ai/v1".to_string();
    ctx.sampling_config.api_key = Some("agent-tui-env-fallback".to_string());
    let (config, _) = read_parent_sampling_config(&ctx).await;
...
fn non_cursor_persona_injected_as_system_reminder() {
    use agent_tui_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &agent_tui_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn resolve_inherited_pool_named_filters() {
    ...
    let result = super::resolve_inherited_mcp_pool(
            Some(pool),
            &agent_tui_agent::config::McpInheritance::Named(vec!["atlassian".into()]),
        )
...
fn make_test_skill(
    ...
    agent_tui_tools::implementations::skills::types::SkillInfo {
        ...
        scope: agent_tui_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
