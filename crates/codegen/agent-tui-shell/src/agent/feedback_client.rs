...
// ============================================================================
// Turn delta wire types (local to agent-tui-shell until cli-chat-proxy catches up)
// ============================================================================
...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(agent_tui_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::grok_auth_credentials::GrokAuthCredentials,
    ) -> Arc<dyn agent_tui_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(agent_tui_auth::StaticAuthCredentialProvider::new(
    ...
    fn record_401_attribution_if_needed(
        ...
        stamp: Option<&agent_tui_auth::StampedBearerSuffix>,
        op: &str,
    ) {
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, agent_tui_version::VERSION)
            .header(
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let (response, stamp) = agent_tui_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
