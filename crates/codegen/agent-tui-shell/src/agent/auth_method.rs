...
pub(crate) fn agent_tui_api_key_auth_method() -> acp::AuthMethod {
...
mod tests {
    ...
    fn env_key_probe_unusable_suppresses_advertise_without_byok() {
        let _set = EnvGuard::set(XAI_API_KEY_ENV_VAR, "agent-tui-dead-key");
    ...
    fn env_key_probe_ok_still_advertises() {
        let _set = EnvGuard::set(XAI_API_KEY_ENV_VAR, "agent-tui-live-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);
