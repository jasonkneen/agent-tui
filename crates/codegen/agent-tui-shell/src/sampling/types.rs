// Re-export all types from the standalone agent-tui-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use agent_tui_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `agent-tui-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `agent-tui-sampler` crate can reference them without a circular
// dep on `agent-tui-shell`.

// Tests for the types now live in agent-tui-sampling-types crate.

use agent_tui_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `agent-tui-sampling-types` or
/// `agent-tui-tools`) so neither of those crates needs to depend on
