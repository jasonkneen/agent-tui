...
pub use self::types::*;
pub use agent_tui_sampler::ApiBackend;
pub use agent_tui_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// agent-tui-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `agent-tui-sampler` crate. We re-export the public surface here so
...
// `agent_tui_sampler::*`. The shell-side `sampling::client::Config`
