...
//!
//! The canonical error types now live in `agent_tui_sampling_types::error`.
...
pub use agent_tui_sampling_types::error::*;
...
/// This stays in agent-tui-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                agent_tui_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub(crate) fn terminal_error_data(
    message: String,
    http_status: Option<u16>,
    kind: agent_tui_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != agent_tui_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == agent_tui_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("agent-tui-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
