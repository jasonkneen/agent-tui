...
enum Shell {
    Running {
        ...
        group: Option<Arc<agent_tui_tty_utils::ProcessGroup>>,
    },
...
impl Shell {
    ...
    fn attach_group(&mut self, enrolled: Arc<agent_tui_tty_utils::ProcessGroup>) {
    ...
    fn reap_now(&mut self) {
        if self.hangup() && self.wait_exit(agent_tui_tty_utils::HANGUP_GRACE) {
            return;
        }
        self.kill();
...
pub(crate) async fn create_pty(
    ...
    if let Some(pid) = shell.pid() {
        ...
        let enrolled = tokio::task::spawn_blocking(move || {
            agent_tui_tty_utils::global_process_scope().enroll_terminal_pid(pid)
        })
        .await
    ...
    if agent_tui_tty_utils::global_process_scope().is_closed() {
...
fn reap(entry: &Arc<Mutex<PtySession>>) {
    ...
    if hung_up && wait_for_exit(entry, agent_tui_tty_utils::HANGUP_GRACE) {
        return;
    }
...
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use agent_tui_config::shell::{WindowsShell, detect_windows_shell};
    ...
    async fn scope_teardown_kills_a_background_grandchild() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let scope = agent_tui_tty_utils::ProcessScope::new();
