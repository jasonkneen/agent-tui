...
pub struct BackgroundTaskManifestEntry {
    ...
    pub cwd: String,
    #[serde(default)]
    pub kind: agent_tui_tools::computer::types::TaskKind,
}
...
pub fn format_resumed_tasks_reminder(entries: &[BackgroundTaskManifestEntry]) -> String {
    ...
    for entry in entries {
        ...
        let kind_label = match entry.kind {
            agent_tui_tools::computer::types::TaskKind::Monitor => " [monitor]",
...
mod tests {
    ...
    fn task_output_dirs_are_owner_only() {
        ...
        let path = get_task_output_path_in(home.path(), "task-perm-test", "t1");
    ...
    fn format_reminder_labels_monitor_tasks() {
        ...
        entry.kind = agent_tui_tools::computer::types::TaskKind::Monitor;
        let reminder = format_resumed_tasks_reminder(&[entry]);
