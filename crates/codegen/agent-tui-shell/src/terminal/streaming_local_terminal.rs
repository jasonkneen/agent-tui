...
/// after we stop waiting. Mirrors the bound the `agent-tui-tools` local terminal
...
fn spawn_with_argv(
    ...
    #[cfg(unix)]
    {
        ...
        let mut cmd = CommandWrap::with_new(program, |cmd| {
            ...
            #[cfg(target_os = "linux")]
            if agent_tui_sandbox::should_restrict_child_network() {
                // SAFETY: single prctl syscall (async-signal-safe).
                unsafe {
                    cmd.pre_exec(|| agent_tui_sandbox::child_net::install_child_network_filter());
                }
            }
        });
