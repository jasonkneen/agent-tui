...
/// Resolved absolute path to bash. On Unix uses the `agent_tui_config` shell
...
pub(crate) fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        agent_tui_config::shell::unix_shell_path(agent_tui_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `agent-tui-tools`.
pub use agent_tui_tools::util::pager_env;
...
/// agent-tui-pager which renders its own UI and doesn't need raw ANSI codes).
///
