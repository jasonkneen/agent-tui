...
use agent_tui_tools::util::truncate::estimate_tokens;
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = agent_tui_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = agent_tui_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = agent_tui_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        grok_version: agent_tui_version::VERSION.to_string(),
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = agent_tui_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        agent_tui_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `agent_tui_agent::prompt::agents_md::read_agents_config_with_paths`
...
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use agent_tui_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[agent_tui_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = agent_tui_tools::types::compat::CompatConfig::default();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use agent_tui_hooks::config::HookOrigin as O;
            ...
            let source = match agent_tui_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(agent_tui_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(agent_tui_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(agent_tui_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &agent_tui_agent::plugins::PluginRegistry,
    skills_config: &agent_tui_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = agent_tui_agent::prompt::skills::list_skills_with_plugins(
...
fn slash_name_counts(
    skills: &[agent_tui_agent::prompt::skills::SkillInfo],
    ...
    for skill in skills.iter().filter(|s| s.user_invocable && s.enabled) {
        ...
        *counts
            .entry(
                agent_tui_tools::implementations::skills::skill::format_skill_name(skill)
                    .to_lowercase(),
            )
            .or_default() += 1;
    }
...
fn slash_collision(
    skill: &agent_tui_agent::prompt::skills::SkillInfo,
    ...
    let qualified =
        agent_tui_tools::implementations::skills::skill::format_skill_name(skill).to_lowercase();
...
fn skill_entry_source(s: &agent_tui_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use agent_tui_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = agent_tui_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[agent_tui_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                agent_tui_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                agent_tui_agent::plugins::PluginScope::Project => Scope::Project,
                agent_tui_agent::plugins::PluginScope::User => Scope::User,
                agent_tui_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: agent_tui_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    agent_tui_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = agent_tui_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
mod tests {
    use super::*;
    use agent_tui_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use agent_tui_tools::implementations::skills::types::SkillScope;
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = agent_tui_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
