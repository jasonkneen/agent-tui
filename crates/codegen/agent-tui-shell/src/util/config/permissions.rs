...
/// `agent-tui-telemetry`; re-exported here so existing call sites continue
/// to work.
pub use agent_tui_telemetry::enums::PermissionMode;
...
/// `agent-tui-workspace`.
use agent_tui_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    // definition in `agent_tui_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = agent_tui_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
