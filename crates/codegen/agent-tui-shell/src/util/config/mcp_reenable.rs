...
use agent_tui_workspace::permission::resolution::McpServerAllowlist;
...
pub(crate) fn reenableable_disabled_stubs(
    ...
    let settings = agent_tui_workspace::permission::resolution::managed_settings();
...
mod tests {
    ...
    use agent_tui_workspace::permission::resolution::AllowedMcpServer;
    ...
    fn isolated_home() -> (
        tempfile::TempDir,
        agent_tui_test_support::EnvGuard,
        agent_tui_test_support::EnvGuard,
    ) {
        ...
        let home_guard = agent_tui_test_support::EnvGuard::set("HOME", home.path());
        let grok_guard = agent_tui_test_support::EnvGuard::set("GROK_HOME", &grok_home);
...
"#,
    ...
    #[test]
    #[serial_test::serial]
    fn toml_duplicate_url_last_wins_matches_merge() {
        let (_home, _hg, _gg) = isolated_home();
        let repo = project_repo(
            r#"
[mcp_servers.first]
...
"#,
        ...
        let merged_names: HashSet<_> = merged
            ...
            .map(|s| mcp_server_name(s).to_string())
            .collect();
        // Last TOML entry wins the URL key on both paths.
