...
/// [`agent_tui_config::env_bool`] parser (`GROK_TOOLS_FIND_BFS` /
...
/// [`agent_tui_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub(crate) fn resolve_search_tools_enabled(
    ...
    let disable = agent_tui_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = agent_tui_config::env_bool(primary).or_else(|| agent_tui_config::env_bool(alias));
...
) -> Option<agent_tui_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<agent_tui_tools::util::ShellEnvironmentPolicy>() {
...
/// the shared [`agent_tui_config::env_bool`] via `BoolFlag`). The secs env var
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            agent_tui_tools::implementations::grok_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        agent_tui_tools::implementations::grok_build::ask_user_question::response_timeout_env_secs(),
...
) -> agent_tui_tools::implementations::grok_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use agent_tui_tools::implementations::grok_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = agent_tui_tools::implementations::grok_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod shell_env_policy_tests {
    use super::*;
    use agent_tui_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
