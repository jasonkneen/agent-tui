...
pub fn auto_mode_classifier_defaults(
    ...
    agent_tui_workspace::permission::ClassifierPromptType,
    Option<agent_tui_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(agent_tui_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(agent_tui_sampling_types::ReasoningEffort::Low)
    });
