...
use agent_tui_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use agent_tui_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `agent-tui-config-types` (config
...
pub use agent_tui_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use agent_tui_config_types::PoolConfig;
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    agent_tui_config::grok_home().join("mcp_preferences.json")
}
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    agent_tui_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = agent_tui_mcp::credentials::McpCredentialStore::load_default() {
...
// `.mcp.json` discovery moved to `agent-tui-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use agent_tui_workspace::project_config::{
...
fn load_cli_plugin_registry(cwd: &std::path::Path) -> agent_tui_agent::plugins::PluginRegistry {
    ...
    let discovered = agent_tui_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    agent_tui_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    agent_tui_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = agent_tui_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = agent_tui_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = agent_tui_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
