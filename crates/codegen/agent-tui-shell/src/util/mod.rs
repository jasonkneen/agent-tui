...
// The foundation utilities live in `agent-tui-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `agent_tui_shell::util::…` users compile
// unchanged.
pub use agent_tui_shell_base::util::*;
