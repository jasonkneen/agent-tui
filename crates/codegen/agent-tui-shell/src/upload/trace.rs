...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        agent_tui_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        agent_tui_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = agent_tui_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: agent_tui_sandbox::is_active(),
    })
}
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&agent_tui_agent::plugins::PluginRegistry>,
) {
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        agent_tui_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(agent_tui_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
fn serialize_chat_history_jsonl(
    messages: &[agent_tui_sampling_types::conversation::ConversationItem],
...
pub(crate) async fn build_chat_history_then_move_capture(
    capture: agent_tui_chat_state::TurnCapture,
) -> (
    Result<Vec<u8>, SessionStateBuildError>,
    agent_tui_chat_state::TurnCapture,
) {
        ...
        match queue {
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    agent_tui_workspace::recovery::purge_spilled_items(&grok_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
