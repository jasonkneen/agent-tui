...
//!
//! 1. [`agent_tui_telemetry::unified_log::warn`] for the local
...
//!
//! [`agent_tui_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`agent_tui_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`agent_tui_sampler::SamplerConfig::attribution_callback`] at every
...
use agent_tui_sampler::{Auth401AttributionCallback, SamplingConsumer};
use agent_tui_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`agent_tui_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `agent_tui_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn agent_tui_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `agent_tui_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `agent-tui-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `agent-tui-shell` go through this helper -- the
...
mod tests {
    ...
    /// `ShellAttribution` implements `agent_tui_tools::Auth401AttributionCallback`
