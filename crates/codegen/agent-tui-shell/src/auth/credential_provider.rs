...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| agent_tui_telemetry::config::deployment_id_from_key(&a.key))
}
/// Sampler [`BearerResolver`](agent_tui_sampler::BearerResolver) over a live
...
impl WireValidBearerResolver {
    ...
    pub(crate) fn shared(auth_manager: Arc<AuthManager>) -> agent_tui_sampler::SharedBearerResolver {
...
impl agent_tui_sampler::BearerResolver for WireValidBearerResolver {
...
pub(crate) fn embedding_session_credentials(
...
) -> agent_tui_memory::EndpointScopedCredentials {
    ...
    agent_tui_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"grok-shell"`   — classic Grok CLI / TUI (agent-tui-shell)
/// - `"grok-pager"`   — new Grok Pager / TUI (agent-tui-pager)
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(agent_tui_version::VERSION, client_identifier)
...
pub(crate) fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        agent_tui_telemetry::external::set_identity(
            agent_tui_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `agent-tui-pager` and `agent-tui-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> agent_tui_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = agent_tui_telemetry::otel_layer::OtelExporterConfig {
    ...
    agent_tui_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    /// pinned in agent-tui-sampler's resolver tests): over a real
    ...
    fn wire_valid_resolver_tracks_manager_across_expiry_and_refresh() {
        use agent_tui_sampler::BearerResolver;
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let mgr = make_manager(
            &dir,
            Some(make_auth("agent-tui-session-token", ChronoDuration::hours(1))),
        );
        let api_key_provider: agent_tui_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use agent_tui_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("agent-tui-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("agent-tui-token-EX").as_str())
        );
