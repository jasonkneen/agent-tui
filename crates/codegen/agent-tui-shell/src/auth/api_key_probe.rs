//! First-party env key probe (`GET {agent_tui_api_base_url}/api-key`) before
//! advertising `xai.api_key`. BYOK is not probed.
//!
//! The base URL is the caller's effective `endpoints.agent_tui_api_base_url` so the
//! probe hits the same host turn traffic uses (`GROK_XAI_API_BASE_URL` /
//! `[endpoints] agent_tui_api_base_url`), not a hardcoded production host.
//!
...
/// (`endpoints.agent_tui_api_base_url`), not a hardcoded public default.
...
///
/// `api_base_url` is the caller's effective `endpoints.agent_tui_api_base_url`, so the
...
mod tests {
    ...
    async fn local_server_invalid_key_is_unusable() {
        ...
        let v = probe_xai_api_key_at_url("agent-tui-bad", &url, Duration::from_secs(2)).await;
    ...
    async fn local_server_blocked_key_is_unusable() {
        ...
        let v = probe_xai_api_key_at_url("agent-tui-blocked", &url, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Unusable);
    }
    ...
    async fn local_server_ok_key_is_usable() {
        ...
        let v = probe_xai_api_key_at_url("agent-tui-good", &url, Duration::from_secs(2)).await;
    ...
    async fn probes_joined_base_url_path() {
        ...
        let v = probe_xai_api_key("agent-tui-good", base, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Usable);
    }
    ...
    async fn retries_429_then_succeeds() {
        ...
        let v = probe_xai_api_key_at_url("agent-tui-retry", &url, Duration::from_secs(2)).await;
        assert_eq!(v, ApiKeyProbeVerdict::Usable);
    }
    ...
    async fn timeout_is_unknown_fail_open() {
        ...
        let v = probe_xai_api_key_at_url("agent-tui-slow", &url, Duration::from_millis(80)).await;
