...
use agent_tui_telemetry::events::ManualAuthSurface;
...
impl AuthManager {
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => agent_tui_telemetry::unified_log::info(
            ...
            Err(e) => agent_tui_telemetry::unified_log::error(
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        agent_tui_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<agent_tui_telemetry::events::ManualAuth> {
...
impl agent_tui_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
/// Build a refreshing [`ApiKeyProvider`](agent_tui_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](agent_tui_tools::types::ApiKeyProvider::current_api_key_async),
/// so it works for both OAuth/session (refreshes) and API-key auth.
pub fn shared_api_key_provider(
    auth_manager: Arc<AuthManager>,
) -> agent_tui_tools::types::SharedApiKeyProvider {
