...
use agent_tui_sampler::SamplerConfig;
...
impl WebFetchToolConfig {
    ...
    pub(crate) fn resolve_params(
    ...
    ) -> agent_tui_tools::implementations::grok_build::web_fetch::WebFetchParams {
        ...
        let allow_local = self
            .allow_local
            .or_else(|| agent_tui_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `agent_tui_tools::registry::types::ToolsetConfig` which holds
...
mod tests {
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            agent_tui_agent::config::AgentDefinition::plan(),
            agent_tui_agent::config::AgentDefinition::explore(),
        ] {
