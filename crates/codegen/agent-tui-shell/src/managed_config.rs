...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    ...
    agent_tui_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    agent_tui_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            agent_tui_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", agent_tui_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            agent_tui_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
pub fn clear_orphan() {
    ...
    if agent_tui_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `agent-tui-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = agent_tui_config::signed_policy::stored_envelope_nonce(
        &crate::util::grok_home::grok_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            agent_tui_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
fn apply_fetched(
    ...
    let verified = if agent_tui_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(agent_tui_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        agent_tui_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(agent_tui_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            agent_tui_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(agent_tui_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &agent_tui_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<agent_tui_config::signed_policy::SignatureEnvelope> {
    use agent_tui_config::signed_policy::now_unix;
    ...
    let claim = match agent_tui_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &agent_tui_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
pub fn classify_auth_mode() -> agent_tui_telemetry::startup::AuthMode {
    auth_mode(
...
) -> agent_tui_telemetry::startup::AuthMode {
    use agent_tui_telemetry::startup::AuthMode;
...
pub async fn ensure_managed_policy_present(
    ...
    agent_tui_telemetry::startup::enter(agent_tui_telemetry::startup::StartupPhase::ManagedPolicy);
    ...
    agent_tui_telemetry::startup::set_auth_mode(auth_mode(has_deployment_key, &signed_in_team));
...
fn bump_managed_rollback_floor() {
    ...
    if !agent_tui_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::grok_home::grok_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            agent_tui_config::bump_rollback_floor(&home);
        }
