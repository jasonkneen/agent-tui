...
// rmcp is quarantined in agent-tui-mcp; see that crate's docs.
...
/// `agent_tui_mcp::wire`), these methods are private to the agent↔client channel and
...
pub(crate) async fn build_mcp_status(
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(agent_tui_file_utils::events::Event::McpHealthCheck {
...
//
// Implements the `McpResourceProvider` trait from agent-tui-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `agent-tui-mcp` directly.
...
impl agent_tui_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    ...
    ) -> Result<Vec<agent_tui_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(agent_tui_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<agent_tui_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(agent_tui_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(agent_tui_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(agent_tui_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
