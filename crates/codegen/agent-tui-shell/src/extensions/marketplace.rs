...
fn load_filtered_marketplace_sources() -> Vec<agent_tui_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                agent_tui_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                agent_tui_plugin_marketplace::SourceKind::Local { path } => {
...
fn refresh_sources(
    sources: &[agent_tui_plugin_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                agent_tui_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let agent_tui_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = agent_tui_plugin_marketplace::git::default_cache_root();
            if let Err(e) = agent_tui_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
...
async fn handle_update(
    ...
    let source_identity = |s: &agent_tui_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            agent_tui_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            agent_tui_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match agent_tui_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        agent_tui_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = agent_tui_plugin_marketplace::git::default_cache_root();
            match agent_tui_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                agent_tui_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = agent_tui_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = agent_tui_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    match update_result {
        ...
        Err(agent_tui_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            agent_tui_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match agent_tui_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    agent_tui_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &agent_tui_plugin_marketplace::MarketplaceSource,
    ...
    let scan = agent_tui_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = agent_tui_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match agent_tui_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: agent_tui_plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> agent_tui_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &agent_tui_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            agent_tui_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if agent_tui_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        agent_tui_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = agent_tui_config::grok_home().join("config.toml");
...
fn remove_source_locked(source_url_or_path: &str) -> agent_tui_hooks_plugins_types::ActionOutcome {
    ...
    let is_official = agent_tui_plugin_marketplace::is_official_source_url(source_url_or_path);
...
pub(crate) fn ensure_official_marketplace_source(grok_home: &std::path::Path) {
    ...
    let toml_sources = agent_tui_plugin_marketplace::load_sources(&parsed);
    let json_sources = agent_tui_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&grok_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, agent_tui_plugin_marketplace::SourceKind::Git { url, .. }
            if agent_tui_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            agent_tui_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                agent_tui_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = agent_tui_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<agent_tui_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        agent_tui_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            agent_tui_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                agent_tui_plugin_marketplace::SourceKind::Git { url, .. }
                    if agent_tui_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            agent_tui_plugin_marketplace::SourceKind::Git { url, .. }
                if url == agent_tui_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                agent_tui_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                agent_tui_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"agent-tui-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                agent_tui_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"agent-tui-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                agent_tui_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use agent_tui_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = agent_tui_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
