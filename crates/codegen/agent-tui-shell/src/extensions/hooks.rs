...
pub(crate) fn hook_spec_to_info(spec: &agent_tui_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: agent_tui_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[agent_tui_hooks::event::HookEventName] = &[
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == agent_tui_hooks::event::MatcherPolicy::Ignored =>
        {
...
mod tests {
    ...
    fn advertised_blocking_events_are_gates() {
        use agent_tui_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use agent_tui_hooks::event::{HookEventEnvelope, HookPayload};
