...
use agent_tui_agent::plugins::discovery::PluginScope;
use agent_tui_agent::plugins::git_install::{self, UpdateStatus};
use agent_tui_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use agent_tui_plugin_marketplace::git::{self, SourceCacheLease};
use agent_tui_plugin_marketplace::{
...
fn marketplace_root_for_provenance(
    provenance: &agent_tui_agent::plugins::install_registry::MarketplaceProvenance,
...
pub(crate) fn marketplace_require_sha() -> bool {
    agent_tui_config::load_effective_config_disk_only()
        .map(|c| agent_tui_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| agent_tui_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &agent_tui_workspace::permission::resolution::MarketplaceAllowlist,
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> agent_tui_workspace::permission::resolution::MarketplaceAllowlist {
        agent_tui_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = agent_tui_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
