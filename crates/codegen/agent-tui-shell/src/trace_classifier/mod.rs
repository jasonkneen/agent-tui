...
/// two-function shape in `agent-tui-tools` (F29).
...
/// `agent-tui-tools` — existing items keep their prior content when
...
/// binary. Wraps a `agent_tui_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: agent_tui_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: agent_tui_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub struct RunArgs {
    ...
    /// `agent_tui_shell::util::grok_home::grok_home()` when `None`.
...
async fn build_sampler_client(
...
) -> Result<agent_tui_sampler::SamplingClient> {
    let default_home;
    ...
    let config = agent_tui_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..agent_tui_sampler::SamplerConfig::default()
    };
    agent_tui_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use agent_tui_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(
            ...
            Some(agent_tui_telemetry::id::agent_id().as_str())
        );
    }
