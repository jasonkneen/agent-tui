...
fn subagent_limit_counts_resolve_env_over_toml_over_remote_over_default() {
    use agent_tui_tools::implementations::grok_build::task::admission;
...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!(
            ...
            cfg.endpoints.agent_tui_api_base_url
        );
    assert_eq!(
...
fn managed_settings_disables_features_and_requirements_overrides() {
    use agent_tui_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> agent_tui_test_support::EnvGuard {
    agent_tui_test_support::EnvGuard::set(agent_tui_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
/// in `agent-tui-agent`. An ABSOLUTE plugin path is used so the merged
...
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use agent_tui_agent::plugins::{TrustStore, discover_plugins};
    use agent_tui_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROK_HOME", home.path());
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
