//! Shim — see `agent_tui_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `agent_tui_shell::instrumentation_timer!`). Keeping the macro here
...
pub use agent_tui_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `agent_tui_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `agent_tui_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `agent_tui_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `agent_tui_telemetry::instrumentation`.
#[macro_export]
