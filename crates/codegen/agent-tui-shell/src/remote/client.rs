...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn agent_tui_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    /// See: crates/codegen/agent-tui-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(agent_tui_version::VERSION),
        );
        Ok(headers)
    }
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(agent_tui_telemetry::id::agent_id)
        .await
        .ok()?;
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.agent_tui_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| agent_tui_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(agent_tui_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<agent_tui_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<agent_tui_sampling_types::CompactionsRemaining> {
...
mod tests {
    ...
    /// ApiKey → `agent_tui_api_base_url` (ApiKey, public default when unset); a custom
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("GROK_DEPLOYMENT_KEY", "agent-tui-token-ENTERPRISE") };
        let managed: toml::Value = toml::from_str(
            r#"[endpoints]
            deployment_key = "agent-tui-token-ENTERPRISE"
            agent_tui_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
        )
        .unwrap();
