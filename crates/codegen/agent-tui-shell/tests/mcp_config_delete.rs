...
"#,
  ...
  "other:https://other.example/mcp": {
    ...
    let project_override =
        agent_tui_shell::util::config::get_mcp_server_config_with_project("shared", &project)
            .unwrap();
    assert_eq!(project_override.tool_timeout_sec, Some(20));
    let compatibility_server =
        agent_tui_shell::util::config::get_effective_mcp_server_config("github.com", &project)
            .unwrap();
    assert!(matches!(
        compatibility_server.transport,
        agent_tui_shell::util::config::McpServerTransportConfig::StreamableHttp { url, .. }
            if url == "https://compat.example/mcp"
    ));
    let disabled_server =
        agent_tui_shell::util::config::get_effective_mcp_server_config("disabled", &project)
            .unwrap();
    assert!(!disabled_server.enabled);

    let deleted = agent_tui_shell::util::config::delete_mcp_server_config_at(&config_path, "shared")
        .await
        .unwrap();
    ...
    assert!(agent_tui_shell::util::config::mcp_server_defined_at(
        &user_config_path,
        "shared"
    ));
    let user_fallback =
    ...
    assert!(
        agent_tui_shell::util::config::forget_mcp_credentials("shared", &shared_url)
            .await
            .unwrap()
    );
