...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p agent-tui-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    local
        .run_until(async {
            ...
            // the same wiring: agent-tui-pager/src/app/leader_cluster/mod.rs
