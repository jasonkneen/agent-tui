...
use agent_tui_sampler::RetryPolicy;
use agent_tui_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
use agent_tui_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, RequestId, SamplerActor,
    SamplerHandle,
};
use agent_tui_test_support::sse::{
...
use agent_tui_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// ```bash
/// cargo test -p agent-tui-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = agent_tui_test_support::git_workdir();
    ...
    let mut cmd = tokio::process::Command::new(agent_tui_test_support::grok_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    agent_tui_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("GROK_HOME", grok_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("GROK_LEADER_SOCKET");

    let result = agent_tui_test_support::run_headless_with_cmd(cmd).await;
    agent_tui_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
