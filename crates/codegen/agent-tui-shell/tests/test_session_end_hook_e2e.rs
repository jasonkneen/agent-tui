...
//! debug build):
//! ```bash
//! cargo test -p agent-tui-shell --test test_session_end_hook_e2e -- --ignored
//! ```
//!
//! CI coverage of the same machinery without a built binary lives in
//! `agent_tui_shell::agent::activity` tests (the flush quiesce loop and its
//! grace expiry) and `agent_tui_pager::acp::spawn` tests (the worker join:
