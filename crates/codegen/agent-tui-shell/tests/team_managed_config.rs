...
use agent_tui_shell::config::ServingIdentity;
use agent_tui_test_support::spawn_counting_server;
...
fn write_team_auth_expiry(home: &std::path::Path, team_id: &str, expires_at: &str) {
    let scope = agent_tui_shell::auth::GrokComConfig::default().auth_scope();
...
async fn team_sync_writes_files() {
    ...
    let wrote = agent_tui_shell::managed_config::sync()
        .await
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    agent_tui_shell::managed_config::sync()
        .await
    ...
    assert!(!agent_tui_shell::config::is_managed_config_stale_for(
...
async fn withdrawn_artifact_is_removed_on_next_sync() {
    ...
    assert!(
        !agent_tui_shell::config::is_managed_config_stale_for(&team_identity("team-007")),
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = agent_tui_shell::managed_config::sync()
        .await
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !agent_tui_shell::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    assert!(
        agent_tui_shell::managed_config::managed_policy_gate().is_ok(),
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        agent_tui_shell::managed_config::managed_policy_gate().is_err(),
    ...
    write_config(&home, &url);
    write_team_auth(&home, "team-007");
    agent_tui_shell::config::mark_managed_config_synced(agent_tui_shell::config::SyncMarker {
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let cfg = agent_tui_shell::agent::config::Config::default();
    let auth_manager = std::sync::Arc::new(agent_tui_shell::auth::AuthManager::new(
        &home,
        agent_tui_shell::auth::GrokComConfig::default(),
    ));
    // `bootstrap`'s Ok type isn't `Debug`, so match rather than `expect_err`.
    let err = match agent_tui_shell::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    let gate_same_key = agent_tui_shell::managed_config::managed_policy_gate();
    ...
    let gate_switched_key = agent_tui_shell::managed_config::managed_policy_gate();
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    let gate = agent_tui_shell::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("GROK_MANAGED_CONFIG");
...
async fn logout_clears_team_config() {
    ...
    agent_tui_shell::managed_config::clear_orphan();
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    let outcome = agent_tui_shell::managed_config::run_setup().await;
    lock.unlock().unwrap();

    assert!(
        matches!(
            outcome,
            agent_tui_shell::managed_config::SetupOutcome::Installed
        ),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = agent_tui_shell::http::shared_client();
...
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("GROK_MANAGED_CONFIG", "0") };
    let outcome = agent_tui_shell::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("GROK_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        agent_tui_shell::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let pinned: agent_tui_shell::auth::GrokAuth = serde_json::from_value(serde_json::json!({
        "key": "pinned-token",
        "auth_mode": "oidc",
        "create_time": "2026-01-01T00:00:00Z",
        "expires_at": FAR_FUTURE,
    ...
    let outcome = agent_tui_shell::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        agent_tui_shell::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
...
async fn post_login_sync_is_latency_bounded() {
    ...
    assert_eq!(
        outcome,
        agent_tui_shell::managed_config::ManagedConfigSync::Failed
    );
    assert_eq!(
        *count.lock().unwrap(),
        2,
