...
//!
//! Run:
//!   cargo test -p agent-tui-shell --features dhat-heap,test-support --test test_session_load_memory \
...
use agent_tui_shell::session::storage::{JsonlStorageAdapter, StorageAdapter, prepare_replay_lines};
use agent_tui_shell::session::testkit::synth::{self, SessionSpec};
use agent_tui_test_support::env::env_parse;
...
mod rss {
    ...
    use agent_tui_test_support::resources::{RssOutcome, RssSampler};
    ...
    async fn count_replayed_notifications(session_id: acp::SessionId, cwd: PathBuf) -> u64 {
        ...
        local
            .run_until(async move {
                ...
                let loaded = agent_tui_shell::session::testkit::e2e::load_session_via_agent(
