...
/// All tests share this as GROK_HOME (the `OnceLock` in agent-tui-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = agent_tui_shell::config::load_effective_config().unwrap();
    let effective_cfg = agent_tui_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    agent_tui_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = agent_tui_shell::util::config::load_config_from_toml(&user_toml);
