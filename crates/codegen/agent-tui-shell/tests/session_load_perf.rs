...
//! [`synth`](agent_tui_shell::session::testkit::synth) generator (redundant
...
//!   cargo test -p agent-tui-shell --features test-support --test session_load_perf -- --nocapture
//!   cargo test -p agent-tui-shell --features test-support --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use agent_tui_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use agent_tui_shell::session::testkit::e2e::load_session_via_agent;
use agent_tui_shell::session::testkit::synth::{self, SessionSpec};
...
async fn full_session_load_e2e() {
    ...
    let _ = tracing_subscriber::registry()
        .with(agent_tui_shell::instrumentation::layer::<Registry>())
        .try_init();
    ...
    local
        .run_until(async move {
            ...
            let _ = agent_tui_shell::instrumentation::finalize();
