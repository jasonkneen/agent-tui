...
fn chat_completion_bodies(server: &agent_tui_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .into_iter()
        ...
        .filter(|r| {
            !r.header("x-grok-req-id")
                .is_some_and(|id| id.starts_with("agent-tui-turn-summary-"))
        })
