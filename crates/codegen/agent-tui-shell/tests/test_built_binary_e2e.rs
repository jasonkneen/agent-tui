//! Built-binary end-to-end tests for the grok (agent-tui-pager) binary.
//!
...
//! ```bash
//! cargo test -p agent-tui-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
...
async fn test_headless_streaming_messages_json_carries_per_response_metadata() {
    use serde_json::json;
    use agent_tui_test_support::scripted::{ScriptedResponse, SseEvent};
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        ...
        agent_tui_test_support::scripted::ScriptedResponse::sse(
            agent_tui_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
