...
Workspace directories are suffixed with a short hash for uniqueness (e.g. `agent-tui-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
GROK_LOG_FILE=/tmp/grok-debug.log RUST_LOG="info,agent_tui_shell::auth=debug" grok
```
