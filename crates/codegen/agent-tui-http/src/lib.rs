...
//! `agent_tui_sampler::shared_http` (one HTTP/2 pooled client plus
...
//! `GROK_EXTRA_CA_BUNDLE` via `agent_tui_extra_ca` (see env-var registry).

use std::sync::OnceLock;

use agent_tui_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `agent_tui_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `agent_tui_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use agent_tui_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `agent-tui-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `agent-tui-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `agent-tui-telemetry` consumes
// the same type via `agent_tui_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use agent_tui_sampler::OriginClientInfo;
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn agent_tui_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(agent_tui_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
pub fn shared_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            agent_tui_extra_ca::with_extra_root_certificates_blocking(
