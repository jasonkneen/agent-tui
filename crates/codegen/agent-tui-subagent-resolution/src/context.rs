...
//!
//! Extracted from `agent-tui-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `agent-tui-chat-state::compaction_utils::strip_system_tags` which
...
/// `agent-tui-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use agent_tui_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(agent_tui_sampling_types::synthesized_reasoning_item(text))
    }
