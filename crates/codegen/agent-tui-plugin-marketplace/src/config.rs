...
//! [[marketplace.sources]]
//! name = "xAI Official"
//! git = "https://github.com/agent-tui-org/agent-tui-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
pub fn env_require_sha() -> bool {
    agent_tui_config::env_bool("GROK_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
pub fn load_extra_sources_from_settings(existing: &[MarketplaceSource]) -> Vec<MarketplaceSource> {
    let roots: Vec<PathBuf> = [
        agent_tui_config::user_grok_home(),
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("agent-tui-org") && branch.as_deref() == Some("main"))
        );
    }
