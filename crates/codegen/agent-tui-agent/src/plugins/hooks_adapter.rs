...
//! `agent-tui-hooks`' parser and inject plugin env vars. Not a second engine.
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        spec.layer = agent_tui_hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            agent_tui_hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
