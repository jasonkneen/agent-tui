...
/// Lives here (not in agent-tui-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("AGENT_TUI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    pub fn resolve_install_dir() -> PathBuf {
        ...
        agent_tui_config::grok_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
    ...
    fn read_install_dir_from_config() -> Option<PathBuf> {
        let root = agent_tui_config::load_effective_config_disk_only().ok()?;
