...
async fn read_agents_config_with_options(
    ...
    let global_dir = agent_tui_tools::util::grok_home::grok_home();
...
/// Used by `agent-tui-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        let content = if is_rules_file {
            agent_tui_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
