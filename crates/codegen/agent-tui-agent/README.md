...
This crate extracts a first-class `Agent` type from `agent-tui-shell`.
...
`agent-tui-shell`, another in-process host, or a headless batch runner.
...
```rust
use agent_tui_agent::{AgentDefinition, AgentBuilder};
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  agent-tui-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`agent-tui-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `agent-tui-agent` depends on it for tool setup.
- **`agent-tui-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `agent-tui-agent` (AGENTS.md
...
```bash
# Check
cargo check -p agent-tui-agent

# Test
cargo test -p agent-tui-agent

# Clippy
cargo clippy -p agent-tui-agent --fix --allow-dirty
