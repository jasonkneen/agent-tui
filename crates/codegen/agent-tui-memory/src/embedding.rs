...
pub(super) fn build_middleware_client(
    ...
    agent_tui_http::with_auth_retry(agent_tui_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn agent_tui_auth::AuthCredentialProvider> = std::sync::Arc::new(
        agent_tui_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl agent_tui_auth::HttpAuth for NoopHttpAuth {
