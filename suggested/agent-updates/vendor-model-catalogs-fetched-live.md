...
- **Claude** — the Claude Code Agent SDK harness via `claude -p --output-format json`
...
**Grounding.**
- `docs/LOCAL_CLI_AUTH.md`, "TUI usage (shipped)": "After `/runtime codex` or `/runtime claude`, Agent TUI loads that vendor's model catalog and **`/model` switches it**. Selection is stored in `~/.agent-tui/runtime.toml` (`codex_model` / `claude_model`)."
- Same doc, immediately following: "**Codex** — `model/list` over warm app-server" and "**Claude** — Claude Code Agent SDK harness via `claude -p --output-format json`".
